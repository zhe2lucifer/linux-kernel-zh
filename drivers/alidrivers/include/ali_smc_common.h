...
#define SMC_CMD_DEACTIVE            _IO('o', 529)                       //!< Deactivate the smart card. 
...
#define SMC_CMD_GET_CLASS			_IOR('o', 548, int32)               //!< Get the currently selected class. 
#define SMC_CMD_SET_CLASS			_IOW('o', 549, int32)               //!< Set new class selection if previous selection fails. 
#define SMC_CMD_T1_TRANS			_IOWR('o', 550, smc_t1_trans_t)     //!< T1 communication, needs to build block frame by driver.
#define SMC_CMD_T1_XCV              _IOWR('o', 551, smc_t1_xcv_t)       //!< T1 communication. 
#define SMC_CMD_T1_NEGO_IFSD        _IOW('o', 552, smc_t1_nego_ifsd_t)  //!< Set T1 negociation maximun information field size.
...
#define SMC_CMD_SET_PROTOCOL		_IOW('o', 558, uint32) 				//!< Set protocol type
#define SMC_CMD_GET_KUMSGQ			_IOR('o', 559, uint32)

#define SMC_CMD_GET_WCLK			_IOR('o', 560, uint32)
#define SMC_CMD_SET_RESET_MODE      _IOW('o', 561, uint32)				//!< Set reset mode: 0: cold reset; 1: warm reset
#define SMC_CMD_ACTIVE            	_IO('o', 562)                       //!< activate the smart card. 
#define SMC_CMD_DISABLE_PPS 		_IOW('o', 563, uint32)				//!< Disable pps when reset:1:disable pps,0:enable pps>
