...
#ifndef CONFIG_ALI_PLATFORM	// For Uboot compile error.
...
/* Boolean type, with possible values of TRUE(1) or FALSE(0) only. */
/* Boolean type, with possible values of TRUE(1) or FALSE(0) only. */
typedef int					_BOOL;
typedef void				VOID;
...
enum
{
    ...
    INNO_ERROR_INTERRUPT_INSTALL,         //!<  9, Interrupt installation failed.
    INNO_ERROR_INTERRUPT_UNINSTALL,       //!<  10, Interrupt uninstallation failed.
    INNO_ERROR_TIMEOUT,                   //!<  11, Timeout occured.
    INNO_ERROR_DEVICE_BUSY,               //!<  12, Device is busy currently.
    INNO_ERROR_SUSPENDED,                 //!<  13, Device is in D1 or D2 state.
    INNO_ERROR_DEVICE_DISABLED,           //!<  14, Device is in disabled state.
    INNO_ERROR_OTHERS,                    //!<  15, Other errors.
};
