...
#define IO_DSC_CALC_CMAC                       (IO_DSC_BASE + 32)
//!< Calculate the cmac.


#define VIRTUAL_DEV_NUM 16
...
typedef struct des_init_param 
{
	...
	unsigned char continuous; //! from cap210 on, IC supports ts/pure data chaining. 0: separate; 1: continuous
	unsigned char ts_format; // 0<ts-188>, 1<ts-200>, 2<ts-188-ltsid>
	unsigned char sc_mode; // scramble control mode. (0: mode1 && 1: mode2)
...
typedef struct KEY_PARAM
{
	...
	*/		
	unsigned char  even_key_locate; //!< 0:host, 1:kl - only KEY_FROM_MIX cares this para
	unsigned char  odd_key_locate; //!< 0:host, 1:kl - only KEY_FROM_MIX cares this para
	unsigned char kl_sel; //!< KL0[0], KL1[1], KL2[2], KL3[3], KL4[4] 
...
typedef struct
{
	...
	unsigned char iv_odd[16];
	unsigned char iv_even[16];
	unsigned int parity;
	unsigned int key_from;
	...
	unsigned char no_odd;
		/*!< If the 'no_odd = 1' is set, the odd clear key will not be updated/configured.
			If the 'no_odd = 0' is set, the odd clear key will be updated/configured.
		*/
	unsigned char  even_key_locate;
		//!< 0:host, 1:kl - only KEY_FROM_MIX cares this para
	unsigned char  odd_key_locate;
		//!< 0:host, 1:kl - only KEY_FROM_MIX cares this para
	unsigned char pos;
	unsigned char kl_sel;
...
typedef struct SHA_DEV
{
	...
	int (*digest)(struct SHA_DEV * , unsigned char *, 
					unsigned char *,unsigned int );
	int (*Ioctl)( struct SHA_DEV * ,unsigned int, unsigned int);
	unsigned char id_number;	
	int (*update)(struct SHA_DEV *, unsigned char *,
					unsigned int);
}SHA_DEV,*pSHA_DEV;
...
/*! @struct DSC_CALC_CMAC
 *   @brief Calculate the cmac.
 */
typedef struct DSC_CALC_CMAC
{
	unsigned int default_area_start;
	unsigned int default_area_size;
	unsigned int cmac_addr;
}DSC_CALC_CMAC,*pDSC_CALC_CMAC;



/*struct dsc_see_ids {
	unsigned int see_dsc_id;
	unsigned int see_aes_id[VIRTUAL_DEV_NUM];
	unsigned int see_des_id[VIRTUAL_DEV_NUM];
	unsigned int see_csa_id[VIRTUAL_DEV_NUM];
	unsigned int see_sha_id[VIRTUAL_DEV_NUM];
};*/

/* dsc function enums for alirpcng*/
enum dsc_funcs {
	ALIRPC_RPC_des_decrypt = 0,
	ALIRPC_RPC_des_encrypt,
	ALIRPC_RPC_des_ioctl,
	ALIRPC_RPC_sha_ioctl,
	ALIRPC_RPC_sha_digest,
	ALIRPC_RPC_aes_encrypt,
	ALIRPC_RPC_aes_decrypt,
	ALIRPC_RPC_aes_ioctl,
	ALIRPC_RPC_dsc_ioctl,
	ALIRPC_RPC_DeEncrypt,
	ALIRPC_RPC_dsc_get_free_stream_id,
	ALIRPC_RPC_dsc_get_free_sub_device_id,
	ALIRPC_RPC_dsc_set_sub_device_id_idle,
	ALIRPC_RPC_dsc_set_stream_id_idle,
	ALIRPC_RPC_csa_decrypt,
	ALIRPC_RPC_csa_ioctl,
	ALIRPC_RPC_dsc_api_attach,
	ALIRPC_RPC_dsc_api_detach,
	ALIRPC_RPC_dsc_create_sbm_task,
	ALIRPC_RPC_dsc_delete_sbm_task,
};

/*dsc and sha function enums for alirpc*/
enum HLD_DSC_FUNC{
    /*TRNG*/
	...
	/*SBM*/
    FUNC_DSC_CREATE_SBM_TASK,
    FUNC_DSC_DELETE_SBM_TASK,
    FUNC_SHA_CREATE_SBM_TASK,
    FUNC_SHA_DELETE_SBM_TASK,

    /*SESSION_MGR*/
    FUNC_DSC_SESSION_HDL_MAP,
    FUNC_DSC_SESSION_HDL_UNMAP,
    FUNC_DSC_SESSION_KEY_MAP,
    FUNC_DSC_SESSION_KEY_UNMAP,
};

#define DSC_SBM_NR_NODES (1024)

struct dsc_sbm_packet {
	unsigned char *input;
	unsigned char *output;
	unsigned int len;
	unsigned int dev_hdl;
	unsigned short sid;
	unsigned char algo;
	unsigned char crypto;
	unsigned int type; /*0: raw, 188: ts188; 200: ts200*/
};

/*! @struct DSC_SESSION_PARAM
 *   @brief for session manager.
 */
typedef struct dsc_session_hdl_map_param {
	void * see_sub_dev;
	unsigned int stream_id;
	unsigned int algo_module;
