...
#define PVR_RPC_IO_CAPTURE_PVR_KEY          (PVR_RPC_IO_BASE + 7)
#define PVR_RPC_IO_RELEASE_PVR_KEY          (PVR_RPC_IO_BASE + 8)
#define PVR_RPC_IO_PVR_PLAYBCK_SET_KEY      (PVR_RPC_IO_BASE + 9)
#define PVR_RPC_IO_RAW_DECRYPT_EVO_SUB          (PVR_RPC_IO_BASE + 10)
...
#define PVR_RECORD_BLOCK_HEADER_PKT_SYNC (0x32323232)
...
typedef struct PVR_BLOCK_ENC_PARAM
{
    ...
    UINT32  reencrypt_type;               //[input]1:c0100 ts reencrypt,other future extend
}PVR_BLOCK_ENC_PARAM;
...
{
    UINT8  *input_key;         //[input],configure pvr encrypt key to see.
    UINT32 valid_key_num;       //[input],keys number
    UINT32 pvr_key_length;      //[input],every key length,byte unit
    UINT32 qn_per_key;          //[input],every key encrypt quantam number
    UINT32 quantum_size;        //[input],every quantam size,the valure must be 188 multiple,byte unit,
    UINT16 stream_id;           //[input],stream id
} PVR_KEY_PARAM;
...
typedef struct pvr_record_header_pkt_t
{
	UINT32 sync;		//sync indicator, always equal to PVR_RECORD_BLOCK_HEADER_PKT_SYNC.
	UINT16 pkt_num;		//Total packet number in this block.
	UINT16 ifm_offset;	//I-frame packet offset, in packet.
} PVR_RECORD_HEADER_PKT;
