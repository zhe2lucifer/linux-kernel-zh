...

{
	...
	uint32	sys_clk_trigger : 1; 		//!< Not used any more.

	uint32	gpio_cd_trigger : 1;		//!< Not used any more.								

	uint32	gpio_cs_trigger : 1;		//!< Not used any more. 
	...
	uint32	apd_disable_trigger: 1;		//!< 0, enable auto pull down IO when detected parity error. 

										//!< 1, disable auto pull down feature and do not check to see if the receiver has pulled the I/O line low to indicate a parity error.  
	...
	uint32	gpio_vpp_trigger : 1;		//!< Not used any more. 

	uint32 	disable_pps: 1;				//!< Whether disable PPS or not. 

	uint32	invert_power: 1;			//!< Whether invert to supply power or not.

	uint32	invert_detect: 1;			//!< Whether invert to detect card or not.

	uint32	class_selection_supported: 1;	 	//!< Whether support multiple operation classes or not.

	uint32 	board_supported_class: 6;			//!< Supported operation class. 

	uint32  board_class: 2;						//!< Current selected operation class. 

	uint32 	en_power_open_drain:1;				//!< Whether enable power pin drain open circuit or not. 

	uint32 	en_clk_open_drain:1;				//!<  Whether enable clock pin drain open circuit or not.

	uint32 	en_data_open_drain:1;				//!<  Whether enable data pin drain open circuit or not.

	uint32 	en_rst_open_drain:1;				//!< Whether enable reset pin drain open circuit or not.
	...
	uint8   force_tx_rx_cmd;			//!< Command forced to send/receive. 

	uint8   force_tx_rx_cmd_len;		//!< Command length forced to send/receive. 

	uint8   intf_dev_type;				//!< Not used any more. 
	...
	uint32	ext_cfg_tag;				//!< Not used any more.

	void	*ext_cfg_pointer;			//!< Not used any more.

	void	(*class_select)(enum class_selection );	//!< Callback function of card operation class. 

	int32	use_default_cfg;						//!< Whether use the default configuration parameters or not. 
