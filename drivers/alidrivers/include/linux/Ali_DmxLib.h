...
#define ALI_DMX_SEC_MATCH_MAX_LEN     32 //!< The maximum filter depth of section.
#define ALI_DMX_TS_STREAM_MAX_PID_CNT 96 //!< The maximum PID filter of one ts stream.


/*! @enum ALI_DMX_ERR_CODE
@brief ALI DMX LIB error code.
	When calling ALI DMX LIB interface, please check the return value.
       According to the return value, we can find the reason for failure.
*/
enum ALI_DMX_ERR_CODE
{
    ALI_ERR_DMX_OK = 0,//!<OK
...
@brief Parameter of ts stream. A ts stream can receive multiple PID TS packets at the same time. Users can be specified in the parameter with multiple pid info.
*/
struct Ali_DmxTsStreamParam
{  
    UINT32 PidList[ALI_DMX_TS_STREAM_MAX_PID_CNT]; //!< PID array. The array element is pid value. 
    INT32  PidCnt; //!< The number of PID which needs to receive.
    UINT32 NeedDiscramble[ALI_DMX_TS_STREAM_MAX_PID_CNT]; //!<PID descramble attribute array. PidList array element and NeedDiscramble array element have one to one correspondence.1-needs to be descrambled 0-not need.
	UINT32 TsFltId[ALI_DMX_TS_STREAM_MAX_PID_CNT]; //!<Index array of PID filter.PidList array element and TsFltId array element have one to one correspondence.
};
...
{  
	...
	UINT32 NeedDiscramble;//!< 1-audio needs to be descrambled 0-not need.
};
...
{  
	...
	UINT32 NeedDiscramble;//!< 1-video needs to be descrambled 0-not need.
};
...
{  
	...
	UINT32 NeedDiscramble;//!< 1-pcr needs to be descrambled 0-not need.
};
...
struct Ali_DmxSecStreamParam
{
	...
	UINT32 NeedDiscramble;//!< 1-section needs to be descrambled 0-not need.	
};
...
{  
	...
	UINT32 NeedDiscramble;//!< 1-pes needs to be descrambled 0-not need.
}; 
...
struct Ali_DmxSecChParam
{
    ...
    UINT32 NeedNoDupPidCheck; //!< 1-Not check Pid conflict 0-check.
};
...
@brief Parameter of demux input.
*/
struct Ali_DmxSrcCtrlStreamParam
{
    ...
    UINT32                     InputPortAttr;//!< Port attribute, please refer to tsi and frontend datasheet.
...
struct ALi_DmxDevInfo
{
    UINT32 OutputDevTsRcvCnt;//!< Count of received TS packages.
};
...
struct Ali_DmxSecSlotInfo
{
    ...
    INT32 SecStreamId;//!< Corresponding section stream of section slot.
...
struct Ali_DmxSecChInfo
{
    ...
    struct Ali_DmxSecChParam ChParam;//!< Parameter of section channel from user.
