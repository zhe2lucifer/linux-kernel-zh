...
RET_CODE avsync_main_enable_get_stc(struct avsync_device *dev, UINT32 enable)
{
	...
	/*do not support any more, use RPC to get stc , Share memory is precious to AS project
		By Andreaw,2015/01/19
	*/
...
#endif
{
	...
	switch(cmd)
	{
		...
		case ALI_AVSYNC_IO_COMMAND:
		{
			...
			if(AVSYNC_IO_GET_VIDEO_SMOOTHLY_PLAY_CFG == io_param.ioctl_cmd)
			{
			...
			else if(AVSYNC_IO_GET_CURRENT_STC_RUNBACK == io_param.ioctl_cmd)
			{
				unsigned int stc_offset;
				ret = avsync_ioctl(dev, io_param.ioctl_cmd, (UINT32)&stc_offset);

				if(RET_SUCCESS == ret)
				{
					if((copy_ret = copy_to_user((void *)io_param.param, (void *)&stc_offset, sizeof(unsigned int))) !=  0)
					{
						printk("%s error line%d\n", __FUNCTION__, __LINE__);
						// Invalid user space address
						return -EFAULT;
					}
					AVSYNC_PRF("kernel: current stc offset %d\n", stc_offset);
				}
			}
			else
...
extern int avsync_debug_procfs_init(void);
extern int avsync_debug_procfs_exit(void);

static int ali_avsync_probe(struct platform_device *pdev)
{
	int ret =0;

	ret = of_get_major_minor(pdev->dev.of_node,&m_avsync_dev_t, 
			0, 1, "ali_avsync");
	if (ret  < 0) {
...
static int ali_avsync_remove(struct platform_device *pdev)
{
...
static const struct of_device_id ali_avsync_of_match[] = {
       { .compatible = "alitech, avsync", },
       {},
};
MODULE_DEVICE_TABLE(of, ali_avsync_of_match);

static struct platform_driver ali_avsync_driver = {
       .driver    = {
               .name  = "ALi avsync",
               .owner = THIS_MODULE,
               .of_match_table = ali_avsync_of_match,
       },
       .probe     = ali_avsync_probe,
       .remove    = ali_avsync_remove,
};
module_platform_driver(ali_avsync_driver);
