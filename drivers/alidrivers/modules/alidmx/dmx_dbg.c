...
static __u32 ali_dmx_dbg_hw_reset = 0;
__u32 ali_dmx_dbg_log_enable = 0;
__u32 ali_dmx_see_dbg_log_enable = 0;
...
extern void dmx_hw_reset(void);
extern void sed_set_see_dbg(__u32 mode);
...
)
{
    ...
    for(;;)
    {
		...
		*/
         ...
         */
        msleep(ali_dmx_dbg_see_show_interval);				
    }
...
static int dmx_dbg_hw_reset_set(void *data, u64 val)
{
    *(u32 *)data = val;
...
DEFINE_SIMPLE_ATTRIBUTE(dmx_dbg_hw_reset_fops, NULL,
	                    dmx_dbg_hw_reset_set, "%llu\n");

static int dmx_dbg_log_enable(void *data, u64 val)
{
    *(u32 *)data = val;
...
DEFINE_SIMPLE_ATTRIBUTE(dmx_dbg_log_enable_fops, NULL,
	                    dmx_dbg_log_enable, "%llu\n");


static int dmx_see_dbg_log_enable(void *data, u64 val)
{
    *(u32 *)data = val;

	if(ali_dmx_see_dbg_log_enable)
	{
		sed_set_see_dbg(1);
	}
	else
...
DEFINE_SIMPLE_ATTRIBUTE(dmx_see_dbg_log_enable_fops, NULL,
	                    dmx_see_dbg_log_enable, "%llu\n");
...
)
{
    ...
    fs_entry = debugfs_create_file("dmx_hw_reset", 0644, ali_dmx_dbg_debugfs_root,
                                   &ali_dmx_dbg_hw_reset, &dmx_dbg_hw_reset_fops);
	if (!fs_entry)
	{
	    printk("%s,%d\n", __FUNCTION__, __LINE__);
		goto Fail;	
    ...
    fs_entry = debugfs_create_file("dmx_log_enable", 0644, ali_dmx_dbg_debugfs_root,
                                   &ali_dmx_dbg_log_enable, &dmx_dbg_log_enable_fops);
	if (!fs_entry)
	{
	    printk("%s,%d\n", __FUNCTION__, __LINE__);
		goto Fail;	
    ...
    fs_entry = debugfs_create_file("dmx_see_log_enable", 0644, ali_dmx_dbg_debugfs_root,
                                   &ali_dmx_see_dbg_log_enable, &dmx_see_dbg_log_enable_fops);
	if (!fs_entry)
	{
	    printk("%s,%d\n", __FUNCTION__, __LINE__);
		goto Fail;	
