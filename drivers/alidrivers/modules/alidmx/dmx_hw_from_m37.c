...
/* for S3821 */
#define ALI_DMX_HW_3_CTRL_BASE_S3821      	0x18038000  
#define ALI_DMX_HW_4_CTRL_BASE_S3821     	0x18039000  

/* for C3505 */
#define ALI_DMX_HW_3_CTRL_BASE_C3505      	0x18038000  
#define ALI_DMX_HW_4_CTRL_BASE_C3505     	0x18039000 
...
)
{
    ...
    switch(dev_id)
    {
        ...
        {			
			...
			{
				ctrl_base = ALI_DMX_HW_3_CTRL_BASE_S3821;
			}
			else if (ALI_C3505 == chip_id)
			{
				ctrl_base = ALI_DMX_HW_3_CTRL_BASE_C3505;
			}
			else
			{
        ...
        {			
			...
			{
				ctrl_base = ALI_DMX_HW_4_CTRL_BASE_S3821;
			}
			else if (ALI_C3505 == chip_id)
			{
				ctrl_base = ALI_DMX_HW_4_CTRL_BASE_C3505;
			}			
			else
			{
...
)
{
    ...
    switch(dev_id)
    {
        ...
        case 2:
        {
            irq_num = (__u32)INT_ALI_DEMUX3;
        }
        break;

        case 3:
        {
            irq_num = (__u32)INT_ALI_DEMUX4;
        }
        break;
...
)
{
	...
	*/
	if (cfg_buf_len >= ((65536 * 188 * 4) + (2 * DMX_SEE_BUF_SIZE)))
	{
	...
	else if (cfg_buf_len < (3 * 1024 * 1024))
	{
		hw_buf_len = (cfg_buf_len - (2 * DMX_SEE_BUF_SIZE) - (2 * DMX_DEENCRYPT_BUF_LEN));
	}
	else if (cfg_buf_len < (4 * 1024 * 1024))
	{
		hw_buf_len = (cfg_buf_len - (2 * DMX_SEE_BUF_SIZE) - (2 * DMX_DEENCRYPT_BUF_LEN)) / 2;
	}
	...
	{
		hw_buf_len = (cfg_buf_len - (2 * DMX_SEE_BUF_SIZE) - (2 * DMX_DEENCRYPT_BUF_LEN)) / 4;
	}
...
)
{
    ...
    return(AliRegGet16(base_addr + DMX_REG_HW_RD_IDX));
}
...
)
{
    ...
    return(AliRegGet16(base_addr + DMX_REG_HW_WR_IDX));
}
...
)
{
	...
	*/
    return(AliRegGet16(base_addr + DMX_REG_HW_BUF_LEN)) - 1;
#else
...
)
{
	...
	*/		
	...
	virt_addr = phys_addr | 0x80000000;
	#else
...
__s32 dmx_hw_pid_exclude_mode_set_m37
(
    __u32 dev_id,
    __u32 pid_exclude_mode
)
{
    __u32 base_addr;
	...
	if(pid_exclude_mode)
	{
	    AliRegSetBits32(base_addr + DMX_REG_CTRL, DMX_BIT_EXCLUDE_MODE_START_IDX,
	                    DMX_BIT_EXCLUDE_MODE_END_IDX, 1);
	}
	else
	{
	    AliRegSetBits32(base_addr + DMX_REG_CTRL, DMX_BIT_EXCLUDE_MODE_START_IDX,
	                    DMX_BIT_EXCLUDE_MODE_END_IDX, 0);	
	}
...
{  
    ...
    */
	...
	interface->hw_pid_exclude_mode_set = dmx_hw_pid_exclude_mode_set_m37;
...
void dmx_hw_reset(void)
{
	...
	ctrl_base = dmx_hw_id2base_m37(0);
	...
	ctrl_base = dmx_hw_id2base_m37(1);
	...
	ctrl_base = dmx_hw_id2base_m37(2);
	...
	ctrl_base = dmx_hw_id2base_m37(3);
