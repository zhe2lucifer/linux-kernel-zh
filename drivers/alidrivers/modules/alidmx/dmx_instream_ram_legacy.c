...
#ifdef DMX_SWDMX_DATA_ENGINE_DELAY
	if(((dev->dest_dev_id == ALI_SWDMX0_OUTPUT_HWIF_ID) || (dev->dest_dev_id == ALI_SWDMX1_OUTPUT_HWIF_ID)) 
		&& (dmx_data_engine_module_get_state(dev->dest_dev_id) != DMX_DATA_ENGINE_TASK_HW_STATE_RUN))
	{
		DMX_PRINTK("sw dmx_data_engine_module_init_usr\n");
		if(dev->dest_dev_id == ALI_SWDMX0_OUTPUT_HWIF_ID)
			dmx_data_engine_module_init_usr(ALI_SWDMX0_OUTPUT_HWIF_ID, ALI_SWDMX0_ENGINE_NAME, DMX_DATA_ENGINE_SRC_VIRTUAL_HW);
		else if(dev->dest_dev_id == ALI_SWDMX1_OUTPUT_HWIF_ID)
...
//          byte_cnt = dmx_data_buf_usr_wr_data(dmx, filp, dest_buf, param.data, param.len, 1);
    ...
    wr_idx = dmx_hw_buf_wr_get(dev->dest_dev_id);

    rd_idx = dmx_hw_buf_rd_get(dev->dest_dev_id);

    if (wr_idx >= rd_idx)
    {
        end_idx = dmx_hw_buf_end_get(dev->dest_dev_id);
        
        pkt_cnt = end_idx - wr_idx + rd_idx;
    }
    else
    {
        pkt_cnt = rd_idx - wr_idx;
    }
	if (pkt_cnt >= 188) {
		pkt_cnt -= 188;
    }
    ...
    if (0 != ret)
    {
		...
		case ALI_DMX_GET_TOTAL_BUF_LEN:
		{
	        end_idx = dmx_hw_buf_end_get(dev->dest_dev_id);
