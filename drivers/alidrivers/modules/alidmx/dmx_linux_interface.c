...
)
{
    ...
    */
	...
	child = of_get_child_by_name(pdev_dt->dev.of_node, interface_name);
	if (!child) {
		pr_err("Don't find child <functions> of DTS node<%s>!\n",interface_name);
		ret = alloc_chrdev_region(&output_dev->linux_dev_id, 0, 1, interface_name);
		if (ret < 0) {
			output_dev->linux_dev_id = 0;
		    return(DMX_ERR_LINUX_INTERFACE_INIT_FAIL);
		}
	}
	else{
		ret = of_get_major_minor(child,&output_dev->linux_dev_id, 0, 1, interface_name);
		if (ret  < 0) {
...
)
{
    ...
    */
	...
	if (!child) {
		...
		ret = of_get_major_minor(child,&input_dev->linux_dev_id, 0, 1, interface_name);
		if (ret  < 0) {
			pr_err("unable to get major and minor for char devive\n");
			return(DMX_ERR_LINUX_INTERFACE_INIT_FAIL);
		}
