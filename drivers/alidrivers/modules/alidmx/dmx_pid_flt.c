        ...
        for (flt_hw_idx = 0; flt_hw_idx < total_hw_flt_cnt; flt_hw_idx++)
        {
            ...
            {    
				...
				*/
				...
				else if(DMX_EXCLUDE_1FFF_PID == pid)
				{
					dmx_hw_pid_exclude_mode_set(src_hw_interface_id, 1);
				
					/* In addition to the pid 1fff, other bypass */
					//list_add_tail(&(flt_hw->link), &(hw_abstraction->hw_run_list));

					/* Set pid to hw filter.
					 */
					dmx_hw_flt_pid_set(src_hw_interface_id, flt_hw_idx, 0x1fff);
				...
				else
				{
					...
					if(dmx_data_engine_cache_typeget(src_hw_interface_id) != DMX_CACHE_TP)
					{
						/* Set pid to hw filter.
						 */
					...
					{
						dmx_hw_flt_disable(src_hw_interface_id, flt_hw_idx);	
					}
				}
    ...
    */
    if (list_empty(&(flt_hw->sw_run_list)))
    {
		...
		*/
		...
		else if(DMX_EXCLUDE_1FFF_PID == flt_hw->pid)
		{
			...
			*/
			...
			dmx_hw_pid_exclude_mode_set(flt_sw->src_hw_interface_id, 0);
		}		
		else
		{
		    /* No, it is a normal pid filter, just clost HW filter.
...
__s32 dmx_pid_flt_all_disable(__u32 src_hw_interface_id)
{
		 ...
		 hw_flt_idx++)
	{
		...
		if(hw_flt->state != DMX_PID_FLT_HW_STATE_IDLE)
		{
		    /* No, it is a normal pid filter, just clost HW filter.
			*/
			dmx_hw_flt_disable(src_hw_interface_id, hw_flt_idx);				
		}
	}
