...
#define DMX_USE_CACHE_BUF_ADDR

#define DMX_HWDMX_DATA_ENGINE_DELAY
#define DMX_SWDMX_DATA_ENGINE_DELAY
#define DMX_SEE2MAINDMX_DATA_ENGINE_DELAY

#define DMX_LINUX_OUTPUT_DEV_CNT         12
#define DMX_LINUX_INPUT_DEV_CNT          6
...
*/
#define DMX_DEENCRYPT_BUF_LEN            (64 * 20 * 188)//Must be Multiple of 64 TS pakcets to meet DSC requirement
...
#define DMX_INSTREAM_RAM_CNT             3
#define DMX_LINUX_OUTPUT_DEV_CNT_LEGACY  6
#define DMX_LINUX_INPUT_DEV_CNT_LEGACY   2
...
#define DMX_DATA_POOL_NODE_CNT           4096//9362//8192//8192//5120
...
/* For retrieving all TS packet from one TP exclude 0x1fff packet.
*/
#define DMX_EXCLUDE_1FFF_PID             0x7FFF

/* IRQ number
   */
#ifdef CONFIG_ARM
#define INT_ALI_DEMUX1        (32 + 7)
#define INT_ALI_DEMUX2        (32 + 8)
#define INT_ALI_DEMUX4        (64 + 2)
#define INT_ALI_DEMUX3        (96 + 5)

#else
#define INT_ALI_DEMUX1          (8 + 7)  //M36_IRQ_DMX1
#define INT_ALI_DEMUX2          (8 + 8)  //M36_IRQ_DMX2
#define INT_ALI_DEMUX3          (8 + 15)
#define INT_ALI_DEMUX4          (8 + 34)

#endif

extern struct platform_device *pdev_dt;
...
{
	DMX_TPSCAN_STATE_CREATE_TASK = 0,
    DMX_TPSCAN_STATE_IDLE,
...
enum DMX_ERR_CODE
{
    ...
    */
	...
	DMX_DATA_ENGINE_CACHE_ERROR = -6904,
	DMX_DATA_ENGINE_BITRATE_ERROR = -6905,
...
enum Ali_DmxCacheRetraceStatus
{
	DMX_RETRACE_STOP,
	DMX_RETRACE_SET,
	DMX_RETRACE_PRESTART,
	DMX_RETRACE_POSTSTART
};

#define DMX_BITRATE_PID_LIST_MAX_LEN 32							//!< Max length of demux bitrate pid list 

/*! @enum AliDmxBitrateParam
@brief Get Bitrate parameter
*/
typedef struct AliDmxBitrateParam
{
	unsigned short pid_list[DMX_BITRATE_PID_LIST_MAX_LEN];		/**< Calculate bitrate of pid list*/
	unsigned int pid_list_length;								/**< Calculate bitrate of pid list length*/
}AliDmxBitrateParam_S;

/*! @struct AliDmxCacheSettingParam
@brief demux cache parameter.
*/
struct AliDmxCacheParam
{
	...
	__u16 pid_list[DMX_CACHE_PID_LIST_MAX_LEN];			//!< Pid list array of demux cache setting.
};
...
struct dmx_data_engine_output
{
	...
	/* demux cache member*/
	__u32 hw_pkt_buf_cnt;

	struct AliDmxCacheParam cache_param;
	enum Ali_DmxCacheStatus cache_status;	
	__s32 cache_tp_flt_id;
	__s32 cache_pid_flt_id[DMX_CACHE_PID_LIST_MAX_LEN];

	bool cache_hwbuf_flush;

	struct Ali_DmxCacheRetraceParam cache_retrace_param;
	enum Ali_DmxCacheRetraceStatus cache_retrace_status;

	__u32 buf_swrd;
	__u32 buf_retrace_swrd;	

	/* demux bitrate */
	AliDmxBitrateParam_S m_bitrate_param;	/**< bitrate parameter*/

	__u8 bitrate_detect;

	__u32 pkts_num_bitrate_pidlist;			/**< pkts number of pid list which in bitrate parameter*/
	__u32 last_pkts_num_bitrate_pidlist;
...
struct dmx_channel
{
	...
	__u8 data_buf_orig_have_scrambler;
	struct timeval data_buf_orig_update_last_time;
    __u8 *dmx_output_device;

	/*for dsc mutiple process*/
	__u32 m_dmx_dsc_id;
	void *m_p_dmx_dsc_deen_param;
};
...
struct dmx_channel_module_legacy
{
	...
	enum Ali_Dmx_ClearStreamEncryptMode dmx_clearstream_encrypt_mode;
	enum Ali_DmxRecordMode m_dmx_record_mode;
	__u32 m_dmx_record_blocksize;
};
...
{
    struct dmx_instream_ram_legacy stream[DMX_INSTREAM_RAM_CNT];
};
...
struct dmx_hw_interface
{
    ...
    */  
	...
	__s32 (*hw_pid_exclude_mode_set)(__u32 dev_id, __u32 pid_exclude_mode);
};
...
/* In addition to the pid 1fff, other enable */
__s32 dmx_pid_flt_all_enable(__u32 src_hw_interface_id);
/* In addition to the pid 1fff, other disable */
__s32 dmx_pid_flt_all_disable(__u32 src_hw_interface_id);
...
__s32 dmx_hw_pid_exclude_mode_set(__u32 interface_id, __u32 pid_exclude_mode);
...
__s32 dmx_hw_resume_m37(__u32 dev_id, struct dmx_hw_interface *interface);
__s32 dmx_hw_deinit_m37(__u32 dev_id, struct dmx_hw_interface *interface);

__s32 dmx_hw_interface_init(__u32 hw_interface_id, enum DMX_HW_INTERFACE_TYPE hw_dev_type, __u32 hw_dev_id);
__s32 dmx_hw_interface_resume(__u32 hw_interface_id, enum DMX_HW_INTERFACE_TYPE hw_dev_type, __u32 hw_dev_id);
__s32 dmx_hw_interface_deinit(__u32 hw_interface_id, enum DMX_HW_INTERFACE_TYPE hw_dev_type, __u32 hw_dev_id);

void dmx_hw_reset(void);
...
__s32 dmx_channel_module_legacy_deinit(void);
