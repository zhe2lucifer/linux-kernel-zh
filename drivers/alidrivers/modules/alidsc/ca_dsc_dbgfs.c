	...
	struct ca_dsc_session *s)
{
	...
	seq_puts(f, "\tID\tlen\tInPage[dma_addr]"
		"\tOutPage[dma_addr]\tDONE\tSBMQ\tRQ\n");
	list_for_each_entry(sbuf, &engine->buf_put, put_node) {
		seq_printf(f, "\t%d\t%d\t%p[%08x]\t%p[%08x]\t%d\t%d\t%d\n",
