...
static void dsc_free_streamid(struct ca_dsc_session *s)
{
...
static void dsc_free_subdevice(struct ca_dsc_session *s)
{
	...
	struct ali_inst_key *key)
{
	...
	if (AES == s->sub_module) {
		...
		if (s->csa_version == CSA3) {
			key_size = 128;
		} else if ((CA_KEY_SIZE_BYTE_8 == key->key_size) || 
			(CA_KEY_SIZE_BYTE_16 == key->key_size)) {
			key_size = key->key_size * 8;
		} else {
			key_size = 64;
		}
...
struct ali_inst_key *inst_key_find_by_handle(
	...
	int key_type, struct ca_dsc_session *s)
{
	...
	if (DSC_INST_CLEAR_KEY == key_type) {
		...
		if (ret || !p_inst_key->cell) {
			...
			p_inst_key->key_size = (p_inst_key->cell->ck_size == 1) ? \
				CA_KEY_SIZE_BYTE_16 : CA_KEY_SIZE_BYTE_8;
		}
...
int ca_keep_consistent_check(struct ca_dsc_session *s, int check_cork)
{
	...
	/* keep consistent rejected when chaining activated and cork on-going */
	if (check_cork && s->ts_chaining && s->opt == CA_SET_CORK)
		return -EPERM;
	...
	int ca_format)
{
	...
	if (format == CA_FORMAT_RAW) {
		...
		/*By default, RAW data should be chained*/
		ts_chaining = 1;
	} else {
		s->dma_mode = TS_MODE;
...
static int ca_init_session(struct ca_dsc_session *s,
	int algo)
{
	int ret = -EINVAL;
	enum WORK_SUB_MODULE sub_module = DSC_INVALID_SUB_MODULE;
	...
	if (!(s->fmt_flag)) {
		dev_dbg(s->dsc->dev, "Pls CA_SET_FORMAT firstly!\n");
		return -EPERM;
	}

	if (DSC_INVALID_SUB_MODULE != s->sub_module) {
		dev_dbg(s->dsc->dev, "sub_module have been configurated\n");
		return -EPERM;
	}

	if (INVALID_ALGO(algo)) {
		dev_dbg(s->dsc->dev, "Invalid algo\n");
		return -EINVAL;
	}
	...
	int ca_opt)
{
	...
	if (s->ts_chaining == CHAINING_NONE) {
		dev_dbg(s->dsc->dev,
			"cork/uncork not supported without chaining\n");
		return -EINVAL;
	}

	if (s->opt != CA_SET_CORK && ca_opt == CA_SET_UNCORK) {
		dev_dbg(s->dsc->dev,
			"cannot uncork while no cork was requested before.\n");
		return -EPERM;
	}

	ret = ca_keep_consistent_check(s, 0);
	if (ret) {
		dev_dbg(s->dsc->dev, "cannot set opt now!\n");
		return ret;
	}
	...
	struct ca_create_otp_key *otp_key)
{
	...
	if (otp_key->otp_key_select == CA_OTP_KEY_FP){
		s->ts_chaining = 0;
		s->parity = CA_PARITY_OTP_KEY_FP;
	}
    ...
    s->algo = otp_key->algo;
    s->crypt_mode = otp_key->crypt_mode;
    s->chaining_mode = otp_key->chaining_mode;
    s->residue_mode = otp_key->residue_mode;
	return 0;
}
	...
	struct ca_pid *pid_info, struct ali_inst_key **target_key, int sc_type)
{
	...
	if (1 == sc_type) {
		...
		*/
		#if 0 //remove for VMX special request.
...
static int ca_pid_del(struct ca_dsc_session *s, struct ca_pid *pid_info)
{
	...
	if (RET_SUCCESS != ret) {
		dev_dbg(s->dsc->dev,
			"IO_ADD_DEL_PID 0x%x:0x%x failed, ret:%d\n",
			pid_param.ltsid, pid_param.pid, ret);
		ret = -EIO;
	}

	/* invalid s->tsc_flag if no more pid in the session */
	list_for_each_entry(key, &s->key_list, key_node)
	...
	struct ali_inst_key **inst_key)
{
	...
	if (INVALID_RESIDUE_MODE(up->residue_mode) &&
	...
	if (INVALID_WORK_MODE(up->chaining_mode) &&
		(up->valid_mask & CA_VALID_CHAINING_MODE)) {
		dev_dbg(s->dsc->dev, "Invalid chaining_mode\n");
		return -EINVAL;
	}

	if (INVALID_CRYPT_MODE(up->crypt_mode) &&
		(up->valid_mask & CA_VALID_CRYPT_MODE)) {
		dev_dbg(s->dsc->dev, "Invalid crypt_mode\n");
		return -EINVAL;
	}

	if (INVALID_PARITY(up->parity) &&
		(up->valid_mask & CA_VALID_PARITY)) {
		dev_dbg(s->dsc->dev, "Invalid parity\n");
		return -EINVAL;
	}

	/*find the key*/
	key = inst_key_find_by_handle(s, up->key_handle);
	if (!key)
		return -EINVAL;
...
{
	int ret;
	struct ali_inst_key *pkey;
	IV_OTHER_PARAM other_param;

	ret = ca_params_update_sc(s, up, &pkey);
	if (ret < 0)
		return ret;
	...
	if (up->valid_mask & CA_VALID_RESIDUE_MODE) {
		...
		if (CA_MODE_CTR == up->chaining_mode) {
			other_param.residue = RESIDUE_BLOCK_IS_RESERVED;
		}
	}

	if (up->valid_mask & CA_VALID_KL_FD) {
		if (pkey->key_type == DSC_INST_CLEAR_KEY)
			pkey->key_type = DSC_INST_MIX_KEY;
		
		if (pkey->kl_fd == -1 ||
			pkey->kl_fd != up->kl_fd) {

			if (pkey->cell)
				put_key_cell(pkey->cell);

			pkey->cell = NULL;
			pkey->kl_fd = up->kl_fd;
			ret = fetch_key_cell_by_fd(pkey->kl_fd, &pkey->cell);
			if (ret || !pkey->cell) {
				dev_dbg(s->dsc->dev, "ERR:fetch key cell,%d\n", ret);
				return ret;
			} else {
				get_key_cell(pkey->cell);
			}
		}
		...
		if (pkey->cell->ck_parity == 1 /*KL_CK_PARITY_ODD*/) {
			pkey->no_odd = 0;
			pkey->odd_locate = 1;
		} else if (pkey->cell->ck_parity == 2 /*KL_CK_PARITY_EVEN*/) {
			pkey->no_even = 0;
			pkey->even_locate = 1;
		} else /*KL_CK_PARITY_ODD_EVEN*/ {
			pkey->no_odd = 0;
			pkey->odd_locate = 1;
			pkey->no_even = 0;
			pkey->even_locate = 1;
	...
	other_param.handle = pkey->key_handle;
	if (up->valid_mask & CA_VALID_IV_ODD) {
		memcpy(pkey->iv_odd, up->iv_odd,
			CA_IV_SIZE_MAX);

		memcpy(other_param.iv_odd, pkey->iv_odd, 16);
		other_param.valid_mask |= UP_PARAM_IV_ODD;
	}

	if (up->valid_mask & CA_VALID_IV_EVEN) {
		memcpy(pkey->iv_even, up->iv_even,
			CA_IV_SIZE_MAX);

		memcpy(other_param.iv_even, pkey->iv_even, 16);
		other_param.valid_mask |= UP_PARAM_IV_EVEN;
	}

	other_param.no_even = pkey->no_even;
	other_param.no_odd = pkey->no_odd;
	other_param.even_key_locate = pkey->even_locate;
	other_param.odd_key_locate = pkey->odd_locate;
	if (pkey->key_type == DSC_INST_CLEAR_KEY)
		other_param.key_from = KEY_FROM_SRAM;
	else if (pkey->key_type == DSC_INST_KL_KEY)
		other_param.key_from = KEY_FROM_CRYPTO;
	else if (pkey->key_type == DSC_INST_MIX_KEY)
		other_param.key_from = KEY_FROM_MIX;

	if (pkey->cell) {
		other_param.pos = pkey->cell->pos;
		other_param.kl_sel = pkey->cell->kl_sel;
	}
...
static int ca_unset_key(struct ca_dsc_session *s, int key_id)
{
	...
	/* free subdevice and streamid if no more key in the session */
	if (list_empty(&s->key_list)) {
		dsc_free_subdevice(s);
		dsc_free_streamid(s);
	}
...
long ca_dsc_ioctl(struct file *file, unsigned int cmd, unsigned long args)
{
	...
	case CA_INIT_SESSION:
	{
		int algo;

		ret = ali_dsc_umemcpy(&algo,
			(void __user *)args, sizeof(int));
		if (0 != ret) {
			dev_dbg(s->dsc->dev, "%s\n", __func__);
			goto DONE;
	...
	case CA_UPDATE_PARAMS:
	{
		...
		memset(&up, 0, sizeof(struct ca_update_params));
		ret = ali_dsc_umemcpy(&up, (void __user *)args,
