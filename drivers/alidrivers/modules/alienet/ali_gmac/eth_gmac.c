...
#else 
#define GMAC_PHY_ADDR		1 //5 //1 //17 
#endif
//#define CONFIG_SOFT_CTL_LIGHT
...
u32 gmac_base = _GMAC_BASE;
static u32 gmac_irq = _GMAC_IRQ;
u8 stb_gmac_mac[] = {0x00, 0x90, 0xe6, 0x00, 0x00, 0x03};
static u32 gmac_mac_hi16 = 0;
static u32 gmac_mac_lo32 = 0;
u32 use_default_mac = 1;

static u32 gmac_phy_addr = GMAC_PHY_ADDR;
static u32 gmac_rx_mode = GMAC_RX_MODE_NAPI; //GMAC_RX_MODE_TASKLET;
static u32 gmac_rx_filter = GMAC_RX_FILTER_PERFECT;

static u32  mac_clk = 198 * 1024 * 1024;  // 198M
static u32 gmac_timer_frq = 0x840000; // gmac_timer_frq / 198M = seconds 
...
unsigned long dbg_runtime_val = 0; 
...
int mac_mdio_read(struct net_device *dev, int phy_addr, int reg_addr) {
	...
	u32 retval = 0;
	int i,	data_val = 0;
	u32 mii_cmd = (0xf6 << 10) | (phy_addr << 5) | reg_addr;
	u32 tmp_u32, cmd_u32;
...
mac_mdio_write (struct net_device *dev, int phy_addr, int reg_addr, int value)
{
	...
	u32 mii_cmd = (0x5002 << 16) | (phy_addr << 23) | (reg_addr << 18) | value;
	int i;
...
int mac_mdio_read_sync(struct net_device *dev, int phy_addr, int reg_addr) {
	u32 tmp_32;
    unsigned long flags = 0;
    pgmac_private pgmac = netdev_priv(dev);
	ALI_LOCK_IRQ(&pgmac->lock, flags);
    tmp_32 = mac_mdio_read(dev, phy_addr, reg_addr);
	ALI_UNLOCK_IRQ(&pgmac->lock, flags);
	return tmp_32; 
}
static void mac_mdio_write_sync (struct net_device *dev, int phy_addr, int reg_addr, int value) {
    unsigned long flags = 0;
    pgmac_private pgmac = netdev_priv(dev);
	ALI_LOCK_IRQ(&pgmac->lock, flags);
	mac_mdio_write(dev, phy_addr, reg_addr, value); 
	ALI_UNLOCK_IRQ(&pgmac->lock, flags);
}
...
void set_gmac_reg(void) {
	...
	if (gmac_phy_mode == ETH_PHY_MODE_RGMII) {
        GMAC_W32(RGMII_Rx_Clk_Select, 0x40000000);
        GMAC_W32(RGMII_Rx_DelayT, 0x4000000);
        GMAC_W32(RGMII_Tx_DelayT, 0x2000000);
        GMAC_W32(RGMII_Rx_DelayT, 0x84000000);
    }
}
void mac_hw_start(pgmac_private pgmac) {
	...
	if(gmac_phy_mode == ETH_PHY_MODE_RMII) {
		if (pgmac->link_speed == LINK_SPEED_100M) {
			rmii_speed = (u32)(RmiiCrSpeedModeBitLow); //100Mbps
		} else {
		...
		if (pgmac->link_speed == LINK_SPEED_1000M) {
			rgmii_speed = (u32)(RmiiCrSpeedModeBitHigh);	//1000Mbps
		} else if (pgmac->link_speed == LINK_SPEED_100M) {
			rgmii_speed = (u32)(RmiiCrSpeedModeBitLow); //100Mbps
		} else {
...
void mac_weird_int(pgmac_private pgmac) {
	...
	if (pgmac->link_established == false) {
        ...
        ctrl = (u16)mac_mdio_read_sync(pgmac->dev, gmac_phy_addr, PhyBasicModeCtrl);
        status = (u16)mac_mdio_read_sync(pgmac->dev, gmac_phy_addr, PhyBasicModeStatus);
        ...
        if (link_up) {
            ...
            mac_mdio_write_sync(pgmac->dev, gmac_phy_addr, 0x1d, 7);
            tmp1 = (u16)mac_mdio_read_sync(pgmac->dev, gmac_phy_addr, 0x1f);
	...
	if (pgmac->cur_isr & (ISRLinkStatus|ISRTimer)) {
        ...
        if (status & BMSRLinkStatus) {
            tmp_u16 = (u16)mac_mdio_read_sync(pgmac->dev, gmac_phy_addr, PhyNWayLPAR);
...
int mac_build_setup(struct net_device *dev) {
	...
	if(gmac_rx_filter == GMAC_RX_FILTER_HASH) {
		...
		list_for_each_entry(ha, &mc->list, list) {
			...
			hashcode = (u16)crc & GMAC_HASH_BITS;
...
bool mac_rx_hdr_chk(struct net_device *dev, prx_desc rx_desc) {
    ...
    ali_trace(15, "pHead->Es %d\n", pHead->ES); 
	if(pHead->ES) {
		if(pHead->WatchdogTimeout || pHead->LateCollision || 
           pHead->Long || pHead->Runt || pHead->CRC || pHead->Dribble) {
            return false;
		}
        ali_trace(15, "pHead->PhysicalLayerError %d\n", pHead->PhysicalLayerError); 
	} else {
		if(pHead->WatchdogTimeout || pHead->LateCollision || 
           pHead->Long || pHead->Runt || pHead->CRC) {
            ali_trace(15, "Watchdog %d LateCollision %d Long %d Runt %d CRC %d\n", pHead->WatchdogTimeout , pHead->LateCollision , 
               pHead->Long ,pHead->Runt ,pHead->CRC);
            return false;
		}
	}	
...
rx_pkt:
		...
		if (mac_rx_hdr_chk(pgmac->dev, rx_desc)) {
			...
			ali_warn("WARNING mac_rx_pkts(head error): Head(%08x).", *(u32 *)pHead);
            dev_kfree_skb_any(skb);
		}
	} /* while (pkts > 0) */
...
int mac_rx_pkts(pgmac_private pgmac, int budget)
{
	...
	while(pkts > 0) {
        ...
        if(!tmp_ret || test_bit(8, &dbg_runtime_val)) {
            ...
            ali_warn("mac_rx_pkts(head error): Head(%08x).", *(u32 *)pHead);
            //GMAC_W32(0x58, (GMAC_R32(0x58) |0x80000000)); 
            printk("---------------------------------\n");
            gmac_print(skb->data, pkt_sz + RING_CRC_SZ);
            printk("--------------------------------\n");
            dev_kfree_skb_any(skb);
            ali_warn("%s free_%d skb 0x%x\n",__FUNCTION__, i, (u32)skb);
            pgmac->rx_skb[i] = NULL;
            show_rx_desc(rx_desc, i);
            ALI_LOCK_IRQ(&pgmac->lock, flags);
            mac_init_for_link_established(pgmac);
...
int phy_detect(pgmac_private pgmac) {
	...
	if (((mode_control_reg != 0xffff) && (status_reg != 0xffff)) && ((mode_control_reg != 0) && (status_reg != 0))) {
        ali_info("%s: phy address is %d", __FUNCTION__, gmac_phy_addr);
		return 1;
	}
...
static const char gmac_ethtool_gstrings_stats[][ETH_GSTRING_LEN] = {
	...
	"rx_align_errors",	/* recv'd frame alignment error */
...
*/
        ...
        if (pgmac->cur_isr & ISRRxComplete) {
...
static int gmac_open (struct net_device *dev) {
    ...
    printk("reset gphy\n");
    gpio_enable_pin(4);
    ali_gpio_set_value(4, 0);
    msleep(2);
    ali_gpio_set_value(4, 1);
#endif
	return 0;
open_err_out:
...
int process_mac_tool_ioctl(struct net_device *dev, pgmac_private pgmac, cmd_param_t * cmd_param) {
...
static int gmac_ioctl(struct net_device *dev, struct ifreq *rq, int cmd) {
  ...
  //                rc = -EOPNOTSUPP;
  //            }
                if(test_bit(21, &dbg_runtime_val)) {
                    printk("recv rx compelete isr %d timer isr %d\n", pgmac->num_rx_complete,
                        pgmac->num_timer);
                        printk("dbg rx pkts %d dbg_tx_pkts %d\n", dbg_rx_pkts, dbg_tx_pkts);
                        printk("dbg tx failed %d dbg_tx_busy %d\n", dbg_tx_failed, dbg_tx_busy);
                }
                dbg_tx_pkts = 0;
                dbg_rx_pkts = 0;
                dbg_tx_failed = 0;
                dbg_tx_busy = 0;
                rc = process_mac_tool_ioctl(dev, pgmac, cmd_param);
                break; 
            }
...
static int cfg_tx_context_desc(pgmac_private priv, \
    ...
    if (skb->protocol == htons(ETH_P_IP)) {
        ...
        desc->ContextDescriptor.L3HeaderLen = (int)pNxthdr - (int)(skb->network_header);
...
static int gmac_attach(void) {
	...
	pgmac->mii_if.mdio_read = mac_mdio_read_sync;
	pgmac->mii_if.mdio_write = mac_mdio_write_sync;
...
void init_stb_mac(void) {
	...
	eth_node = of_find_compatible_node(NULL, NULL, "alitech,ali-gmac");
	if (!eth_node) {
		ali_error("no ethernet device \n");
		return;
	}
	addr = of_get_property(eth_node, "local-mac-address", &len);
	if (addr != NULL && len == 6) {
	    if (!is_valid_ether_addr(addr)) {
		ali_error("error mac address from dts file \n");
		return ;
	    }
	    memcpy(stb_gmac_mac, addr, ETH_ALEN);
	}
#endif
    return;
}
...
void set_soc_reg(void) {
    ...
    reg32 = SOC_R32(0x6C);
    phy_reg = 0x0;
    tmp_u16 = (u16)mac_mdio_read(NULL, gmac_phy_addr, phy_reg);
    tmp_u16 |= (1<<11);  //power down ephy 
    mac_mdio_write(NULL, gmac_phy_addr, phy_reg, tmp_u16);
    tmp_u16 = (u16)mac_mdio_read(NULL, gmac_phy_addr, phy_reg);
    printk("phy reg 0 is %x\n", tmp_u16);
...
#endif
    ali_info("SOC_R32(0) is %x\n", (u32)SOC_R32(0));
    g_chip_info = SOC_R32(0);
    g_chip_id = (SOC_R32(0) >> 12);
    ali_info("Chip Info: %x\n", g_chip_info);
    ali_info("Chip ID: %x\n", g_chip_id);
    set_soc_reg();
...
void phy_reg_set(pgmac_private pgmac) {
    ...
    if (g_chip_id == C3921B) {
        ...
        printk("remove step 9 for C3921B\n");
    } else {
        mac_mdio_write(dev, gmac_phy_addr, 0x1d, 0x8);
...
MODULE_DESCRIPTION("ALI GMAC driver");
MODULE_LICENSE("GPL");
