                              ...
                              unsigned long screen_height, unsigned long param)
{
    ...
    FB_PRF("canvas width %d height %d\n", (unsigned int)screen_width, (unsigned int)screen_height);

    if ((screen_width == 0) || (screen_height == 0) || (!posd_rect))
    {
        pr_debug("err:((screen_width == 0) || (screen_height == 0) || (!posd_rect))\n");
        return;
    }
...
int ali_vpo_create_GMA_region(struct alifb_info *info, int region_id)
{
    ...
    #else
	{
...

		{
			...
			pars.dmem_start = (gma_info->data_buffer & mask);
			...
			if(gma_create_region(dev, region_id, &pars) == RET_SUCCESS){
				FB_PRF("gma create region <%d %d %d %d> pitch %d buffer 0x%lx\n",
...
int ali_vpo_update_GMA_region(struct alifb_info *info, int region_id)
{
	...
	if(gma_info->pending_flag & ALIFB_GMA_FLAG_SCALE_PENDING)
    {
        ...
        {
            rect_parm.x = gma_info->x_off;
            rect_parm.y = gma_info->y_off;
...
#else
	{
		...
		if(gma_info->pending_flag & ALIFB_GMA_FLAG_SCALE_PENDING) {
               ...
               || (ali_sys_ic_get_chip_id() == ALI_C3505))
			{
				...
				if((pars.dmem_start & mask) != (gma_info->data_buffer & mask))
				{
                ...
                if((info->DE_layer == DE_LAYER3)&& (info->fbinfo->var.vmode & FB_VMODE_YWRAP))
                {
                    pars.rect.x = rect_parm.x;
                    pars.rect.y = rect_parm.y;
                }
		...
		if(gma_info->pending_flag & ALIFB_GMA_FLAG_RECT_PENDING) {
               ...
               || (ali_sys_ic_get_chip_id() == ALI_C3505))
            {
			    if((pars.dmem_start & mask) != (gma_info->data_buffer & mask))
			    {
                    FB_PRF("gma new data buffer 0x%lx=>0x%lx\n", pars.dmem_start, gma_info->data_buffer);
				    pars.dmem_start = gma_info->data_buffer;
			    }
            }
		...
		if(gma_info->pending_flag & ALIFB_GMA_FLAG_BUF_PENDING) {
                ...
                || (ali_sys_ic_get_chip_id() == ALI_C3505))
            {
    			...
    			if(fb_idx == 0 && (!ali_sys_ic_split_enabled()))
    			{
                    cq_phy_addr = __REG32ALI(0x18006080) | 0xA0000000;
    				...
    				while(cnt > 0)
    				{
    					if(*(block_addr) == 0xB80069E0)
    					{
    						...
    						*(block_addr + 1) = (gma_info->data_buffer & mask);
                            update = 1;
...
#ifdef CONFIG_RPC_HLD_GMA
static void ali_gma_update_region_addr(struct alifb_info *info)
{
	struct region_pars pars;
    struct gma_device *dev = NULL;
	...
	for(i = 0; i < VIDEO_MAX_GMA_REGION_NUM; i++)
    {
        ...
        {
            memset((void *)&pars, 0, sizeof(pars));
        	gma_get_region_info(dev, i, &pars);

    	    if((pars.dmem_start & mask) != (gma_info->data_buffer & mask))
    	    {
                FB_PRF("gma new data buffer 0x%lx=>0x%lx\n", pars.dmem_start, gma_info->data_buffer);
    		    pars.dmem_start = gma_info->data_buffer;
                gma_set_region_info(dev, i, &pars);
    	    }
	    }
	}
}
...
INT32 ali_gma_ioctl(struct alifb_info *info, UINT32 cmd, UINT32 param)
{
    RET_CODE ret = RET_SUCCESS;
	...
	dev = (fb_idx == 0) ? g_gma_dev0 : g_gma_dev1;
    ret = gma_io_control(dev, cmd, param);
    if(ret != RET_SUCCESS)
    {
        FB_PRF("gma ioctl %lx fail\n", cmd);
    }
#endif
...
extern void enable_semaphore_irq(struct alifb_info *info);
INT32 ali_gma_wait_for_vsync(struct alifb_info *info)
{
    ...
    if (info->support_sema)
    {
        if (fb_idx != 0)
            return -ENODEV;
        info->sema_flag = 1;
    	count = info->sema_count;
    	ret = wait_event_interruptible_timeout(info->sema_wait, count != info->sema_count , msecs_to_jiffies(50));
    	if (ret == 0)
    	{
		    return -ETIMEDOUT;
    	}
    ...
    if((ali_sys_ic_get_chip_id() == ALI_C3921)
       || (ali_sys_ic_get_chip_id() == ALI_S3821) || (ali_sys_ic_get_chip_id() == ALI_C3505)) {
        reg = (fb_idx == 0) ? 0x18006084 : 0x1800608C;
...
void ali_vpo_show_GMA_layer(struct alifb_info *info, int on)
{
	//if(info->gma_info[info->cur_region_id].valid)
	{
		...
		if (!info->standard_gma_region_init && on)
			return;

		dev = fb_idx == 0 ? g_gma_dev0 : g_gma_dev1;
		gma_show(dev, on);
        FB_PRF("gma%d onoff %d\n", fb_idx, on);
        #endif
	}
}
...
void ali_vpo_set_GMA_palette(struct alifb_info *info)
{
	//if(info->gma_info[info->cur_region_id].valid)
	{
        ...
        pars.pallette_buf = (UINT32)((((unsigned long)info->palette_buffer) & 0x1FFFFFFF) | 0xA0000000);

		pars.color_num = info->pallete_size;
		gma_set_pallette(dev, &pars);
        FB_PRF("gma set pallette, buffer 0x%lx type %d num %d\n",
...
void ali_vpo_set_GMA_alpha(struct alifb_info *info, unsigned char alpha)
{
	...
	dev = fb_idx == 0 ? g_gma_dev0 : g_gma_dev1;
    ali_gma_update_region_addr(info);
