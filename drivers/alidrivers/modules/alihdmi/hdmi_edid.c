...
void edid_support_video_format(enum pic_fmt *format)
{
...
int edid_get_audio_cea_item(unsigned short idx, SHORT_AUDIO_DESCRIPTOR_t *cea_item)
{
	unsigned short num = 0;
	SHORT_AUDIO_DESCRIPTOR_t  *short_a_descriptor = short_audio_descriptor;

	while((num<idx) && (short_a_descriptor != NULL))
	{
		num++;
		short_a_descriptor = short_a_descriptor->next;
	}
	...
	if(short_a_descriptor != NULL)
	{
		memcpy(cea_item, short_a_descriptor, sizeof(SHORT_AUDIO_DESCRIPTOR_t));
	}
	else
	{
		return false;
...
int edid_get_video_cea_item(unsigned short idx, SHORT_VIDEO_DESCRIPTOR_t *cea_item)
{
	unsigned short num = 0;
	SHORT_VIDEO_DESCRIPTOR_t  *short_a_descriptor = short_video_descriptor;
	...
	if(short_a_descriptor != NULL)
	{
		memcpy(cea_item, short_a_descriptor, sizeof(SHORT_VIDEO_DESCRIPTOR_t));
	}
	else
	{
		return false;
...
int edid_get_audio_or_video_cea_item_num(unsigned char type, int *num)
{
	unsigned short count = 0;
	SHORT_AUDIO_DESCRIPTOR_t  *short_a_audio_descriptor = NULL;
	SHORT_VIDEO_DESCRIPTOR_t  *short_a_video_descriptor = NULL;
	if(AUDIO_TYPE == type)
	{
		short_a_audio_descriptor = short_audio_descriptor;
		while((short_a_audio_descriptor != NULL))
		{
			count++;
			short_a_audio_descriptor = short_a_audio_descriptor->next;
		}
	}
	else if(VIDEO_TYPE == type)
	{
		short_a_video_descriptor = short_video_descriptor;
		while((short_a_video_descriptor != NULL))
		{
			count++;
			short_a_video_descriptor = short_a_video_descriptor->next;
		}
	}
