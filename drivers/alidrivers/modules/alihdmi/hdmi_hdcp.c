...
void hdmi_hdcp_load_keys_to_hardware(void)
{
	...
	if(hdmi_drv->hdcp.key[0] == 0x01)
		hdmi_drv->hdcp.key[0] = 0x00; 
...
void hdmi_hdcp_send_err_msg(unsigned char* message)
{
	HDCP_DEBUG("%s, msg: %s, len: %d\n", __func__, message, strlen(message));
	hdmi_drv->hdmi2usr.flag |= HDMI2USR_HDCP_MSG;
	hdmi_drv->hdmi2usr.hdcp_notify.len = strlen(message);
	memcpy(hdmi_drv->hdmi2usr.hdcp_notify.data, message, hdmi_drv->hdmi2usr.hdcp_notify.len);
...
HDCP_DEBUG("HDCP A5: Link Integrity Check (Ri Ready)\n");
    ...
    if((Ri_rx[0]!=Ri_tx[0]) || (Ri_rx[1]!=Ri_tx[1]))
    {
		...
		hdmi_hdcp_send_err_msg("fail (Tx Ri != Rx Ri)");
        return false;
    }
    else
    {
...
HDCP_DEBUG("HDCP A8: Wait for Ready. \n");
    ...
    {
    	if(false == HOT_PLUG_STATE)
		{
			HDCP_DEBUG("\t%s: Hot-Plug Out fail\n", __FUNCTION__);
			hdmi_hdcp_send_err_msg("Hot-Plug Out fail");
			return false;
		}
    ...
    if(hdmi_drv->hdcp.repeater_ready_timeout)
    {
		...
		hdmi_hdcp_send_err_msg("READY indication not received.");
...
HDCP_DEBUG("HDCP A9: HDCP Tx reads the KSVs, V(Rx) and verifies V(Tx) == V(Rx)\n");
        ...
        || ((hdmi_drv->hdcp.source_out_onlyrep == false) && (Bstatus.device_count == 0)) )
    {
		...
		hdmi_hdcp_send_err_msg("ReadKSV List.");
        return false;   // Go back to A0
    }
    ...
    if(ksv_list_bstatus == NULL)
    {
		...
		hdmi_hdcp_send_err_msg("kmalloc for ksvlist fail.");
        return false;   // Go back to A0
    }
    if(false == HOT_PLUG_STATE)
	{
    ...
    if((hdmi_drv->control.hdcp_int_status & B_V_RDY) != B_V_RDY )
    {
		...
		hdmi_hdcp_send_err_msg("Wait V(Tx) Ready fail");
	    kfree(ksv_list_bstatus);
        return false;
    }
    ...
    if((hdmi_drv->control.hdcp_int_status & B_V_MATCH ) != B_V_MATCH  )
    {
		...
		hdmi_hdcp_send_err_msg("V(Rx) and V(Tx) are not match");
    	kfree(ksv_list_bstatus);
        return false;
    }
...
{
    int                 retry_count = 0;
    unsigned char  	    An[8], AKsv[5], BKsv[5], R0_tx[2], R0_rx[2], M0[8];//, Ainfo;
    HDCP_BCAPS          Bcaps;
    HDCP_BSTATUS        Bstatus;
    
HDCP_DEBUG("hdmi_hdcp_1st_establish_share_value\n");
	hdmi_reg_set_mono_screen(true);
	//udelay(5);
    ...
    if((hdmi_drv->control.hdcp_int_status & B_AKSV_RDY) != B_AKSV_RDY )
    {
		...
		hdmi_hdcp_send_err_msg("wait AKSV Ready flag fail");
		return false;
    }
...
HDCP_DEBUG("HDCP A0: Wait for Active HDCP Rx\n");
	...
	if(retry_count == 10)
	{
		HDCP_DEBUG("\tRead BKsv fail, Reciver not support HDCP\n");
		hdmi_hdcp_send_err_msg("Read BKsv fail, Reciver not support HDCP");
		return false;
	}
...
//	hdmi_hdcp_read_from_sink(HDCP_RX_PRI_BCAPS, (unsigned char*)&Bcaps, sizeof(Bcaps));
    ...
    if( hdmi_hdcp_validate_ksv(BKsv) != true)
    {
		...
		hdmi_hdcp_send_err_msg("BKsv Not Valid!!");
		return false;
    }
    else
    {
...
HDCP_DEBUG("HDCP A2: Computations\n");
    ...
    if((hdmi_drv->control.hdcp_int_status & B_RI_RDY) != B_RI_RDY )
    {
		...
		hdmi_hdcp_send_err_msg("Wait R0(Tx) Ready fail");
        return false;
    }
...
HDCP_DEBUG("HDCP A3: Validate HDCP Rx\n");
    ...
    if((R0_rx[0]!=R0_tx[0]) || (R0_rx[1]!=R0_tx[1]))
    {
		...
		hdmi_hdcp_send_err_msg("fail (Tx R0 != Rx R0)");
        return false;
    }
    else
    {
