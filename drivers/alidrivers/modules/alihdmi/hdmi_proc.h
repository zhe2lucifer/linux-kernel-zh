...
typedef struct
{
	...
	bool                           hdmi_av_enable;    // TRUE: only enable HDMI av output(black screen and audio reset), FALSE: only disable HDMI av output
	bool			audio_transcoding_manual;
...
typedef struct
{
	unsigned char data[16];	// buffer
	unsigned long len;	// length
}CEC_MSG;

typedef struct
{
	unsigned char data[64];	// buffer
	unsigned long len;	// length
}HDCP_MSG;
