...
#define local_irq_save(flags) osal_interrupt_disable

#define local_irq_restore(flags) osal_interrupt_enable
...
static int ali_i2c_start_xfer_one_msg(struct ali_i2c *i2c)
{
	...
	if (0 == i2c->msgs->len) {
		I2C_ERR("I2C transfer ZERO length!\n");
	}
...
void __ali_i2c_dump_registers(struct ali_i2c *i2c)
{
	int i = 0;

	printk(KERN_ERR "|");
	for (; i<32; ++i) {
		printk(KERN_ERR "%.2x|", ali_reg_read(i2c, i));
	}
	printk(KERN_ERR "\n");
