...
#define IR_REPEAT_DELAY_DEFAULT	 		(600)
#define IR_REPEAT_INTERVAL_DEFAULT	 	(350)
...
static struct semaphore m_ir_sem;
...
static unsigned long g_ir_repeat_delay_rfk = IR_REPEAT_DELAY_DEFAULT;
static unsigned long g_ir_repeat_interval_rfk = IR_REPEAT_INTERVAL_DEFAULT;
...
static const struct linux_ir_key_map_t ali_linux_key_map[] =
{
	...
	{(IR_ALI01_HKEY_P_UP),		KEY_PAGEUP},
	{(IR_ALI01_HKEY_P_DOWN),	KEY_PAGEDOWN},
...
//cara.shi    2016/8/1
struct ali_ir_com_device_data
{
	//struct	cdev cdev;
	struct kumsgq *ir_kumsgq;
	struct mutex ir_mutex;
	//void    *priv;
};

struct ali_ir_com_device_data ali_ir_dev;
...
static int ali_ir_send_msg(unsigned long  code, int ir_protocol, int ir_state)
{
	...
	ret = ali_kumsgq_sendmsg(ali_ir_dev.ir_kumsgq, &msg, sizeof(msg));
	if (-1 == ret)
	{
			...
			{						
				...
				{					
					...
					if(2 != phy_code)
					{
						...
						if (match >= 0)
						{							
							if (IR_KEY_PRESSED == g_ir_key_state)
							{								
							...
							input_report_key(input, match, IR_KEY_PRESSED);								
							input_sync(input);	
							IR_PRINTK("[ %s %d ],  ir code %08x pressed, tick %ld\n", 
								__FUNCTION__, __LINE__, match, (unsigned long)(jiffies * 1000 / HZ));														
						} 
						else
						{
							key_cnt = 0;							
							...
							if (IR_KEY_RELEASE != g_ir_key_state)
							{
								...
								{									
									input_report_key(input, g_ir_match, IR_KEY_RELEASE);
									input_sync(input);
									IR_PRINTK("[ %s %d ],  ir code %08x release, tick %ld\n",
										__FUNCTION__, __LINE__, g_ir_match, (unsigned long)(jiffies * 1000 / HZ));
								}
							}
						}
					}
				...
				{			
					...
					else
					{
						repeat_tick = jiffies;
						last_repeat_width = (unsigned long)(((long)repeat_tick - (long)last_repeat_tick));		
						if (last_repeat_width > g_ir_repeat_delay_rfk)
						{		
							last_repeat_tick = repeat_tick;												
							g_ir_state = IR_KEY_REPEAT;
							if ((g_ir_match >= 0) && (IR_KEY_PRESSED == g_ir_key_state))
							{
								input_event(input, EV_KEY, g_ir_match, IR_KEY_REPEAT);
								input_sync(input);
								IR_PRINTK("[ %s %d ],  ir code %08x repeat, tick %ld\n",
									__FUNCTION__, __LINE__, g_ir_match, (unsigned long)(jiffies * 1000 / HZ));								
							}
						}
					}					
...
{		
	...
	while (!kthread_should_stop())
	{
		...
		if(((IR_KEY_PRESSED == g_ir_key_state) || (IR_KEY_REPEAT == g_ir_key_state)) && (key_cnt >= 1))
		{
			unsigned long tick;
...
static int ali_ir_open(struct inode *inode, struct file *file)
{
	ali_ir_dev.ir_kumsgq = ali_new_kumsgq();
	if (!ali_ir_dev.ir_kumsgq)
	{
		goto out0;
    }
	return 0;
...
static int ali_ir_release(struct inode *inode, struct file *file)
{
	ali_destroy_kumsgq(ali_ir_dev.ir_kumsgq);
	ali_ir_dev.ir_kumsgq = NULL;
	return 0;
}
...
#endif
{
	...
	switch(cmd)
	{
		...
		case ALI_FP_GET_KUMSGQ:
		{
			int flags = -1;
			int ret = -1;
			mutex_lock(&ali_ir_dev.ir_mutex);	
			if(copy_from_user(&flags, (int *)param, sizeof(int)))
			{
				IR_PRINTK("Err: copy_from_user\n");
				mutex_unlock(&ali_ir_dev.ir_mutex);
				up(&m_ir_sem);
				return -EFAULT;
			}
			ret  = ali_kumsgq_newfd(ali_ir_dev.ir_kumsgq, flags);
			if(ret> 0)
			{
				mutex_unlock(&ali_ir_dev.ir_mutex);
				up(&m_ir_sem);
...
static int __devinit ali_ir_init(void) 
{
	...
	mutex_init(&ali_ir_dev.ir_mutex);	
	...
	g_ir_input->rep[REP_DELAY] = IR_REPEAT_DELAY_DEFAULT;
	g_ir_input->rep[REP_PERIOD] = IR_REPEAT_INTERVAL_DEFAULT;		
	for (i = 0; i < ARRAY_SIZE(ali_linux_key_map); i++) 
	{			
