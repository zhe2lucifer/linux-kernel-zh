...
//#define IRC_TRACE
#ifdef IRC_TRACE
	#define IRC_TRACE_TYPE			IR_TYPE_LAB
...
static struct ir_attr ir_attr_lab = 
{
	...
	15000,
	{
		{IR_LEADING, PULSE_LOW | 160, PULSE_HIGH | 7386, 100},
		{IR_SPECIAL, PULSE_LOW | 160, PULSE_HIGH | 7386, 100},	/* maybe logic 0 or 1 */
		{IR_DATA, PULSE_LOW | 160, PULSE_HIGH | 4870, 100},	//logic '0'
		{IR_DATA, PULSE_LOW | 160, PULSE_HIGH | 7386, 100},	// logic '1'
		{IR_NULL, 0, 0, 0},
		{IR_NULL, 0, 0, 0},
		{IR_STOP, PULSE_LOW | 160, PULSE_HIGH | 15000, 100}, 
	},
	ir_lab, 
	NULL, 
};
 ...
 static unsigned long irc_decode(struct irc_decoder *ird, unsigned long pulse_width, unsigned long pulse_polarity)
{
	...
	IRC_DEBUG_PRINTF("[ %s %d ], pulse_width = %d, pulse_polarity = %d\n",
	...
	{		
		if (ird->decode_step == 0) /*just check leading pulse*/
		{
			...
			if (half_pol ^ (pulse_polarity << 31))
			{
				...
				#endif
				IRC_DEBUG_PRINTF("[ %s %d ], Error leading half pulse polarity!\n", __FUNCTION__, __LINE__);
			...
			if (!INRANGE(pulse_width, half_width, ird->attr->pulse[0].tolerance))
			{
				...
				if (ird->attr->type == IRC_TRACE_TYPE)
				#endif
				IRC_DEBUG_PRINTF("[ %s %d ], Leading half pulse not in range!(%d)\n", __FUNCTION__, __LINE__, pulse_width);
		...
		IRC_DEBUG_PRINTF("[ %s %d ], got half and return, step = %d\n\n\n", __FUNCTION__, __LINE__, ird->decode_step);
		return (IR_KEY_TEST_BASE + 3);
	}
	else
	{
		IRC_DEBUG_PRINTF("[ %s %d ], step = %d\n", __FUNCTION__, __LINE__, ird->decode_step);
		...
		if (ird->attr->normal)
		{
			...
			if (ird->attr->normal[ird->decode_step] == IR_DATA)
			{
				...
				IRC_DEBUG_PRINTF("[ %s %d ], accum_pulse_width = %d, accum_pulse = %d, tolerance_pulse = %d, same_polarity_1 = %d\n",
					__FUNCTION__, __LINE__,
					accum_pulse_width, accum_pulse, tolerance_pulse, same_polarity_1);

				IRC_DEBUG_PRINTF("[ %s %d ], accum_pulse_width = %d, accum_pulse_1 = %d, tolerance_pulse_1 = %d\n",
					__FUNCTION__, __LINE__,
					accum_pulse_width, accum_pulse_1, tolerance_pulse_1);
					...
					INRANGE(accum_pulse_width, accum_pulse_1, tolerance_pulse_1)) && (same_polarity_1 ? 1 : 0))
				{
					...
					if (ird->attr->type == IRC_TRACE_TYPE)
					#endif
					IRC_DEBUG_PRINTF("[ %s %d ], Got [1]!\n", __FUNCTION__, __LINE__);
				}
				else
				{
					result = IRC_DECODE_FAIL;
					IRC_DEBUG_PRINTF("[ %s %d ], fail!\n", __FUNCTION__, __LINE__);
				}
			}
			else
			{
				IRC_DEBUG_PRINTF("[ %s %d ], protocol = %d, step = %d, accum_pulse_width = %d, accum_pulse = %d, tolerance_pulse = %d\n",
					__FUNCTION__, __LINE__,
					ird->attr->type, step,
					accum_pulse_width, accum_pulse, tolerance_pulse);

				if ((ird->attr->type == IR_TYPE_LAB) && (step == IR_SPECIAL))
				{
				...
				result == IRC_DECODE_DATA1)
			{
				...
				{					
					...
					IRC_DEBUG_PRINTF("[ %s %d ], last_key = 0x%x, current key = 0x%x\n", __func__, __LINE__, 
						(unsigned int)last_key, (unsigned int)ird->last_key_code[0]);					
					...
					{
						ird->last_key_code[0] &=(0x1 <<15) ;  // RC6, bit15 is toggle bit
					}					
					
					IRC_STD_PRINTK("[ %s %d ], press %08x, %ld\n", __func__, __LINE__, 
						(unsigned int)ird->last_key_code[0], jiffies*1000/HZ);
					
					if (ird->attr->type != IR_TYPE_LAB)
					{
						return ird->last_key_code[0];
					}
					else
					{
						return ird->last_key_code[0] & 0x000001ff;	// bit9 is a special bit of LAB.
					}
				}

				IRC_DEBUG_PRINTF("[ %s %d ], step = %d, return\n", __FUNCTION__, __LINE__, ird->decode_step);				
			...
			else if (result == IRC_DECODE_FAIL)
			{
				...
				if (ird->attr->type == IRC_TRACE_TYPE)
				#endif
				IRC_DEBUG_PRINTF("[ %s %d ], decode failed @ step %d!\n", __FUNCTION__, __LINE__, ird->decode_step);
			...
			else	/* IRC_DECODE_STOP */
			{
				...
				ird->key_bit_cnt = 0;
				CLEAR_CODE(ird->key_code);
				IRC_DEBUG_PRINTF("[ %s %d ], stop, return\n", __FUNCTION__, __LINE__);				
		...
		{			
			...
			if (result == IRC_DECODE_SUCCEED)
			{
				...
				if (ird->attr->repeat[ird->decode_step] == IR_END)
				{
					...
					IRC_STD_PRINTK("[ %s %d ], repeat %08x, %ld\n", __func__, __LINE__, 
		...
		else
		{
			...
			IRC_DEBUG_PRINTF("[ %s %d ], reset, return\n", __FUNCTION__, __LINE__);			
...
void irc_decode_status_clear(void)
{
	unsigned long i = 0;

	for (i = 0; i < IRP_CNT; i ++) {
		irc_decoders[i].decode_step = 0;
		irc_decoders[i].first_half_got = 0;
		irc_decoders[i].key_bit_cnt = 0;
		irc_decoders[i].last_pulse_width = 0;
	}
}
...
EXPORT_SYMBOL(irc_decode_status_clear);
MODULE_LICENSE("GPL");
