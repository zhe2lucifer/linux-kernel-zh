...
#define ALI_NAND_DRIVER_VERSION "ali_nand_ver_2015_1118"
...
struct boot_area_nand_parameter	
{
	...
	u32 wReadClock;		//Read clock settings 
	u32 wWriteClock;	//Write clock settings
...
struct ali_nand_host {
	...
	struct mtd_partition	*parts;	//!< mtd partition information.
	struct device		*dev;		//!< ALi nand device.
...
/*chip id*/
#define C3505			0x3505		/*!< Chip ID: 3505*/
...
enum E_NF_Control
{
	NF_CEJ	= (1<<7),				//!< Chip enabled to PAD
	NF_ALE	= (1<<5),				//!< Address latch enabled to PAD
	NF_CLE	= (1<<4),				//!< Command latch enabled to PAD
};
...
/*! @enum E_NF_CRYPTO
@brief NAND Flash Crypro Engine settings.
*/
enum E_NF_CRYPTO
{
	NF_CRYPTO_EN 	= (1<<7),		//!< NF Crypto Enable.
	NF_TRB 			= (0x08),			/*Delay to start ecc engine decode.*/
};
...
enum E_NF_INTFLAG
{
	NF_DMA_FLAG = (1<<7),			//!< NF finishes reading or writing data in DMA mode.
	NF_ECC_FLAG = (1<<5),			//!< NF ECC finishes during reading data from NF.
...
#define C3921_NF_PIN_SEL		(1<<3)	//!< NF Pinmux Select.
#define C3921_SF_PIN_SEL		(1<<19)	//!< SF Pinmux Select.
...
#define C3503_SOC_RESET_REG1 	0x84
#define C3503_NF_RESET	(1<<20)
...
#define C3505_SOC_LOCAL_CLK_CTRL	0x60	
#define C3505_NF_CLK_GATING		(1 << 2)	
#define C3505_SOC_SYSTEM_CLOCK	0x78
#define C3505_NF_CLK_SEL			(1 << 30)
#define C3505_NF_CLK_118M			(~(1 << 30))
#define C3505_NF_CLK_198M			(1 << 30)
#define C3505_SOC_PINMUX_REG3		0x490
#define C3505_PINMUX_CTRL_BY_REG	(1 << 10)
#define C3505_XNF_SEL				(1 << 9)
#define C3505_XNF_WPJ_SEL		(1 << 4)
#define C3505_XSF_SEL				(1<<8 | 1<<3 | 1<<2 | 1<<1 | 1<<0)
