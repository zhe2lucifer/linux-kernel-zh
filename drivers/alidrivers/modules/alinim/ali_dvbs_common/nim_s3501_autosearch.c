...
INT32   delta_num_last = 0;
INT32   residue_point=0 ;
...
static INT32 get_threshold_value(NIM_S3501_FIND_CHANNEL_PARAMS *params,int i_input)
{
	UINT32 signal_counter = 0;
    UINT32 index = 0;
    INT32  signal_value = 0;
...
//for Ali  C3031B Tuner API
INT32 nim_s3501_fft_wideband_scan_m3031(struct nim_device *dev, UINT32 tuner_if, UINT32 adc_sample_freq)
{
    ...
    UINT8 Overflow=0;
    UINT32 update_num=0,delta_num=0,update_start_addr=0,nouse_num=0;
    UINT8 sum1_overflow=0;
    UINT32 sum1_f=40;

    WBSCAN_PRINTF("tuner_if = %d, last_tuner_if = %d \n",tuner_if,last_tuner_if);

    // remove value<=0 point
    for(i=0;i<1024;i++)/// 25bit
    {
    ...
    {
        //calc freq_step
        delta_freq=tuner_if-last_tuner_if;
        if(delta_freq>(adc_sample_freq/2))
        {
            WBSCAN_PRINTF("freq_step too large ~ \n");
            return SUCCESS;
        }
        if(delta_freq<=0)
        {
            WBSCAN_PRINTF("freq_step too small ~ \n");
            return SUCCESS;
        }
        else
        {
            //calc parameter
            delta_num = (((delta_freq*1024) + residue_point + adc_sample_freq)/(adc_sample_freq*2)); //1/2*delta_freq
						if(delta_num_last != delta_num)//Make compensation
							residue_point = (delta_freq*1024)/2 - (delta_num*adc_sample_freq);
						else
							residue_point += (delta_freq*1024)/2 - (delta_num*adc_sample_freq);
						delta_num_last = delta_num ;
						
            update_num = 512-delta_num; // need to update number
            update_start_addr = 512-delta_num; // the first update addr
            nouse_num = 512-delta_num*2; // no-use points
            WBSCAN_PRINTF("delta_freq=%d, update_num=%d, update_start_addr=%d, nouse_num=%d residue_point = %d delta_num_last = %d\n",
                          delta_freq,update_num,update_start_addr,nouse_num,residue_point,delta_num_last);

            sum1=0;
            sum2=1;
            factor=0;
            for(i=update_start_addr-20;i<update_start_addr+20;i++)
            {
                tmp_addr=chlspec_num-512+(i-nouse_num);
                sum1 +=channel_spectrum[tmp_addr];
                sum2 +=fft_energy_1024[i];
				if(sum1> max_int32)
                {
                	sum1_overflow = 1 ;	
                	WBSCAN_PRINTF("sum1 overflow error with 3031 Tuner\n");
                	break ;
                }	               
            }
            ...
            {	
            	...
            	{
                	tmp_addr=chlspec_num-512+(i-nouse_num);
                	sum1 +=channel_spectrum[tmp_addr]/sum1_f;
                	sum2 +=fft_energy_1024[i]/sum1_f;
            	}            	
            }
            ...
            for(i=(chlspec_num-update_num);i<chlspec_num;i++)
            {
                tmp_addr = update_start_addr+(i-(chlspec_num-update_num));
                Overflow=0;
            ...
            tmp_addr = update_start_addr+i-(chlspec_num-update_num);
            WBSCAN_PRINTF("add_start_addr = %d \n",tmp_addr);
