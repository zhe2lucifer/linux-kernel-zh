...
DWORD nim_c3501h_multu64div(UINT32 v1, UINT32 v2, UINT32 v3)
{
    ...
    *tmp64 = lo;
	...
	//ret = tmp/v3;//this must be replaced by the below lines, otherwise, it will have link error,"undefined reference to `__udivdi3'"
	do_div(tmp, v3);
    ret = tmp;
    return ret;
}
...
INT32 nim_c3501h_reg_read(struct nim_device *dev, UINT16 bMemAdr, UINT8 *pData, UINT8 bLen)
{
    ...
    if (err)
    {
        ...
        NIM_PRINTF("c3501h i2c read error = %d,chip_adr=0x%x,origin_addr=0x%x,bMemAdr=0x%x,I2C_FOR_C3501H = %d\n", -err,chip_adr, pData[0],i2c_type_id);
    }
    else
    {
...
INT32 nim_c3501h_reg_write(struct nim_device *dev, UINT16 bMemAdr, UINT8 *pData, UINT8 bLen)
{
    ...
    if (err != 0)
    {
        ...
        nim_c3501h_set_err(dev);
        NIM_PRINTF("c3501h i2c write error = %d,chip_adr=0x%x,origin_addr=0x%x, bMemAdr=0x%x,I2C_FOR_C3501H = %d\n", -err,chip_adr, pData[0],i2c_type_id);
    }
    else
    {
        if (priv->t_param.t_i2c_err_flag)
...
INT32 nim_c3501h_i2c_through_open(struct nim_device *dev)
{
    ...
    {
		mutex_lock(&priv->tuner_open_mutex);
        ...
        {
        	mutex_unlock(&priv->tuner_open_mutex);
...
#else
INT32 nim_c3501h_set_ext_lnb(struct nim_device *dev)
{
		struct nim_c3501h_private *priv = NULL;
		struct LNB_IO_FUN         *lnb_io_fun = NULL;
		...
		lnb_io_fun = lnb_setup(priv->ext_lnb_type);
		if(NULL == lnb_io_fun)
		{
			ERR_PRINTF("Lnb_setup is Error in %s %d!\n",__FUNCTION__,__LINE__);
			return ERR_NO_DEV;
		}

		priv->ext_lnb_command = lnb_io_fun->pf_command;
		if(NULL == priv->ext_lnb_command)
		{
			ERR_PRINTF("Ext_lnb_command Pointer check Error in %s %d \n",__FUNCTION__,__LINE__);
			return ERR_NO_DEV;
		}

		priv->ext_lnb_command(&priv->ext_lnb_id, LNB_CMD_INIT_CHIP, (UINT32) (&priv->ext_lnb_config));
...
INT32 nim_c3501h_set_agc1(struct nim_device *dev, UINT8 low_sym, UINT8 s_case, UINT8 frz_agc1)
{
    ...
    if ((priv->tuner_config_data.qpsk_config & C3501H_AGC_INVERT) == 0x0)
    {
        data = data ^ 0x80;
    }
...
INT32 nim_c3501h_set_polar(struct nim_device *dev, UINT8 polar)     
{
	...
	if (priv->ext_lnb_command)
	{
		...
		NIM_MUTEX_ENTER(priv);
		result = priv->ext_lnb_command(&priv->ext_lnb_id, LNB_CMD_SET_POLAR, (UINT32) polar);
        NIM_MUTEX_LEAVE(priv);
		return result;
	}
	...
	if(priv->tuner_config_data.disqec_polar_position != DISEQC_POLAR_SET_NO_USE_GPIO)
	{
		if(data & 0x40)
...
INT32 nim_c3501h_get_lock_check_limited(struct nim_device *dev, UINT8 *lock_state)
{
    ...
    priv->ul_status.check_lock_time = jiffies;
...
INT32 nim_c3501h_channel_change(struct nim_device *dev, struct NIM_CHANNEL_CHANGE * pst_channel_change)
{
...
#ifdef TSN_SUPPORT
	if (NULL == pst_channel_change->tsn)
	{
		NIM_PRINTF("error, NULL == pst_channel_change->tsn\n", __FUNCTION__, __LINE__);
		return ERR_FAILED;
	}
  ...
  // initial param
	...
	#endif
	{
		...
		comm_sleep(10); //sleep 2ms                                                         // report the information, it is wield.20150819 
	}
    ...
    if (0 == sym%2)
    {   
        pr_info("Set c3501h fc/rs search range as channel change\n");
    ...
    {
        pr_info("Set c3501h fc/rs search range as soft search\n");
	...
	queue_delayed_work(dev->channel_change_workqueue, &dev->delay_channel_change_work, 0);
#endif
...
INT32 nim_c3501h_waiting_channel_lock(struct nim_device *dev, UINT32 freq, UINT32 sym, UINT8 change_type, struct nim_dvbs_isid *p_isid)
{
	...
	NIM_PRINTF("[%s %d]pl_find_thr = %d\n", __FUNCTION__, __LINE__, pl_find_thr);
#endif

	search_start_single = jiffies;
	search_start_total = jiffies;
	lock_time_s = jiffies;
	PLSN_PRINTF("[%s %d]: lock_time_s = %d \r\n", __FUNCTION__, __LINE__, lock_time_s);

	while (priv->ul_status.c3501h_chanscan_stop_flag == 0)
	{
	    timeout = jiffies - lock_time_s;
		...
		if (use_default_flag)
		{
			...
			if (PLSN_FIND == state)
			{
				...
				use_default_flag = 0;
				lock_time_s = jiffies;//reinit the start time
        ...
        {
        	NIM_PRINTF("[%s %d]intdata = 0x%x\n", __FUNCTION__, __LINE__, intdata);
            tr_lock_flag = 1;
            time_tr_lock = jiffies;
        }
        ...
        {      
            if((0x80 == (intdata&0x80)) && (pl_find_flag == 0) && (tr_lock_flag))     
            {
                ...
                pr_info("*********C3501H PL Find, timeout %d, intdata = 0x%x ************\n", timeout, intdata);
#endif
               ...
               if(pl_find_reset_fsm == 1)
				{
					...
					if (priv->plsn.super_scan)
					{
						...
						{			
							comm_sleep(2);
							if (jiffies - stop_start > 6000)
							{
								PLSN_PRINTF("[%s %d]force stop timeout = %d ms! \n", __FUNCTION__, __LINE__, jiffies - stop_start);
								break;
							}
						}
						...
						queue_delayed_work(dev->plsn_search_workqueue, &dev->delay_plsn_search_work,0);	// Start PLSN search workqueue					
					}
					search_start_single = jiffies;
					search_start_total = jiffies;
               	}
            ...
            // Can't find pl
            if ((0 == pl_find_flag) && ((jiffies - time_tr_lock) > pl_find_thr) && tr_lock_flag)
            {         
...
#if (defined(ES_TEST) && !defined(C3501H_DEBUG_FLAG))	 
			pr_info("*********Lock tp cost %d ms time*********\r\n", jiffies - c3501h_tp_lock_time);
#endif
...
ERR_HANDLE:
		if (err_flag)
		{
			...
			else if ((PLSN_OVER == state) || (PLSN_RUNNING == state))
			{
				...
				if ((PLSN_OVER == state) || (jiffies - search_start_single > search_single_timeout))
				{
					if(priv->plsn.plsn_same_cnt >= 2)
					{
						...
						PLSN_PRINTF("[%s %d]: Lock tp fail cost %d ms time\n", __FUNCTION__, __LINE__, jiffies - c3501h_tp_lock_time);
						PLSN_PRINTF("[%s %d]: lock fail, search cost %d ms time\n", __FUNCTION__, __LINE__, jiffies - search_start_total);
						priv->ul_status.c3501h_chanscan_stop_flag = 0;
						priv->tsk_status.m_lock_flag = NIM_LOCK_STUS_SETTING;
						p_isid->get_finish_flag = 1;
						//NIM_PRINTF("[%s %d]p_isid->get_finish_flag=%d\n", __FUNCTION__, __LINE__, p_isid->get_finish_flag);
					...
					else if (jiffies - search_start_total < search_total_timeout)
					{
						PLSN_PRINTF("[%s %d]: start next search plsn, state=%d!, search_single_timeout=%d ms, search_total_timeout=%d ms, timeout=%d ms\n", 
								__FUNCTION__, __LINE__, state, search_single_timeout, search_total_timeout, jiffies - search_start_single); 			
...
{                                                                                                                                       
    ...
    {        
		...
		{                                                                                                                        
			...
			if (priv->nim_tuner_command != NULL)
			{
				priv->nim_tuner_command(priv->tuner_index, NIM_TUNER_GET_C3031B_FREQ_ERR, &channel_freq_err);                                                                                                                                                                          
			}
			...
			#ifdef __C3501H_LINUX__
				nim_send_as_msg(priv->nim_kumsgq , 0, 0, 0, 0, 0, 1);
		        priv->ul_status.c3501h_autoscan_stop_flag = 0;
			#endif
				//comm_free(channel_spectrum);                                                                                            
				//comm_free(channel_spectrum_tmp);                                                                                        
	...
	if (NIM_SUCCESS == params->success)
	{
		...
		for (i = 0; i < tp_number; i++)
		{
			...
			if (params->priv->ul_status.c3501h_autoscan_stop_flag)
			{
                ...
                if (symbol_rate_est[i]/2 > (UINT32)temp_freq)
                {
...
nim_as_break:
	#ifdef __C3501H_TDS__
		params->pst_auto_scan->callback(2, 0, 0, 0, 1);  /* Tell callback search finished */
	#endif

	#ifdef __C3501H_LINUX__
		nim_send_as_msg(priv->nim_kumsgq , 0, 0, 0, 0, 0, 1);  /* Tell callback search finished */
	#endif
...
#if (defined(ES_TEST) && !defined(C3501H_DEBUG_FLAG))   
                pr_info("*********C3501H PL Find, timeout %d , pl_find_indicator = 0x%x ************\n", timeout, (data&0x0c));
#endif
		...
		if(tp_scan_param->tr_lock_thr == timeout)
		{
			...
			if((lock_monitor&0x40) == 0x00) // TR never lock
			{
				...
				{			
					...
					if (jiffies - stop_start > 6000)
					{
						pr_info("[%s %d]force stop timeout = %d ms! \n", __FUNCTION__, __LINE__, (int)(jiffies - stop_start));
						break;
					}
				}
				PLSN_PRINTF("[%s %d]stop cost = %d ms! \n", __FUNCTION__, __LINE__, (int)(jiffies - stop_start));
			...
			if (pl_find_flag)
			{
				...
				else if ((PLSN_OVER == state) || (PLSN_RUNNING == state))
				{
					...
					if ((PLSN_OVER == state) || (jiffies - search_start_single > search_single_timeout))
					{
						...
						{
							PLSN_PRINTF("[%s %d]plsn_same_cnt = %d \n", __FUNCTION__, __LINE__, priv->plsn.plsn_same_cnt);
							PLSN_PRINTF("[%s %d]over,state=%d!, timeout=%d ms\n", __FUNCTION__, __LINE__, state, jiffies - search_start_single);
							PLSN_PRINTF("[%s %d]lock fail, cost %d ms time\n", __FUNCTION__, __LINE__, jiffies - search_start_total);
							nim_c3501h_clear_interrupt(dev);
 ...
 {     
	...
	switch (s_case)
	{
		case NIM_SIGNAL_INPUT_OPEN:
			osal_cache_invalidate((UINT32)priv->ul_status.adc_data, 0x20000 * 2);
		...
		case NIM_SIGNAL_INPUT_CLOSE:
			osal_cache_flush((UINT32)priv->ul_status.adc_data, 0x20000 * 2);
	...
	{
		case NIM_SIGNAL_INPUT_OPEN:
			sys_ioctl(fd_dmx, ALI_DMX_IO_SET_BYPASS_MODE, (UINT32) (priv->ul_status.adc_data));//Set bypass mode
			break;
	...
	if (sys_close(fd_dmx))
	{
		AUTOSCAN_PRINTF("%s in:Error closing the dmx_hdl.\n", __FUNCTION__);
	}
#endif
...
INT32 nim_c3501h_get_plsn_search_ret(struct nim_device *dev)
{
	...
	mutex_lock(&priv->plsn_mutex);//robin.gan add
...
INT32 nim_c3501h_search_plsn_hw(struct nim_device *dev)
{
    ...
    #ifdef PLSN_DEBUG
	    start_t = jiffies; //record the start time
    #endif
...
void nim_c3501h_gpio_output(struct nim_device *dev, UINT8 pin_num, UINT8 value)
{
	...
	data = data | (1 << pin_num);
	pr_info("[%s %d]enable gpio pinmux, data=0x%02x\n", __FUNCTION__, __LINE__, (int)data);
	...
	pr_info("[%s %d]output, data=0x%02x\n", __FUNCTION__, __LINE__, (int)data);
