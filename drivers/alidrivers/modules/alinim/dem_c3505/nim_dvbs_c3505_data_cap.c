...
{	
	...
	dram_base = virt_to_phys(cap_buffer);
	//Reset ADCDMA.	  
   	nim_reg_read(dev,0x16c, &data, 1);
	data |= 0x80;	
	nim_reg_write(dev,0x16c, &data, 1);
...
INT32 nim_c3505_adc2mem_calculate_energy(struct nim_device *dev,UINT8 *cap_buffer,UINT32 dram_len)
{
	...
	__CACHE_FLUSH_ALI(cap_buffer,dram_len);
	#endif
	...
	#if ((defined(__C3505_LINUX__)) && (defined(CONFIG_MIPS)))
	__CACHE_INV_ALI(cap_buffer,dram_len);
	#endif
