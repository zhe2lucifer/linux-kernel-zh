 ...
 * Return Value: INT32
*****************************************************************************/
INT32 nim_c3505_get_agc_dbm(struct nim_device *dev, UINT8 *agc_dbm)
{
	UINT8 agc_demod = 0 ;
	INT32 agc_tuner = 0 ;
	INT8 level_db_tmp = 0 ;	
...
#define IMPROVE_CNR_PATCH

#ifdef IMPROVE_CNR_PATCH
//{-0.13357, -0.00037138, -0.0000014325, -0.0000000092603, -0.00000000081069};
static long long index_a[5] = {13357, 37138, 14325, 92603, 81069};
//{22.392, 0.18019, 0.0034689, 0.00010671, 0.000012927};
static long long index_b[5] = {22392, 18019, 34689, 10671, 12927};
//{-1273.8, -32.364, -3.3704, -0.57858, -0.243};
static long long index_c[5] = {12738, 32364, 33704, 57858, 243};
//{27747, 4438.4, 2754.8, 1905.8, 1526.9};
static long long index_d[5] = {277470, 44384, 27548, 19058, 15269};


unsigned int CNR_multu64div(unsigned long long v1, unsigned int v2)
{
    unsigned int v = 0;
    while(v1 >= v2)
    {
...
//{-0.13357, -0.00037138, -0.0000014325, -0.0000000092603, -0.00000000081069};
long long C_Noise_a(long long cr_noise, UINT8 index)
{
	long long ret = 0;
	if(index == 0)
	{
		ret = (cr_noise*cr_noise*cr_noise*index_a[index]);// 10000;
		ret = CNR_multu64div(ret, 10000);
	}
	else if(index == 1)
	{
		ret = (cr_noise*cr_noise*cr_noise*index_a[index]);// 10000000;
		ret =  CNR_multu64div(ret, 10000000);
	}
	else if(index == 2)
	{
		ret = (cr_noise*cr_noise*cr_noise*index_a[index]); // 1000000000;
		ret = CNR_multu64div(ret, 1000000000);
	}
	else if(index == 3)
	{
		ret = (cr_noise*cr_noise*cr_noise*index_a[index]); // 1000000000000;
		ret = CNR_multu64div(ret, 1000000000);
		ret = CNR_multu64div(ret, 1000);
	}
	else if(index == 4)
	{
		ret = (cr_noise*cr_noise*cr_noise*index_a[index]); //10000000000000;
		ret = CNR_multu64div(ret, 1000000000);
		ret = CNR_multu64div(ret, 10000);
	}
	return ret;
...
//{22.392, 0.18019, 0.0034689, 0.00010671, 0.000012927};
long long C_Noise_b(long long cr_noise, UINT8 index)
{
	long long ret = 0;
	if(index == 0)
	{
		ret = (cr_noise*cr_noise*index_b[index]); //100;
		ret = CNR_multu64div(ret, 100);
	}
	else if(index == 1)
	{
		ret = (cr_noise*cr_noise*index_b[index]); //10000;
		ret = CNR_multu64div(ret, 10000);
	}
	else if(index == 2)
	{
		ret = (cr_noise*cr_noise*index_b[index]); // 1000000;
		ret = CNR_multu64div(ret, 1000000);
	}
	else if(index == 3)
	{
		ret = (cr_noise*cr_noise*index_b[index]); // 10000000;
		ret = CNR_multu64div(ret, 10000000);
	}
	else if(index == 4)
	{
		ret = (cr_noise*cr_noise*index_b[index]); // 100000000;
		ret = CNR_multu64div(ret, 100000000);
	}
	return ret;
}
//{-1273.8, -32.364, -3.3704, -0.57858, -0.243};
long long C_Noise_c(long long cr_noise, UINT8 index)
{
	long long ret = 0;
	if(index == 0)
	{
		ret = (cr_noise*index_c[index]);
	}
	else if(index == 1)
	{
		ret = (cr_noise*index_c[index]); // 100;
		ret = CNR_multu64div(ret, 100);
	}
	else if(index == 2)
	{
		ret = (cr_noise*index_c[index]); // 1000
		ret = CNR_multu64div(ret, 1000);
	}
	else if(index == 3)
	{
		ret = (cr_noise*index_c[index]); // 10000
		ret = CNR_multu64div(ret, 10000);
	}
	else if(index == 4)
	{
...
//{27747, 4438.4, 2754.8, 1905.8, 1526.9};
long long C_Noise_d(UINT8 index)
{
	long long ret = 0;
	ret = index_d[index];
	return ret;
	...
	*/
...
#ifdef IMPROVE_CNR_PATCH
    long long cr_noise = 0;
    UINT32 curve_1 = 60;
    UINT32 curve_2 = 200;
    UINT32 curve_3 = 1000;
    UINT32 curve_4 = 5000;
    UINT8 index = 0;
    long long noise_float = 0;
#endif
	...
	switch(modu)
	{
		...
		case 11: // QPSK
...
#ifdef IMPROVE_CNR_PATCH
            if (cr_noise <= curve_1)    // 0~60
            {
                index = 0;
            }
            else if (cr_noise <= curve_2)   // 61~200
            {
                index = 1;
            }
            else if (cr_noise <= curve_3)   // 201~1000
            {
                index = 2;
            }                
            else if (cr_noise <= curve_4)   // 1001~5000
            {
                index = 3;
            }
            else    // > 5000
			...
			/*
			noise_float = cr_noise*cr_noise*cr_noise*index_a[index] + cr_noise*cr_noise*index_b[index] \
                + cr_noise*index_c[index] + index_d[index];
			*/
			noise_float = (C_Noise_b(cr_noise, index)+C_Noise_d(index)-C_Noise_a(cr_noise, index)-C_Noise_c(cr_noise, index));
			noise_float = ((UINT16)noise_float)/10;
			snr_cn = (INT16)noise_float;
            if (snr_cn > 6000)
                snr_cn = 6000;
            ...
            *snr_db = snr_cn;     
            ADPT_NEW_CR_PRINTF("snr_float = %lld, snr_rpt = %d\n", noise_float, *snr_db);
#else
...
UINT8 nim_c3505_get_snr_index(struct nim_device *dev)
{
 ...
 /*****************************************************************************
 * UINT32 nim_C3505_get_CURFreq(struct nim_device *dev)
 * Description: 
 *
 * Arguments:
 *	Parameter1: struct nim_device *dev
 ...
 * Return Value: void
 *****************************************************************************/
//UINT32 nim_c3505_get_cur_freq(struct nim_device *dev, UINT32 *freq)
//clean warning
...
INT32 nim_c3505_get_bb_header_info(struct nim_device *dev)
{
	...
	UINT8 work_mode = 0;
	UINT8 time_out = 0;
	//UINT8 reg106_bak = 0x27; //clean warning unused variable
	UINT32 i = 0;
