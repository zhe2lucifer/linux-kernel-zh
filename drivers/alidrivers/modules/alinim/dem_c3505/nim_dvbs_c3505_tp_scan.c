...
INT32 nim_c3505_channel_change(struct nim_device *dev, struct NIM_CHANNEL_CHANGE *pst_channel_change)
{
    ...
    struct nim_c3505_tp_scan_para *tp_scan_param = NULL;
	//plsn_state state = PLSN_UNSUPPORT; //clean warning unused variable
	//UINT8 temp_index = 0; //clean warning unused variable
	...
	if (priv->plsn.super_scan)
	{
		...
		//cancel last time delay_plsn_search_work
...
INT32 nim_c3505_waiting_channel_lock(struct nim_device *dev, UINT32 freq, UINT32 sym, UINT8 change_type, struct nim_dvbs_isid *p_isid)
 {
	...
	UINT8 intdata = 0;
	UINT8 code_rate = 0;
	//UINT8 data = 0; //clean warning unused variable
    UINT8 lock = 0;
    int lock_cnt = 0;
	struct nim_c3505_private *priv = NULL;
	INT32 result;
	...
	//UINT8 count_run = 0; //clean warning unused variable
	...
	// Start PLSN search workqueue
	if(1 == priv->plsn.super_scan)
		queue_delayed_work(dev->plsn_search_workqueue, &dev->delay_plsn_search_work,0);
    // Set timeout
	if (sym > 40000)
		locktimes = 204;
	else if (sym < 2000)
		...
		{			
            ...
            {   
				...
				if ((result != SUCCESS) && (priv->ul_status.c3505_chanscan_stop_flag || priv->ul_status.c3505_autoscan_stop_flag))
				{
					...
					__FUNCTION__, __LINE__, (int)result, (int)p_isid->get_finish_flag);
...
ERR_HANDLE:
		if (err_flag)
		{
			...
			else if ((PLSN_OVER == state) || (PLSN_RUNNING == state))
			{
				...
				{
					if (jiffies - search_start_total < search_total_timeout)
					{
					...
					else
					{
						...
						priv->t_param.phase_noise_detect_finish = 1;
						nim_c3505_interrupt_clear(dev);
						NIM_PRINTF("[%s %d]timeout= %d locktimes =%d\n", __FUNCTION__, __LINE__, (int)timeout, (int)locktimes);
						NIM_PRINTF("[%s %d]Lock tp fail cost %d ms time\n", __FUNCTION__, __LINE__, (int)(jiffies - tp_lock_time));
						PLSN_PRINTF("[%s %d]lock fail, search cost %d ms time\n", __FUNCTION__, __LINE__, (long unsigned int)jiffies - search_start_total);
						priv->ul_status.c3505_chanscan_stop_flag = 0;
						priv->tsk_status.m_lock_flag = NIM_LOCK_STUS_SETTING;
						p_isid->get_finish_flag = 1;
						NIM_PRINTF("[%s %d]p_isid->get_finish_flag=%d\n", __FUNCTION__, __LINE__, p_isid->get_finish_flag);
           ...
           params->fft_freq += step_freq)
    {
		...
		{				
			...
			if(params->priv->ul_status.c3505_autoscan_stop_flag)
			{
				...
				//comm_free((int *)priv->ul_status.adc_data_malloc_addr);
...
INT32 nim_c3505_autoscan_try_tp(struct nim_device *dev, struct nim_c3505_autoscan_params *params)
{
	...
	if (NIM_SUCCESS == params->success)
	{
		...
		for (i=0;i<tp_number;i++)
		{
			...
			if (SUCCESS == nim_c3505_channel_search(dev, 0))
			{
            ...
            #ifdef AUTOSCAN_DEBUG
                params->end_t = jiffies;
			    AUTOSCAN_PRINTF("Lock SUCCESS %d-th TP[freq %d, sym %d] ,Cost time     %dms\n", \
				...
				if ((cur_freq >= params->pst_auto_scan->sfreq) && (cur_freq <= params->pst_auto_scan->efreq))
				{
					...
					else
					{
						...
						{	
                            ...
                            AUTOSCAN_PRINTF("Find Program %d-th TP[freq%d, sym %d] ---> SUCCESS! ,Cost time        %dms\n", i, final_est_freq, final_est_sym_rate, jiffies - params->end_t);
				...
				{					
					AUTOSCAN_PRINTF("tleave out of the frequence range %d-th TP[freq%d, sym %d] ,Cost time     %dms\n",i,final_est_freq,final_est_sym_rate,jiffies - params->end_t);
					if (params->priv->ul_status.c3505_autoscan_stop_flag)
					{
				#ifdef __C3505_LINUX__
						nim_send_as_msg(priv->nim_kumsgq , 0, 0, 0, 0, 0, 1);
...
{                                                                                                                 
	...
	//data = 0x1ff;
	time_thr = 0x1ff;
	nim_c3505_set_hw_timeout(dev, time_thr); 
			...
			if (pl_find_flag)
			{
				...
				else if ((PLSN_OVER == state) || (PLSN_RUNNING == state))
				{
					...
					if ((PLSN_OVER == state) || (jiffies - search_start_single > search_single_timeout))
					{
						...
						{
							//PLSN_PRINTF("[%s %d]over,state=%d!, search_timeout=%d ms, timeout=%d ms\n", __FUNCTION__, __LINE__, state, search_timeout, jiffies - search_start_single);
...
INT32 nim_c3505_cap_fft_find_channel(struct nim_device *dev, UINT32 *tune_freq)
{
	...
	if (1 == success)
	{
		for (i = 0; i < ch_number; i++)
		{
			...
			NIM_PRINTF("    m_Freq[%d] is %d\n", (int)priv->ul_status.m_cr_num, (int)priv->ul_status.m_freq[priv->ul_status.m_cr_num]);
