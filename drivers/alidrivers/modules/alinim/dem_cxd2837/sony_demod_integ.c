...
//#define DVBT_T2_MONITOR_RF_LEVEL
//#define DVBC_MONITOR_RF_LEVEL
...
sony_result_t sony_integ_CalcRF_LEVEL(sony_demod_t * pDemod,int32_t *rf_level)
{
    ...
    else	
    {
        ...
        switch(pDemod->system)
        {
			...
			case SONY_DTV_SYSTEM_DVBT:
            {
                result = sony_integ_dvbt_t2_monitor_RFLevel (pDemod, rf_level);
            }	
            break;
          
	        case SONY_DTV_SYSTEM_DVBC:
	        {
	             result = sony_integ_dvbc_monitor_RFLevel (pDemod, rf_level);
	        }	
	        break;
            default:
    ...
    {
    //SONY_SLEEP(10);
    CXD2837_LOG(pDemod, "[sony_integ_CalcRFL_EVEL]:failed,result=%d\n",result);
    }
...
sony_result_t sony_integ_CalcSNR(sony_demod_t * pDemod,uint32_t *pSNR)
{
    ...
    else	
    {
        ...
        switch(pDemod->system)
        {
            case SONY_DTV_SYSTEM_DVBT2:
            {
                result = sony_demod_dvbt2_monitor_SNR (pDemod, pSNR);
            }	
            break;
            case SONY_DTV_SYSTEM_DVBT:
            {
                result = sony_demod_dvbt_monitor_SNR (pDemod, pSNR);
            }	
            break;
            case SONY_DTV_SYSTEM_DVBC:
            {
                result = sony_demod_dvbc_monitor_SNR (pDemod, pSNR);
            }	
            break;
            default:
                return SONY_RESULT_ERROR_ARG;
