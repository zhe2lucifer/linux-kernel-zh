...
void cxd2838_log_i2c(sony_demod_t* param, UINT8 err, UINT8 write, UINT8 slv_addr, UINT8 *data, int len)
{
   ...
   //     return;
    ...
    if (write)
        CXD2838_PRINTF(NIM_LOG_DBG,"I2C_Write,0x%02X", (slv_addr&0xFE));
    else
        CXD2838_PRINTF(NIM_LOG_DBG,"I2C_Read,0x%02X", (slv_addr|1));

    for ( i=0; i<len; ++i )
    {
        CXD2838_PRINTF(NIM_LOG_DBG,",0x%02X", data[i]);
    }

    if (err)
        CXD2838_PRINTF(NIM_LOG_DBG,"\terror");
    CXD2838_PRINTF(NIM_LOG_DBG,"\r\n");
}
...
static INT32 config_tuner(struct nim_device *dev, UINT32 freq, UINT32 bandwidth, UINT8 *ptr_lock)
{
	...
	do
	{
		...
		{
			CXD2838_PRINTF(NIM_LOG_DBG,"Config tuner failed, I2c failed!\r\n");
		}
		...
		CXD2838_PRINTF(NIM_LOG_DBG,"Tuner Lock Times=0x%d,Status=0x%d\r\n",Tuner_Retry,lock);
	...
	if(Tuner_Retry > 5)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"ERROR! Tuner Lock Fail\r\n");
		return ERR_FAILUE;
	}
	return SUCCESS;
}
...
INT32 try_to_lock_ISDBT_signal(struct nim_device *dev, BOOL NeedToInitSystem,BOOL NeedToConfigTuner,NIM_CHANNEL_CHANGE_T *change_para)
{
	...
	switch(tuneResult){
	case SONY_RESULT_OK:
		 CXD2838_PRINTF(NIM_LOG_DBG,"[SONY]ISDBT TS Locked.\n");
		return SUCCESS;
	case SONY_RESULT_ERROR_UNLOCK:
		CXD2838_PRINTF(NIM_LOG_DBG,"[SONY]ISDBT TS Unlocked.\n");
		return ERR_FAILUE;
	case SONY_RESULT_ERROR_TIMEOUT:
		CXD2838_PRINTF(NIM_LOG_DBG,("[SONY]ISDBT Wait TS Lock but Timeout.\n"));
		return ERR_FAILUE;
	default:
		CXD2838_PRINTF(NIM_LOG_DBG,"[SONY] default: in sony_integ_isdbt_Tune,tuneResult = %d\n",tuneResult);
		return ERR_FAILUE;
	}
}
...
{	
	...
	CXD2838_PRINTF(NIM_LOG_DBG,"[%s]:line=%d,system:%d\r\n",__FUNCTION__,__LINE__,param->system);
	...
	{
	    CXD2838_PRINTF(NIM_LOG_DBG,"[%s]:line=%d error:bandwidth=%d \n", __FUNCTION__,__LINE__,change_para->bandwidth);
        mutex_unlock(&param->demodMode_mutex_id);
		return ERR_FAILUE;
	}
	
	CXD2838_PRINTF(NIM_LOG_DBG,"%s: usage_type %d, freq %d, bandwidth %d, priority %d, t2_signal %d, plp_index %d, plp_id  %d, profile %d\r\n",
	...
	do
	{
		...
		if ( need_to_config_tuner(dev, change_para->freq, change_para->bandwidth) )
		{
			...
			CXD2838_PRINTF(NIM_LOG_DBG,"[%s]:line=%d,need_to_config_tuner!\r\n",__FUNCTION__,__LINE__);
		...
		{
			CXD2838_PRINTF(NIM_LOG_DBG,"[%s]:line=%d,try_to_lock_ISDBT_signal play_program =%d \n",__FUNCTION__,__LINE__,play_program);
			...
			{
				CXD2838_PRINTF(NIM_LOG_DBG,"[%s]:line=%d,try_to_lock_DVBT_signal failed!\n",__FUNCTION__,__LINE__);
			}
...
INT32 nim_cxd2838_open(struct nim_device *dev)
{
    ...
    {
		CXD2838_PRINTF(NIM_LOG_DBG,"nim_device is null\n");
	...
	if (result != SONY_RESULT_OK)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error: Unable to configure DEMOD_CONFIG_PARALLEL_SEL. (status=%d, %s)\r\n", result, FormatResult(result));
		return result;
	}
	...
	if (result != SONY_RESULT_OK)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error: Unable to configure DEMOD_CONFIG_TSERR_ENABLE. (status=%d, %s)\r\n", result, FormatResult(result));
		return result;
	}
	...
	if (result != SONY_RESULT_OK)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error: Unable to configure IFAGCNEG. (status=%d, %s)\r\n", result, FormatResult(result));
		return result;
	}
#else
	...
	if (result != SONY_RESULT_OK)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error: Unable to configure SONY_DEMOD_CONFIG_IFAGC_ADC_FS. (status=%d, %s)\r\n", result, FormatResult(result));
		return result;
	}
	...
	if(result != SONY_RESULT_OK)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error in sony_demod_GPIOSetConfig for TS error.\n");
		return result;
	}
	...
	if (result != SONY_RESULT_OK)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error: Unable to configure SPECTRUM_INV. (status=%d, %s)\r\n", result, FormatResult(result));
		return result;
	}
#endif
    ...
    /* Default is disabled. 1: Enable, 0: Disable. */
        ...
        if (result == SONY_RESULT_OK) {
        	CXD2838_PRINTF(NIM_LOG_DBG,"Demodulator configured to enable RF level monitoring.\n");
        }
        else {
        	CXD2838_PRINTF(NIM_LOG_DBG,"Error: Unable to configure RFLVMON_ENABLE. (result = %d)\n", result);
            return -1;
        }
#endif
	...
	if (result != SONY_RESULT_OK)
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error: Unable to configure RFLVMON_ENABLE. (status=%d, %s)\r\n", result, FormatResult(result));
		return result;
	}
#endif
    return SUCCESS;
