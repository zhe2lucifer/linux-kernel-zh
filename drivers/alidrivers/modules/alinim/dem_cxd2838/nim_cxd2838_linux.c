...
static void nim_cxd2838_set_config(struct nim_device *dev, struct ali_nim_mn88436_cfg *nim_cfg)
{
    ...
    CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,flag=%d,i2c_type=%d,i2c_addr=0x%x\n", __FUNCTION__, __LINE__,
	...
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"[%s]line=%d,Tuner API is NULL!\n",__FUNCTION__,__LINE__);
	}
}
...
static INT32 ali_cxd2838_nim_hw_initialize(struct nim_device *dev, struct ali_nim_mn88436_cfg *nim_cfg)
{
	...
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Tuner Configuration API structure is NULL!/n");
		return ERR_NO_DEV;
	}
	CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,start:\n", __FUNCTION__, __LINE__);
    ...
    CXD2838_PRINTF(NIM_LOG_DBG,"ext_dm_config.i2c_base_addr = 0x%x\n",nim_cfg->ext_dm_config.i2c_base_addr);
	...
	{
	    CXD2838_PRINTF(NIM_LOG_DBG,"sony_demod_Create error!\n");
		return ERR_NO_DEV;
	}
	nim_cxd2838_set_config(dev,nim_cfg);
	CXD2838_PRINTF(NIM_LOG_DBG,"i2cAddressSLVX = 0x%x\n",priv->i2cAddressSLVX);
    ...
    if(priv->tuner_control.nim_tuner_init != NULL)
    {
        ...
        {
            CXD2838_PRINTF(NIM_LOG_DBG,"Error: Init Tuner Failure!\r\n");
            return ERR_NO_DEV;
        }
	...
	{
		CXD2838_PRINTF(NIM_LOG_DBG,"Error: Init Tuner is NULL!\r\n");
	}
    if (SONY_RESULT_OK != nim_cxd2838_open(&ali_cxd2838_nim_dev))
    {
        CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,nim_cxd2838_open failed!\n", __FUNCTION__, __LINE__);
		return ERR_FAILURE;
	}
    CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,end!\n", __FUNCTION__, __LINE__);
    return SUCCESS;
}
...
static INT32 ali_cxd2838_tuner_adaption(struct nim_device *dev, struct ali_nim_mn88436_cfg *nim_cfg)
{
    ...
    if (SUCCESS == nim_cxd2838_read(dev, 
		                        ...
		                        1))
    {	
		CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,adaption success!,i2c_type=%d,i2c_addr=0x%x\n", 
			      __FUNCTION__, __LINE__,
			      priv->tuner_control.tuner_config.i2c_type_id,
			      priv->tuner_control.tuner_config.c_tuner_base_addr);
...
static int ali_cxd2838_nim_ioctl(struct file *file, unsigned int cmd, unsigned long parg)
{
    ...
    switch(cmd)
	{
	    ...
	    case ALI_NIM_CHANNEL_CHANGE:
	    {
	        ...
	        CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,nim_param.fec=%d\n", __FUNCTION__, __LINE__, nim_param.fec);
			...
			{
				CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,nim_cxd2838_channel_change_smart failed!\n", __FUNCTION__, __LINE__);
			}
		...
		case ALI_NIM_DRIVER_T2_SIGNAL_ONLY:
	    {
			...
			//PRINTK_INFO("[kangzh]line=%d,%s,2!\n",__LINE__, __FUNCTION__);
			//get_user(t2_flag, (unsigned int*)parg);
		/*	
	...
	*/
	    ...
	    default:
	    {
	        CXD2838_PRINTF(NIM_LOG_DBG,"ERROR-->[%s]line=%d,23\n", __FUNCTION__, __LINE__);
	        ret = -ENOIOCTLCMD;
	        break;
	    }
	 }
...
static RET_CODE ali_cxd2838_nim_open(struct inode *inode, struct file *file)
{
    ...
    CXD2838_PRINTF(NIM_LOG_DBG,"[%s] line=%d,enter\n", __FUNCTION__, __LINE__);
...
static int __devinit ali_cxd2838_nim_init(void)
{
    ...
    CXD2838_PRINTF(NIM_LOG_DBG,"register NIM device end.\n");
