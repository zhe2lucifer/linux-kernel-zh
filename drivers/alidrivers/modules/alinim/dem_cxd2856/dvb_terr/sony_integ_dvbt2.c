                                    ...
                                    sony_dvbt2_tune_param_t * pTuneParam)
{
	...
	ALI_CXD2856_DATA_T *user = pInteg->user;
	/************************************/
	...
	#endif 
	/************* add by ali *****************/
	if(user->tuner_i2c_communication_mode == SONY_DEMOD_TUNER_I2C_CONFIG_REPEATER) //repteater mode,else is getway
	{
		/* Enable the I2C repeater */
	    result = sony_demod_I2cRepeaterEnable (pInteg->pDemod, 0x01);
	    if (result != SONY_RESULT_OK) {
    ...
    /* Tune the RF part */
	if(NULL == user->tuner_control.nim_tuner_control)
	{
	...
	{	
		//tune tuner
		if(user->tuner_control.nim_tuner_control(user->tuner_id,pTuneParam->centerFreqKHz,pTuneParam->bandwidth,0,0,0) < 0)
		{
...
/****************************ali add ***********************************/
	...
	{	
		...
		//tune tuner
		if(user->tuner_control.nim_tuner_control(user->tuner_id,centerFreqKHz,bandwidth,0,0,0) < 0)
		{
			result = SONY_RESULT_ERROR_I2C;
			SONY_TRACE_RETURN (result);
		}
