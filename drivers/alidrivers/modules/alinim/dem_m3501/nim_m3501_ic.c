...
INT32 nim_s3501_fft_find_channel(struct nim_device *dev, UINT32 *tune_freq)
{
    ...
    if (priv->ul_status.m_s3501_type == NIM_CHIP_ID_M3501B)
    {
        ...
        {
          S3501_PRINTF(NIM_LOG_DBG,"Need not configure TSO speed.\n");
    ...
    S3501_PRINTF(NIM_LOG_DBG,"tune_freq = %d\n", (int)(*tune_freq));
    ...
    if (1 == success)
    {
        for (i = 0; i < ch_number; i++)
        {
            ...
            S3501_PRINTF(NIM_LOG_DBG,"    m_Freq[%d] is %d\n", priv->ul_status.m_crnum,
    ...
    {
        S3501_PRINTF(NIM_LOG_DBG,"    Base band width is %d, ch_number is%d\n", (int)(if_freq / 1024), (int)ch_number);
    }
...
void nim_s3501_fft_result_read(struct nim_device *dev)
{
    ...
    for (m = 0; m < 1000; m++)
    {
        ...
        {
#ifdef NIM_TS_PORT_CAP
            S3501_PRINTF(NIM_LOG_DBG," ts data transfer finish\n");
            break;
#else
            nim_s3501_cap_iq_enerage(dev);
            //R2FFT
            R2FFT(priv->ul_status.FFT_I_1024, priv->ul_status.FFT_Q_1024);
            S3501_PRINTF(NIM_LOG_DBG," ADC DATA  transfer finish\n");
            break;
...
INT32 nim_s3501_cap_start(struct nim_device *dev, UINT32 startfreq, UINT32 sym, UINT32 *cap_buffer)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_S3501_cap_start:\n");
	...
	if (dev_priv->nim_tuner_control != NULL)
	{
	    ...
	    if (result == SUCCESS)
	    {
	        S3501_PRINTF(NIM_LOG_DBG,"Config tuner %dMHz success\n", (int)freq);
	    } 
	    else
	    {
	        S3501_PRINTF(NIM_LOG_DBG,"Config tuner %dMHz failed\n", (int)freq);
	    }
	}
	...
	if (dev_priv->nim_tuner_status != NULL)
	{
	    for( i = 0; i < 1000; i++ )
	    {
	        ...
	        if (lock)
	        {
	            S3501_PRINTF(NIM_LOG_DBG,"Tuner lock after %d times delay\n", (int)(i + 1));
	            break;
	        }
	    }
	}
    ...
    if(lock == 0)
    {
        S3501_PRINTF(NIM_LOG_DBG,"Tuner unlock at %dMHz\n", (int)freq);
    }
...
INT32 nim_s3501_cap(struct nim_device *dev, UINT32 startfreq, UINT32 *cap_buffer, UINT32 sym)
{
    ...
    priv = (struct nim_s3501_private *) dev->priv;
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_S3501_cap:\n");
...
INT32 nim_s3501_freq_offset_reset(struct nim_device *dev, UINT8 low_sym)
{
    ...
    if (priv->tuner_config_data.qpsk_config & M3501_QPSK_FREQ_OFFSET)
    {
        if (1 == low_sym)
        {
        nim_s3501_set_freq_offset(dev, (-3 * 1024));
         }
        else
        {
...
INT32 nim_s3501_freq_offset_reset1(struct nim_device *dev, UINT8 low_sym, INT32 delfreq)
{
    ...
    if (priv->tuner_config_data.qpsk_config & M3501_QPSK_FREQ_OFFSET)
    {
		if (1 == low_sym)
		{
			nim_s3501_set_freq_offset(dev, (-3 * 1024) + delfreq);
		}
		else
		{
...
INT32 nim_s3501_wideband_scan(struct nim_device *dev, UINT32 start_freq, UINT32 end_freq)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter function nim_s3501_WidebandScan:\n");
    priv = (struct nim_s3501_private *) dev->priv;
    ...
    if(SUCCESS == nim_s3501_wideband_scan_open(dev, start_freq, end_freq, step_freq))
    {
        for (fft_freq = start_freq; fft_freq < end_freq; fft_freq += step_freq)
        {
            ...
            {
                //comm_free(priv->ul_status.adcdata_malloc_addr);
                S3501_PRINTF(NIM_LOG_DBG,"the nim_s3501_WidebandScan function  works wrong !\n");
                S3501_PRINTF(NIM_LOG_DBG,"ERR_FAILURE, Leave the nim_s3501_WidebandScan function !\n");
                return ERR_FAILURE;
            }
        ...
        //comm_free(priv->ul_status.adcdata_malloc_addr);
        S3501_PRINTF(NIM_LOG_DBG,"SUCCESS, Leave the nim_s3501_WidebandScan function !\n");
        return SUCCESS;
    }
    else
    {
        S3501_PRINTF(NIM_LOG_DBG,"ERR_NO_MEM, Leave the nim_s3501_WidebandScan function !\n");
        return ERR_NO_MEM;
    }
}
...
INT32 nim_s3501_channel_search(struct nim_device *dev, UINT32 crnum)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_channel_search:\n");
...
static INT32 nim_s3501_locked_channel(struct nim_device *dev,NIM_SEARCH_PARAMS *params)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"            rs_input is %d\n", (int)rs_input);
    ...
    S3501_PRINTF(NIM_LOG_DBG,"            code_rate is %d\n", code_rate);

    nim_s3501_reg_get_work_mode(dev, &work_mode);
    S3501_PRINTF(NIM_LOG_DBG,"            work_mode is %d\n", work_mode);

    nim_s3501_reg_get_map_type(dev, &map_type);
    S3501_PRINTF(NIM_LOG_DBG,"            map_type is %d\n", map_type);
    ...
    if (priv->ul_status.m_s3501_type == NIM_CHIP_ID_M3501B)
    {
        if(priv->ul_status.m_s3501_sub_type == NIM_CHIP_SUB_ID_M3501C)
        {
            ...
            modcod = modcod >> 1;
            S3501_PRINTF(NIM_LOG_DBG,"            Modcod is %x\n", modcod);
        }
    }
...
static INT32 nim_s3501_r04_0x04(struct nim_device *dev,NIM_SEARCH_PARAMS *params)
{
    ...
    if (params->tr_lock_flag == 0)
    {
        ...
        else
        {
            ...
            {
                 channel_freq_err = 0;				 
				 if (params->priv->is_m3031 && (params->priv->nim_tuner_command != NULL))
				 {
				 	params->priv->nim_tuner_command(params->priv->tuner_index, NIM_TUNER_SET_M3031_FREQ_ERR, &channel_freq_err);
				 }
				 ...
				 if (params->priv->is_m3031 && (params->priv->nim_tuner_command != NULL))
				 {
				 	params->priv->nim_tuner_command(params->priv->tuner_index, NIM_TUNER_GET_M3031_FREQ_ERR, &channel_freq_err);
				 }
            }
			...
			if(params->priv->is_m3031)
			{
				ndelfreq = params->delfreq-channel_freq_err*4; ///kHz (1/256)*1024=4
                nim_s3501_freq_offset_reset1(dev, params->low_sym, ndelfreq);
			}
...
static INT32 nim_s3501_init_search(struct nim_device *dev,NIM_SEARCH_PARAMS *params)
{
    ...
    //S3501_PRINTF(NIM_LOG_DBG,"Tuner lock = %d\n",lock);
...
static INT32 nim_s3501_soft_search(struct nim_device *dev, UINT32 *est_rs, UINT32 *est_freq, INT32 delfreq)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_soft_search:\n");
    ...
    while (1)
    {
        if (params.lock == 0)
        {
            ...
            if (params.lock == 1)
            {
                S3501_PRINTF(NIM_LOG_DBG,"Tuner lock = %d at timeout = %d\n", (int)params.lock, (int)params.timeout);
            }
        ...
        S3501_PRINTF(NIM_LOG_DBG,"        Lock status is 0x%x\n", params.intindex);
