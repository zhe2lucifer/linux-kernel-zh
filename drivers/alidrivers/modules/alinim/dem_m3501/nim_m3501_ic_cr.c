...
INT32 nim_s3501_open_ci_plus(struct nim_device *dev, UINT8 *ci_plus_flag)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"open ci plus enable REG_ad = %02x \n", data);
...
INT32 nim_s3501_di_seq_c_operate(struct nim_device *dev, UINT32 mode, UINT8 *cmd, UINT8 cnt)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"[%s] mode = %d\n",__FUNCTION__, (int)mode);
    switch (mode)
    {
    ...
    case NIM_DISEQC_MODE_BYTES:
        ...
        if(cnt > CNT_NUM_8)
        {
            S3501_PRINTF(NIM_LOG_DBG,"\t\t NIM_ERROR : Diseqc cnt larger than 8: cnt = %d\n", cnt);
            return ERR_FAILED;
        }
        else
        {
...
INT32 nim_s3501_adc_setting(struct nim_device *dev)
{
    ...
    if (data != ver_data)
    {
        S3501_PRINTF(NIM_LOG_DBG," wrong 0x8 reg write\n");
        return ERR_FAILED;
    }
...
INT32 nim_s3501_set_phase_noise(struct nim_device *dev)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"            Eenter function set_phase_noise\n");
    sdat = 0xba;
    ...
    for (debug_time = 0; debug_time < debug_time_thre; debug_time++)
    {
        ...
        if (verdata != data)
        {
            S3501_PRINTF(NIM_LOG_DBG,"!!! RESET BER ERROR!\n");
        }
        data = 0x80;
        ...
        if (verdata != data)
        {
            S3501_PRINTF(NIM_LOG_DBG,"!!! RESET PER ERROR!\n");
        }
        comm_delay(100);
        ...
        S3501_PRINTF(NIM_LOG_DBG,"--- snr/ber/per = %x/%x/%x\n", snr, (unsigned int)ber, (unsigned int)per);
    ...
    {
        S3501_PRINTF(NIM_LOG_DBG,"per_buf[%d] = 0x%x\n", (int)i, (unsigned int)per_buf[i]);
        per_buf[i] >>= 4;
    ...
    if (min_per <= max_per)
    {
        ...
        S3501_PRINTF(NIM_LOG_DBG,"No phase noise detected!\n");
    }
    else
    {
        priv->t_param.t_phase_noise_detected = 1;
        S3501_PRINTF(NIM_LOG_DBG,"Phase noise detected!\n");
        data = 0x42;
        nim_reg_write(dev, R33_CR_CTRL + 0x04, &data, 1);
    }

    S3501_PRINTF(NIM_LOG_DBG,"min_per is %d, max_per is %d\n", (int)min_per, (int)max_per);
    ...
    S3501_PRINTF(NIM_LOG_DBG,"--------------------EXIT set_phase_noise, REG_b5 = 0x%x\n", sdat);
...
INT32 nim_m3501c_fec_ts_off (struct nim_device *dev)
{
    ...
    data = data | 0x10;
    nim_reg_write(dev, RAF_TSOUT_PAD, &data, 1);
    S3501_PRINTF(NIM_LOG_DBG,"            fec ts off\n");
    return SUCCESS;
}
...
INT32 nim_m3501c_fec_ts_on (struct nim_device *dev)
{
    ...
    data = data & 0xef;
    nim_reg_write(dev, RAF_TSOUT_PAD, &data, 1);
    S3501_PRINTF(NIM_LOG_DBG,"            fec ts on\n");
    return SUCCESS;
}
...
INT32 nim_m3501c_reset_tso (struct nim_device *dev)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"            Reset TSO\n");
    return SUCCESS;
}
...
#endif
        S3501_PRINTF(NIM_LOG_DBG,"            Found plug_in_out and reset tso \n");
    }
    return SUCCESS;
}
...
INT32 nim_set_ts_rs(struct nim_device *dev, UINT32 rs_input)
{
    ...
    data = data | 0x10;
    nim_reg_write(dev, RDC_EQ_DBG_TS_CFG, &data, 1);
    S3501_PRINTF(NIM_LOG_DBG,"xxx set rs from register file........... \n");
    return SUCCESS;
}
                                     ...
                                     UINT8 channel_change_flag)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"xxx bit rate kkk is %d \n", (int)bit_rate);
    if (work_mode != M3501_DVBS2_MODE)// DVBS mode
    {
        ...
        data = (rdata & 0x08) | 0x16;
            ...
            S3501_PRINTF(NIM_LOG_DBG,"DVBS USE SPI normal dummy, no SSI debug mode \n");
        }
        else
        {
            ...
            nim_reg_write(dev, RD8_TS_OUT_SETTING, &data, 1);
            S3501_PRINTF(NIM_LOG_DBG,"DVBS  USE SSI debug mode \n");
        }
    }
    else    // DVBS2 mode
    {
        if (((bit_rate >= BIT_RATE_98) || (bit_rate <= ssi_clock_tab[8])) && channel_change_flag)
        {
            ...
            S3501_PRINTF(NIM_LOG_DBG,"DVBS2 Enter normal SPI mode, not use SSI debug..\n");
        }
        else
        {
            ...
            nim_reg_write(dev, RD8_TS_OUT_SETTING, &data, 1);
            S3501_PRINTF(NIM_LOG_DBG,"DVBS2 Enter SSI debug..\n");
        }
    }
    return ci_plus_flag;
                                     ...
                                     UINT8 channel_change_flag)
{
    ...
    data = (rdata & 0x08) | 0x16;    //moclk interv for 4bit.
        ...
        data = 0x20;
        nim_reg_write(dev, RAD_TSOUT_SYMB + 0x01, &data, 1);
        S3501_PRINTF(NIM_LOG_DBG,"S3501D Enter 4bit Mode============= \n");
    }
    else
    {
        //TS 4bit mode
        ...
        if (work_mode != M3501_DVBS2_MODE)// DVBS mode
        {
            ...
            nim_reg_write(dev, RDC_EQ_DBG_TS_CFG, &data, 1);
            S3501_PRINTF(NIM_LOG_DBG,"M3501B Enter DVBS 4bit mode \n");
        }
        else    // DVBS2 mode
        {
            // For CI plus test.
            ...
            {
                S3501_PRINTF(NIM_LOG_DBG,"Low bit rate, Close CI plus......................... \n");
            }
            else
            {
                nim_s3501_open_ci_plus(dev, &ci_plus_flag);
            }
            S3501_PRINTF(NIM_LOG_DBG,"M3501B Enter DVBS2 4bit mode..\n");
        }
                                     ...
                                     UINT8 channel_change_flag)
{
    ...
    data = (rdata & 0x08) | 0x16;
        ...
        data = 0x00;
        nim_reg_write(dev, RAD_TSOUT_SYMB + 0x01, &data, 1);
        S3501_PRINTF(NIM_LOG_DBG,"S3501D Enter 2bit Mode============= \n");
    }
    else
    {
        //TS 2bit mode
        ...
        if (work_mode != M3501_DVBS2_MODE)// DVBS mode
        {
            ...
            data = data | 0x20;                              //add for ssi_clk change point
            nim_reg_write(dev, RDC_EQ_DBG_TS_CFG, &data, 1);
            S3501_PRINTF(NIM_LOG_DBG,"Enter DVBS 2bit mode \n");
        }
        else    // DVBS2 mode
        {
            // For CI plus test.
            ...
            S3501_PRINTF(NIM_LOG_DBG,"Enter DVBS2 2bit mode..\n");
        }
                                     ...
                                     UINT8 channel_change_flag)
{
    ...
    if (work_mode != M3501_DVBS2_MODE)
    {
        ...
        data = data | 0x20;                             //add for ssi_clk change point
        nim_reg_write(dev, RDC_EQ_DBG_TS_CFG, &data, 1);
        S3501_PRINTF(NIM_LOG_DBG,"DVBS USE SSI mode \n");
    }
    else
    {
        //  DVBS2 mode
        ...
        S3501_PRINTF(NIM_LOG_DBG,"DVBS2 USE SSI mode \n");
    }
                            ...
                            UINT32 rs_input, UINT8 channel_change_flag)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_set_ts_mode:\n");
    ...
    if (priv->ul_status.m_s3501_type == NIM_CHIP_ID_M3501B)
    {
        if((priv->ul_status.m_s3501_sub_type == NIM_CHIP_SUB_ID_M3501C))
        {
            ...
            {
                S3501_PRINTF(NIM_LOG_DBG,"C3501C use NEW TSO, but enter wrong function, nim_s3501_set_ts_mode \n");
                return SUCCESS;
            }
            else
            {
                S3501_PRINTF(NIM_LOG_DBG,"C3501C use OLD TSO, nim_s3501_set_ts_mode \n");
            }
        }
        nim_reg_read(dev, RC0_BIST_LDPC_REG, &data, 1);
        data = data | 0xc0;
    ...
    else
    {
        ...
        if(rdata != data)
        {
            S3501_PRINTF(NIM_LOG_DBG,"M3501A set TS mode faily !(%0x , %0x )\n", data, rdata);
        }
        else
        {
            S3501_PRINTF(NIM_LOG_DBG,"M3501A set TS mode successfully !( %0x )\n", data);
        }

    }

    S3501_PRINTF(NIM_LOG_DBG,"Leave nim_s3501_set_ts_mode\n");
...
INT32 nim_s3501_set_polar(struct nim_device *dev, UINT8 polar)
{
	...
	S3501_PRINTF(NIM_LOG_DBG,"[%s]line=%d,enter,polar=%d!\n",__FUNCTION__,__LINE__,polar);
    ...
    if ((priv->tuner_config_data.qpsk_config & M3501_POLAR_REVERT) == 0x00) //not exist H/V polarity revert.
    {
        ...
        {
            data &= 0xBF;
			S3501_PRINTF(NIM_LOG_DBG,"[%s]line=%d,CR5C is 00!\n",__FUNCTION__,__LINE__);
        }
        else if (NIM_PORLAR_VERTICAL == polar)
        {
            data |= 0x40;
			S3501_PRINTF(NIM_LOG_DBG,"[%s]line=%d,CR5C is 40!\n",__FUNCTION__,__LINE__);
        }
        else
        {
            S3501_PRINTF(NIM_LOG_DBG,"nim_s3501_set_polar error\n");
            return ERR_FAILUE;
        }
    }
...
INT32 nim_m3501c_close_dummy (struct nim_device *dev)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"            nim m3501c close dummy for dvbs \n");
...
INT32 nim_m3501c_open_dummy (struct nim_device *dev)
{
    ...
    data = data | 0x02;
    nim_reg_write(dev, RF0_HW_TSO_CTRL, &data, 1);
    S3501_PRINTF(NIM_LOG_DBG,"            nim m3501c insert dummy\n");
...
INT32 nim_m3501_ts_off (struct nim_device *dev)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"            Final ts on\n");
    return SUCCESS;
}
...
INT32 nim_m3501_ts_on (struct nim_device *dev)
{
    ...
    data = data | 0x80;
    nim_reg_write(dev, R9D_RPT_DEMAP_BETA + 1, &data, 1);
    S3501_PRINTF(NIM_LOG_DBG,"            Final ts off\n");
    return SUCCESS;
}
...
INT32 nim_m3501c_invert_moerrj (struct nim_device *dev)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"            nim m3501c invert_moerrj \n");
    return SUCCESS;
}
...
INT32 nim_m3501c_recover_moerrj (struct nim_device *dev)
{
    ...
    data = data & 0xf7;
    nim_reg_write(dev, RF0_HW_TSO_CTRL + 6, &data, 1);
    S3501_PRINTF(NIM_LOG_DBG,"            nim m3501c recover_moerrj\n");
    return SUCCESS;
}
...
void nim_s3501_set_freq_offset(struct nim_device *dev, INT32 delfreq)
{
    ...
    if(temp > 0xffff)
    {
        S3501_PRINTF(NIM_LOG_DBG,"\t\t NIM_Error: Symbol rate set error %d\n", (int)temp);
        temp = 0xffff;
    }
...
void nim_s3501_set_rs(struct nim_device *dev, UINT32 rs)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"\t\t set ts %d with dsp clock %d\n", (int)rs, (int)(sample_rate / 1000));
...
INT32 nim_s3501_demod_restart(struct nim_device *dev)
{
		if(dev == NULL)
			return -1;

		nim_s3501_demod_ctrl(dev, NIM_DEMOD_CTRL_0X91);
		comm_sleep(5000);//Wait 5ms
