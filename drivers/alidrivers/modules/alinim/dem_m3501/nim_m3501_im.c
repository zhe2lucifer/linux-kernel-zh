...
INT32 nim_s3501_read(struct nim_device *dev, UINT8 bmemadr, UINT8 *pdata, UINT8 blen)
{
    ...
    if (err)
    {
        ...
        S3501_PRINTF(NIM_LOG_DBG,"s3501 i2c read error = %d,chip_adr=0x%x,bmemadr=0x%x,I2C_FOR_S3501 = %d\n", (int)(-err), 
    ...
    else
    {
        ...
        {
            S3501_PRINTF(NIM_LOG_DBG,"t_i2c_err_flag==1\n");
...
INT32 nim_s3501_write(struct nim_device *dev, UINT8 bmemadr, UINT8 *pdata, UINT8 blen)
{
    ...
    if (err != 0)
    {
        ...
        nim_s3501_set_err(dev);
        S3501_PRINTF(NIM_LOG_DBG,"s3501 i2c write error = %d,chip_adr=0x%x,bmemadr=0x%x,I2C_FOR_S3501 = %d\n", 
...
void nim_s3501_task(UINT32 param1, UINT32 param2)
{
	...
	S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_task:\n");
    ...
    while (priv->work_alive)
    {
        ...
        if((flag_ptn & NIM_FLAG_CHN_CHG_START) && (flag_ptn != OSAL_INVALID_ID))
        {
            ...
            S3501_PRINTF(NIM_LOG_DBG,"\t\t Here is the task for M3501C wait channel lock \n");
        }
#endif
        if (priv->ul_status.m_s3501_type == NIM_CHIP_ID_M3501B)
        {
                 ...
                 (priv->tsk_status.m_lock_flag == NIM_LOCK_STUS_CLEAR))
            {
                if(v_cnt_val  == CNT_VAL_10)
                {
                    ...
                    v_cnt_val = 0;
                   // S3501_PRINTF(NIM_LOG_DBG,"\t\t Here is the task for M3501C plug_in_out \n");
#ifdef HW_ADPT_CR
...
INT32 nim_s3501_channel_change(struct nim_device *dev, UINT32 freq, UINT32 sym, UINT8 fec)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_channel_change : freq=%d, sym=%d, fec=%d \n", (int)freq, (int)sym, fec);
    ...
    if (priv->nim_tuner_control != NULL)
    {
		...
		{
			priv->nim_tuner_command(priv->tuner_index, NIM_TUNER_SET_M3031_FREQ_ERR, &channel_freq_err);
		}
		...
		if (priv->is_m3031 && (priv->nim_tuner_command != NULL))
		{
			priv->nim_tuner_command(priv->tuner_index, NIM_TUNER_GET_M3031_FREQ_ERR, &channel_freq_err);
		}
    }
    ...
    if(priv->is_m3031)
    {
		...
		//nim_s3501_set_freq_offset(dev, channel_freq_err_khz);//here the unit is khz
    ...
    {
        S3501_PRINTF(NIM_LOG_DBG,"ERROR: CHIP_ID_type=0x%0x \n", (unsigned int)priv->ul_status.m_s3501_type);
    }
    ...
    priv->ul_status.s3501_chanscan_stop_flag = 0;
    S3501_PRINTF(NIM_LOG_DBG,"Leave nim_s3501_channel_change\n");
...
static INT32 nim_s3501_fft_scan(struct nim_device *dev,NIM_AUTOSCAN_PARAMS *params)
{
    ...
    {
		step_freq = 25;//params->adc_sample_freq/4;
    }
	else
	{
...
INT32 nim_s3501_autoscan(struct nim_device *dev, NIM_AUTO_SCAN_T *pstauto_scan)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_autoscan:\n");
   ...
   (int)(((params.end_t - params.start_t) + 500) / 1000 - (((params.end_t - params.start_t) + 500) / 1000 / 60) * 60));
    S3501_PRINTF(NIM_LOG_DBG,"Leave nim_s3501_autoscan\n");
