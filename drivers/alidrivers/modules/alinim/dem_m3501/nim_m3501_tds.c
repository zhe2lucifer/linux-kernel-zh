...
static int           g_log_level = NIM_LOG_DBG;
...
void libc_printf_x(int log_level,const char *fmt, ...)
{
	va_list arg;
    ...
    {
	   va_start(arg, fmt);
	   libc_vprintf(NULL, (BYTE_OUTPUT)output_to_uart, fmt, arg);
   	   va_end(arg);
    }
...
INT32 nim_s3501_attach(struct QPSK_TUNER_CONFIG_API *ptrqpsk_tuner)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_attach:\n");

    if (ptrqpsk_tuner == NULL)
    {
        S3501_PRINTF(NIM_LOG_DBG,"Tuner Configuration API structure is NULL!/n");
        return ERR_NO_DEV;
    }
    if (nim_dev_num > NIM_DEV_NUM_2)
    {
        S3501_PRINTF(NIM_LOG_DBG,"Can not support three or more S3501 !/n");
        return ERR_NO_DEV;
    }
    ...
    if (dev == NULL)
    {
        S3501_PRINTF(NIM_LOG_DBG,"Error: Alloc nim device error!\n");
        return ERR_NO_MEM;
    }
    ...
    {
        dev_free(dev);
        S3501_PRINTF(NIM_LOG_DBG,"Alloc nim device prive memory error!/n");
        return ERR_NO_MEM;
    }
    ...
    {
        S3501_PRINTF(NIM_LOG_DBG,"Error: Register nim device error!\n");
        comm_free(priv_mem);
        dev_free(dev);
        return ERR_NO_DEV;
    }
            ...
            (priv_mem->tuner_config_data.qpsk_config & 0xc0) == M3501_2BIT_MODE)
    {
        if(SYS_MAIN_BOARD == BOARD_DB_M3606_01V01)
        {
            ...
            S3501_PRINTF(NIM_LOG_DBG,"M3501A SSI 2bit mode, auto change to 1bit mode\n");
        }
    }

    ptrqpsk_tuner->device_type = priv_mem->ul_status.m_s3501_type;

    S3501_PRINTF(NIM_LOG_DBG,"Leave nim_s3501_attach\n");
...
INT32 nim_s3501_open(struct nim_device *dev)
{
    ...
    S3501_PRINTF(NIM_LOG_DBG,"Enter nim_s3501_open:\n");
...
#endif
    S3501_PRINTF(NIM_LOG_DBG,"Leave nim_s3501_open\n");
    return SUCCESS;
}
...
INT32 nim_s3501_ioctl_ext(struct nim_device *dev, INT32 cmd, void *param_list)
{
    ...
    priv = (struct nim_s3501_private *) dev->priv;
    S3501_PRINTF(NIM_LOG_DBG,"    Enter fuction nim_s3501_event_control\n");
    switch (cmd)
    {
...
#endif
        S3501_PRINTF(NIM_LOG_DBG,">>>[JIE] NIM_DRIVER_AUTO_SCAN\n");
    ...
    case NIM_DRIVER_CHANNEL_CHANGE:
        /* Do Channel Change */
        S3501_PRINTF(NIM_LOG_DBG,">>>[JIE] NIM_DRIVER_CHANNEL_CHANGE\n");
        {
