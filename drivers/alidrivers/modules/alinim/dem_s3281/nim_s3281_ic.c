...
// 20160622 add by paladin, for recognizing the version of IC
BOOL new_hw_ver = FALSE;

extern UINT8 g_j83b_channel_change_flag;
...
static void     nim_s3281_dvbc_set_rs(struct nim_device * dev,UINT32 sym);
static void        nim_s3281_dvbc_set_search_rs(struct nim_device * dev,UINT16 rs_min, UINT16 rs_max );

extern UINT32 ali_sys_ic_get_chip_id(void);
...
static void nim_s3281_dvbc_mon_catch_ad_data(struct nim_device *dev)
{
    ...
    data = (data & 0xcf) | 0x10;
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM1, &data, 1); // set to catch mode.
    ...
    do
    {
        ...
        {
            S3281_PRINTF(NIM_LOG_DBG,"Fast Config tuner failed !\n");
        }

        // Slow config tuner  //, SLOW_TIMECST_AGC, _1ST_I2C_CMD
        if(dev_priv->nim_tuner_control(dev_priv->tuner_id, freq, 0) == ERR_FAILUE)
        {
            S3281_PRINTF(NIM_LOG_DBG,"Slow Config tuner failed!\n");
        }
        ...
        S3281_PRINTF(NIM_LOG_DBG,"catch AD Tuner Lock Times=%d, *lock=%d !!\n", tuner_retry, lock);
    }
    while(0 == lock);
    ...
    while(1)
    {
        ...
        {
            //S3281_PRINTF(NIM_LOG_DBG,"AGC lock time_out !!\n");
            goto out;
        }
        time_out += 1;
        ...
        if(0x01 == (data & 0x01))
        {
            //S3281_PRINTF(NIM_LOG_DBG,"AGC locked!\n");
            break;
        }
    }

    nim_s3281_dvbc_read(dev, NIM_S3202_FSM11, &data, 1);
    data = (data & 0xE7) | 0x08;
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM11, &data, 1);
    ...
    while(!fft_buf_full)
    {
        ...
        {
            //S3281_PRINTF(NIM_LOG_DBG,"FFT buffer full time out!!\n");
            goto out;
        }
        time_out += 1;
        nim_s3281_dvbc_read(dev, NIM_S3202_MONITOR1, &data, 1);
    ...
    {
        data = i;
        nim_s3281_dvbc_write(dev, NIM_S3202_MONITOR2, &data, 1);
        nim_s3281_dvbc_read(dev, NIM_S3202_MONITOR3, data_reg, 3);
        di[i] = (INT16)((data_reg[2] & 0x0f) << 6 | (data_reg[1] & 0xfc) >> 2);
        dq[i] = (INT16)((data_reg[1] & 0x03) << 8 | (data_reg[0]));
        //S3281_PRINTF(NIM_LOG_DBG,"ram[%03d] I,Q= 0x%03x, 0x%03x \n",i,di[i],dq[i]);
    ...
    for (i = 0 ; i < 256; i++)
    {
        ...
        //S3281_PRINTF(NIM_LOG_DBG," %03d, di = 0x%03x, dq = 0x%03x, Result = %d \n",i , di[i], dq[i],spectrum);
    ...
    S3281_PRINTF(NIM_LOG_DBG," Sum1 = %d, Sum2 = %d \n", sum1, sum2);
#endif
...
//#ifdef ACI_IMPROVED_ENABLE    //start for 6M ACI performance improve by grace
		...
		if (g_j83b_channel_change_flag)
			nim_s3281_dvbc_improve_impluse_noise(dev);
...
INT32 nim_s3281_dvbc_monitor_agc_status(struct nim_device *dev,UINT16 *cir)
{
    ...
    while (i < RF_GAIN_TIMES)
    {
        ...
        comm_sleep(10);
        nim_s3281_dvbc_read(dev, 0x15, data, 2);
    ...
    {
        nim_s3281_dvbc_read(dev, 0x12, data, 1);
        if (data[0] > if_sml_val1)
        {
            ...
            S3281_PRINTF(NIM_LOG_DBG,"Reach Max sensitivity once!\n ");
        }
    }
    else if (rf_gain_sum > rf_big_th2) // so-called TH2.
    {
        ...
        if ((data[0] < if_def_val2) && (reset_once == FALSE))
        {
            ...
            S3281_PRINTF(NIM_LOG_DBG,"take over point recovey! \n");
...
static INT32 nim_s3281_agc0a(struct nim_device *dev, UINT8 data)
{
    UINT8 new_cr0a = 0;
    UINT8 agc_ref = 0;
    ...
    if(data >= MAGIC_CONST_0X1F)
    {
        if (1 == fsm_trans)    // flag to record and clear the time out cnt from unlock to lock
        {
            ...
            nim_s3281_dvbc_read(dev, 0x0a, &new_cr0a, 1);

            if (new_cr0a != agc_ref)
            {
                new_cr0a = agc_ref;
                nim_s3281_dvbc_write(dev, 0x0a, &new_cr0a, 1);
            }
        }
...
INT32 nim_s3281_dvbc_monitor_agc0a_loop(struct nim_device *dev,UINT16 *agc0a_loop)
{
    ...
    if ((dev_priv->s3281_cur_channel_info.modulation != MODULATION_6) || (dev_priv->s3281_cur_channel_info.symbol_rate < SYM_6500))
    {
        if (first_cal == 0)
        {
            first_cal = 1;
    ...
    //agc_ref = dev_priv->tuner_config_data.AGC_REF;//warning: variable 'agc_ref' set but not used
    nim_s3281_dvbc_read(dev, 0x56, &data, 1);
    data = data & 0x3f;

    ret = nim_s3281_agc0a(dev, data);
    return ret;
...
static INT32 nim_s3281_dvbc_monitor_cr_loop(struct nim_device *dev, UINT16 *cr_loop)
{
    ...
    if (read_ber_cnt > READ_BER_CNT_4)
    {
        ...
        {
            nim_s3281_dvbc_read(dev, 0xd8, &data, 1);
            if (data != 0x0a)
            {
                data = 0x0a;
                nim_s3281_dvbc_write(dev, 0xd8, &data, 1);
            }
    ...
    else
    {
        if (((data & 0x3f) < 0x0f) && (1 == eq_st_flg))
        {
            ...
            if (eq_unlock_cnt > EQ_UNLOCK_CNT_2)
            {
                ...
                S3281_PRINTF(NIM_LOG_DBG,"%s,      CRd8  =   0x%x \n", __FUNCTION__, data);
                ...
                if (eq_unlock_cnt > EQ_UNLOCK_CNT_3)
                {
                    ...
                    if (fff_len_ch == 0)
                    {
                        nim_s3281_dvbc_read(dev, 0x28, &data, 1);
                        data = (data & 0xf8) | 0x03;
                        nim_s3281_dvbc_write(dev, 0x28, &data, 1);
                    ...
                    S3281_PRINTF(NIM_LOG_DBG,"%s,      CR28  =   0x%x \n", __FUNCTION__, data);
...
INT32 nim_s3281_hw_init(struct nim_device *dev)
{
	...
	UINT32 chip_id = ali_sys_ic_get_chip_id(); //clean warning
    if(dev == NULL)
    {
	    return ERR_FAILURE;
	}	
    ...
    data1 = 0x20;
    nim_s3281_dvbc_write(dev, NIM_S3281_CBR1_R408, &data1, 1);

	// Modify tso clock to 30M by Gavin 20160422, for auto qam paly issue
	data1 = 0x11;
    nim_s3281_dvbc_write(dev, NIM_S3281_CBR3_R40A, &data1, 1);	

	data1 = 0x00;
    nim_s3281_dvbc_write(dev, NIM_S3281_CBR2_R409, &data1, 1);	
    //set  INI_FREQ_OFFSET to zero
    nim_s3281_dvbc_read(dev, NIM_S3202_FSM4, &data1, 1);
    data1 = 0;
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM4, &data1, 1);

    //ad out sign set from unsigned to signed
    nim_s3281_dvbc_read(dev, NIM_S3281_ANA_INT_SET_R420, &data1, 1);
    data1 = data1 | 0x02;
    nim_s3281_dvbc_write(dev, NIM_S3281_ANA_INT_SET_R420, &data1, 1);

    //Set the buffer addr for J83B deinterleave: AutoRun12=wm 0xb8028428 buffer_addr

    //TSO set:AutoRun13=wm 0xb8028404 0x0C210010
    nim_s3281_dvbc_read(dev, NIM_S3281_TSO1_R406, &data1, 1);
    data1 = data1 & 0xFC;    //SPI select
    nim_s3281_dvbc_write(dev, NIM_S3281_TSO1_R406, &data1, 1);

    //AutoRun14=wm 0xb8028400 0xFF1F4781
    nim_s3281_dvbc_read(dev, NIM_S3281_TSO0_R400, &data1, 1);
    data1 =  0x81;
    nim_s3281_dvbc_write(dev, NIM_S3281_TSO0_R400, &data1, 1);
    ...
    nim_s3281_dvbc_read(dev, NIM_S3202_AGC10, &data1, 1);
    data1 = data1 & 0xdf;
    data1 = data1 | 0x80;
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC10, &data1, 1);

    //set the AGC lock length: AutoRun8=wm 0xB802800C 0xA8C5003B
    nim_s3281_dvbc_read(dev, NIM_S3202_AGC5, &data1, 1);
    data1 = data1 & 0xfb;
    data1 = data1 | 0x05;
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC5, &data1, 1);
    ...
    nim_s3281_dvbc_read(dev, NIM_S3281_CLKGATE_R40F, &data1, 1);
    data1 = data1 & 0x3f;
    nim_s3281_dvbc_write(dev, NIM_S3281_CLKGATE_R40F, &data1, 1);
    ...
    data1 = 0x12;
    nim_s3281_dvbc_write(dev, NIM_S3202_CR_INT_PATH_GAIN_1, &data1, 1);
    data1 = 0x11;
    nim_s3281_dvbc_write(dev, NIM_S3202_CR_INT_PATH_GAIN_2, &data1, 1);
    data1 = 0x08;
    nim_s3281_dvbc_write(dev, 0xf6, &data1, 1);
    data1 = 0x0a;
    nim_s3281_dvbc_write(dev, 0xf7, &data1, 1);


    //Modify for SKW M3202C 674M can't lock issue.(2012-8-13)
    data = 0x3a; //bypass pre-echo, awgn still on. Reg[0x0c]
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC3, &data, 1);

    //joey, 20120720, for skyworth 674M issue. modify CR register for 64Qam easy lock.
    data = 0x11; // CR_P_GAIN_4,Reg[0xcd]
    nim_s3281_dvbc_write(dev, NIM_S3202_CR_PRO_PATH_GAIN_3, &data, 1);

    data = 0x09; // CR_I_GAIN_4,Reg[0xd6]
    nim_s3281_dvbc_write(dev, NIM_S3202_CR_TIME_OUT_FOR_ACQ_2, &data, 1);

    data = 0x13; // CR_P_GAIN_5,Reg[0xce]
    nim_s3281_dvbc_write(dev, NIM_S3202_CR_PRO_PATH_GAIN_4, &data, 1);
    ...
    nim_s3281_dvbc_write(dev, NIM_S3202_CR_LOCK_THRD_27, &data, 1);

    data = 0x64; // CR_THRED_LOCK_64_6,Reg[0xba]
    nim_s3281_dvbc_write(dev, NIM_S3202_CR_LOCK_THRD_28, &data, 1);
    ...
    data = 0x10;//Reg[0x5e]
    nim_s3281_dvbc_write(dev, NIM_S3202_RS_BER1, &data, 1);

    data = 0x08;//Reg[0x5f]
    nim_s3281_dvbc_write(dev, NIM_S3202_RS_BER2, &data, 1);

    data = 0x9f; // Change threshod to 0x9f. Reg[0x67]
    nim_s3281_dvbc_write(dev, NIM_S3202_EQ_COEF5, &data, 1);
   ...
   //kent for S3921 (2014/3/17)
    ...
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC6, &data, 1);
    data = tunerconfig.RF_AGC_MIN;
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC7, &data, 1);
    data = tunerconfig.IF_AGC_MAX;
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC8, &data, 1);
    data = tunerconfig.IF_AGC_MIN;
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC9, &data, 1);
    data = tunerconfig.AGC_REF;
    nim_s3281_dvbc_write(dev, NIM_S3202_AGC1, &data, 1);
    //20130107 modified by russell, for Kingvon issue
    nim_s3281_dvbc_read(dev, NIM_S3202_Q32_MODE1_1, &data, 1);
    data &= 0xf7;
    nim_s3281_dvbc_write(dev, NIM_S3202_Q32_MODE1_1, &data, 1);

    //20130107 modified by russell. for watch-dog timer (FSM reset gap) enlarge to 5.8s.
    data = 0xa0;
    nim_s3281_dvbc_write(dev, NIM_S3202_TIMEOUT_THRESHOLD, &data, 1);

    // 20160622 add by paladin, for recognizing the version of IC
    nim_s3281_dvbc_read(dev, 0x13c, &data1, 1);
    data = 0x5a;
    ...
    nim_s3281_dvbc_write(dev, 0x13c, &data1, 1);
    // end
    
	 //set I/Q swap
    nim_s3281_dvbc_read(dev, NIM_S3202_ADC_CONFIG, &data, 1);
    if (qam_config.qam_config_advanced & 0x04)
    {
		   data |= 0x20;
		   nim_s3281_dvbc_write(dev, NIM_S3202_ADC_CONFIG, &data, 1);
    }
	
	//20160526 leo.liu set the QAM2_mode and DEMUX TS SPI select
	
	S3281_PRINTF(NIM_LOG_DBG,"[%s] line=%d,here chip == 0x%x qam_addr = 0x%x!\n", __FUNCTION__, __LINE__,chip_id,priv->qam_addr);
	if (ALI_C3505 == chip_id)	
	{
		if(S3281_QAM1_SOC_BASE_ADDR == priv->qam_addr)//QAM1
		{
			if(0x01 == priv->qam_mode)
			{
				__REG32ALI(0X1800006c) &= ~((1 << 19) | (1 << 20));//clear bit j83b
				//__REG32ALI(0X1800006c) |= 0x00800000;//j83b
			}
			else
			{
				__REG32ALI(0X1800006c) &= ~((1 << 19) | (1 << 20));//clear bit
				__REG32ALI(0X1800006c) |= (1 << 19);
				//__REG32ALI(0X1800006c) |= 0x00A00000;//j83ac
			}
		}
		else if(S3281_QAM2_SOC_BASE_ADDR == priv->qam_addr)//QAM2
		{
			__REG32ALI(0x1800006c) |= (1 << 23);//DMX TS SPI select QAM2_TS,not DVB-S2
			if(0x01 == priv->qam_mode)
			{
				__REG32ALI(0X1800006c) &= ~((1 << 21) | (1 << 22));//clear bit j83b
				//__REG32ALI(0X1800006c) |= 0x00800000;//j83b
			}
			else
			{
				__REG32ALI(0X1800006c) &= ~((1 << 21) | (1 << 22));//clear bit
				__REG32ALI(0X1800006c) |= (1 << 21);
				//__REG32ALI(0X1800006c) |= 0x00A00000;//j83ac
			}
		}
		else
		{
			S3281_PRINTF(NIM_LOG_DBG,"[%s] line=%d,no this qam!\n", __FUNCTION__, __LINE__);
		}
	}
	else if (ALI_S3821 == chip_id)
	{
		if(0x01 == priv->qam_mode)
		{
			__REG32ALI(0X1800006c) &= ~((1 << 19) | (1 << 20));//j83b
		...
		{
			__REG32ALI(0X1800006c) &= ~((1 << 19) | (1 << 20));//clear bit
			__REG32ALI(0X1800006c) |= (1 << 19);//j83ac
		}
	}
...
void nim_set_pro_path_gain(struct nim_device *dev ,UINT32 sym,UINT8 fec)
{
    UINT8 data = 0;
    UINT8 data1 = 0;
    ...
    if (sys_ic_get_rev_id() == IC_REV_0)
    {
            ...
            ((0x08 == fec) && (sym <= SYM_1800)) )
        {
            ...
            data1 = 0x12;
            nim_s3281_dvbc_write(dev,NIM_S3202_CR_PRO_PATH_GAIN_3, &data, 1);
            nim_s3281_dvbc_write(dev,NIM_S3202_CR_PRO_PATH_GAIN_4, &data1, 1);
        }
        else
        {
            data = 0x24;
...
void nim_set_rs_and_range(struct nim_device *dev,UINT32 sym)
{
    UINT32 rs_max = 0;
    UINT32 rs_min = 0;
    ...
    nim_s3281_dvbc_set_rs(dev, sym);  // eg. sym = 6900
    nim_s3281_dvbc_set_search_rs(dev,rs_min, rs_max);
}
...
INT32 nim_s3281_dvbc_monitor_berper(struct nim_device *dev, BOOL *bervalid)
{
    ...
    static UINT32 ref_per = 9700;
    static UINT32 max_per = 10000;
    static UINT32 ref_ber = 66000;
    static UINT32 max_ber = 75000;    
    static UINT32 slope_ber = 30;
    static UINT32 delta_ber = 9000;
    ...
    //CR62.
    nim_s3281_dvbc_read(dev, NIM_S3281_BERPER_RPT6_R43E, &data, 1);
    if(0 != (data & 0x40))
    {
        data_b[2] = data & 0x3f;
        nim_s3281_dvbc_read(dev, NIM_S3281_BERPER_RPT4_R43C, &data_b[0], 2);
        ber_tmp = (UINT32)((data_b[2] << 16 | data_b[1] << 8 | data_b[0]) >> 1);

        nim_s3281_dvbc_read(dev, NIM_S3281_BERPER_RPT3_R43B, &data, 1);
        btemp = (data & 0x7f) << 8;
        nim_s3281_dvbc_read(dev, NIM_S3281_BERPER_RPT2_R43A, &data, 1);
        btemp |= data;
        PER_COUNTS = btemp ;
        ...
        // Correct ber when per exist, paladin.ye 04/12/2016
        if(PER_COUNTS < 9600)    // 0 ~ 9600
        {
            BER_COUNTS = (ber_tmp * PER_COUNTS * ber_factor) / ((10000 - PER_COUNTS)<< 3) + ber_tmp;
        }
        else if(PER_COUNTS < 9800)   // 9600 ~ 9800
        {
            BER_COUNTS = (ber_tmp * PER_COUNTS * ber_factor) / ((10000 - PER_COUNTS)<< 3) + ber_tmp;
            ref_ber = BER_COUNTS;
            ref_per = PER_COUNTS;
            delta_ber = max_ber - ref_ber;
            slope_ber = delta_ber/(max_per - ref_per);  
            S3281_PRINTF(NIM_LOG_DBG,"Refresh param, BER_COUNTS = %d, PER_COUNTS = %d, delta_ber = %d, slope_ber = %d \n", BER_COUNTS, PER_COUNTS, \
                delta_ber, slope_ber);
        }
        else    // more than 9800
        {
            BER_COUNTS = ref_ber + (PER_COUNTS - ref_per) * slope_ber;
        }
        // end
...
INT32 nim_s3281_dvbc_set_perf_level(struct nim_device *dev, UINT32 level)
{
    ...
    nim_s3281_dvbc_write(dev, NIM_S3202_EQ2, &data[2], 1);
    nim_s3281_dvbc_write(dev, NIM_S3202_EQ17, &data[0], 2);

    return SUCCESS;
}


static void nim_s3281_dvbc_set_rs(struct nim_device *dev,UINT32 sym)    //Unit:KBaud
{
    UINT8 data = 0;
    UINT32 sym_cal = 0;
    ...
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM2, &data, 1);

    //CR3A
    nim_s3281_dvbc_read(dev, NIM_S3202_FSM3, &data, 1);
    data &= 0xe0;
    data |= (UINT8)((sym_cal >> 8) & 0x1f) ;
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM3, &data, 1);
}

void nim_s3281_dvbc_set_search_freq(struct nim_device *dev,UINT16 freq_range)    //+/-KHz
{
    UINT8 data = 0;
    UINT16 fr_range_cal = 0;

    if(0xFFFF == freq_range)
        return;
    //kent
    ...
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM13, &data, 1);
    //CR45.
    nim_s3281_dvbc_read(dev, NIM_S3202_FSM14, &data, 1);
    data = (UINT8)((data & 0xf0) | ((fr_range_cal >> 8) & 0x0f));
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM14, &data, 1);
...
INT32 nim_s3281_fsm_reset(struct nim_device *dev)
{
    ...
    S3281_PRINTF(NIM_LOG_DBG, " QAM FSM RESET!!!");
    return SUCCESS;
}

void nim_s3281_dvbc_set_search_rs(struct nim_device *dev,UINT16 rs_min, UINT16 rs_max)    //KBaud
{
    UINT8 data = 0;
    UINT16 rs_min_cal = 0;
    UINT16 rs_max_cal = 0;
    ...
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM9, &data, 1);
    //CR41
    nim_s3281_dvbc_read(dev, NIM_S3202_FSM10, &data, 1);
    data = (UINT8)((data & 0xe0) | ((rs_min_cal >> 8) & 0x1f));
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM10, &data, 1 );

    //CR3E
    data = (UINT8)(rs_max_cal & 0xff);
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM7, &data, 1);
    //CR3F
    nim_s3281_dvbc_read(dev, NIM_S3202_FSM8, &data, 1);
    data = (UINT8)((data & 0xe0) | ((rs_max_cal >> 8) & 0x1f));
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM8, &data, 1);
}

void nim_s3281_dvbc_set_delfreq(struct nim_device * dev,INT16 delfreq)    //+/-KHz
{
    UINT8 data = 0;
    INT16 delfrq_cal = 0;
    ...
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM4, &data, 1);
    //CR3C
    nim_s3281_dvbc_read(dev, NIM_S3202_FSM5, &data, 1);
    data = (UINT8)((data & 0xe0) | ((delfrq_cal >> 8) & 0x1f));
    nim_s3281_dvbc_write(dev, NIM_S3202_FSM5, &data, 1);
}
...
INT32 nim_s3281_dvbc_set_tuner(struct nim_device *dev,NIM_CHANNEL_CHANGE_T *pstchl_change)
{
    ...
    dev_priv = (struct nim_s3281_private *)dev->priv;
    dev_priv->nim_tuner_control(dev_priv->tuner_index, freq, sym) ;
