...
static INT32 nim_s3281_dvbc_open(struct nim_device *dev)
{
    ...
    {
        S3281_PRINTF(NIM_LOG_ERR,"nim_s3281_dvbc_open: Create mutex failed!\n");
    }
...
static INT32 nim_s3281_dvbc_close(struct nim_device *dev)
{
    ...
    {
        S3281_PRINTF(NIM_LOG_DBG,"nim_s3281_dvbc_close: Delete mutex failed!\n");
        ret = ERR_FAILUE;
    }
    ...
    {
        S3281_PRINTF(NIM_LOG_ERR,"nim_s3281_dvbc_close: Delete minitoring task failed!\n");
        ret = ERR_FAILUE;
    }
...
static INT32 nim_s3281_dvbc_ioctl(struct nim_device *dev,INT32 cmd,UINT32 param)
{
    ...
    case NIM_DRIVER_SET_RESET_CALLBACK:
        S3281_PRINTF(NIM_LOG_DBG,"[%s] line = %d,wakeup!\n", __FUNCTION__, __LINE__);
    ...
    case NIM_TURNER_SET_STANDBY:
        S3281_PRINTF(NIM_LOG_DBG,"[%s] line = %d,standby!\n", __FUNCTION__, __LINE__);
        /*struct nim_s3281_private *priv;
        priv = (struct nim_s3281_private *)dev->priv;
        if (priv->nim_Tuner_Control != NULL)
         {
...
INT32 nim_s3281_dvbc_attach(struct QAM_TUNER_CONFIG_API *ptrqam_tuner)
{
    ...
    if (dev == NULL)
    {
        S3281_PRINTF(NIM_LOG_ERR,"Error: Alloc nim device error!\n");
        return ERR_NO_MEM;
    }
    ...
    {
        dev_free(dev);
        S3281_PRINTF(NIM_LOG_ERR,"Alloc nim device prive memory error!/n");
        return ERR_NO_MEM;
    }
    ...
    priv->tuner_id = ptrqam_tuner->tuner_config_ext.tuner_id;
   ...
   // dev->channel_search = nim_s3281_dvbc_channel_search;
    ...
    {
        S3281_PRINTF(NIM_LOG_DBG,"Error: Register nim device error!\n");
        comm_free(priv);
        dev_free(dev);
        return ERR_NO_DEV;
    }
