...
INT32 nim_s3503_cap_fft_find_channel(struct nim_device *dev, UINT32 *tune_freq)
{
    ...
    S3503_PRINTF(NIM_LOG_DBG,"tune_freq = %d\n", *tune_freq);
    ...
    if (1 == success)
    {
        for (i = 0; i < ch_number; i++)
        {
            ...
            S3503_PRINTF(NIM_LOG_DBG,"    m_Freq[%d] is %d\n", priv->ul_status.m_crnum,
    ...
    {
        //S3503_PRINTF(NIM_LOG_DBG,"err\n");
        S3503_PRINTF(NIM_LOG_DBG,"    Base band width is %d, ch_number is%d\n", if_freq / 1024, ch_number);
    }
...
void nim_s3503_cap_fft_result_read(struct nim_device *dev)
{
    ...
    comm_delay(1000);
    comm_memset(priv->ul_status.adcdata_malloc_addr, 0, BYPASS_BUF_SIZE * 2);//0x20000
    ...
    for (m = 0; m < 1000; m++)
    {
        ...
        if (0x01 == (read_data & 0x01))
        {
            ...
            S3503_PRINTF(NIM_LOG_DBG," ADC DATA  transfer finish\n");
            break;
        }
    }
...
#endif
    //S3503_PRINTF(NIM_LOG_DBG,"  leave Fuction nim_s3503_cap_fft_result_read \n");
}
...
INT32 nim_s3503_cap_start(struct nim_device *dev, UINT32 startfreq, UINT32 sym, UINT32 *cap_buffer)
{
    ...
    S3503_PRINTF(NIM_LOG_DBG,"Enter nim_s3503_cap_start : Tuner_IF=%d\n", startfreq);
    ...
    if (result == SUCCESS)
    {
        S3503_PRINTF(NIM_LOG_DBG,"nim_tuner_control OK\n");
    }
    else
    {
        S3503_PRINTF(NIM_LOG_DBG,"nim_tuner_control failed\n");
    }
    ...
    for( i = 0; i < 1000; i++ )
    {
        ...
        if (lock)
        {
            S3503_PRINTF(NIM_LOG_DBG,"Tuner lock after %d times delay\n", i + 1);
            break;
        }
    }
    if(lock == 0)
    {
        S3503_PRINTF(NIM_LOG_DBG,"Tuner can not lock\n");
    }
...
INT32 nim_s3503_cap(struct nim_device *dev, UINT32 startfreq, UINT32 *cap_buffer, UINT32 sym)
{
	...
	priv = (struct nim_s3501_private *) dev->priv;
    S3503_PRINTF(NIM_LOG_DBG,"Enter nim_s3503_cap : Tuner_IF=%d\n", startfreq);
...
INT32 nim_s3503_adc2mem_entity(struct nim_device *dev,  UINT8 *cap_buffer, UINT32 dram_len)
{
    ...
    while(1)
    {
        if(data & 0x01)
        {
            S3503_PRINTF(NIM_LOG_DBG,"Reset ADCDMA done\n");
            break;
        }
        nim_reg_read(dev, 0x100, &data, 1);
    }
    ...
    S3503_PRINTF(NIM_LOG_DBG,"Enter function %s with DRAM base address: 0x%08x\n", __FUNCTION__, dram_base);
    ...
    S3503_PRINTF(NIM_LOG_DBG,"\t Configure DRAM length: 0x%08x.\n", dram_len << 15);
    ...
    if (is_test_mode)
    {
        S3503_PRINTF(NIM_LOG_DBG,"\t Configure test pattern mode and test mode.\n");
    ...
    //Start ADC DMA
    S3503_PRINTF(NIM_LOG_DBG,"\t Start ADC DMA.\n");
    ...
    //Wait ADC DMA finish.
    S3503_PRINTF(NIM_LOG_DBG,"\t Wait ADC DMA finish.\n");
    while(1)
    {
        if (user_force_stop_adc_dma)
        {
            ...
            data |= 0x40;
            nim_reg_write(dev, 0x100, &data, 1);
            S3503_PRINTF(NIM_LOG_DBG,"ADC_DMA force stopped by user.\n");
            break;
        }
        nim_reg_read(dev, 0x100, &data, 1);
        S3503_PRINTF(NIM_LOG_DBG,"\t Waiting: ... CR 0x100: 0x%02x.\n", data);

        if (data & 0x10)
        {
            S3503_PRINTF(NIM_LOG_DBG,"ADC_DMA overflowed.\n");
            break;
        }

        if (data & 0x08)
        {
            S3503_PRINTF(NIM_LOG_DBG,"ADC_DMA finished.\n");
            break;
        }
        comm_sleep(50);
    }

    S3503_PRINTF(NIM_LOG_DBG,"%s Exit.\n", __FUNCTION__);
...
INT32 nim_s3503_set_hbcd_timeout(struct nim_device *dev, UINT8 s_case)
{
    ...
    case NIM_OPTR_HW_OPEN:
        ...
        break;
    default:
        S3503_PRINTF(NIM_LOG_DBG," CMD for nim_s3503_set_hbcd_timeout ERROR!!!");
        break;
    }
    return SUCCESS;
}
...
INT32 nim_s3503_set_acq_workmode(struct nim_device *dev, UINT8 s_case)
{
    ...
    case NIM_OPTR_HW_CLOSE:
        ...
        break;
    default:
        S3503_PRINTF(NIM_LOG_DBG," CMD for nim_s3503_set_acq_workmode ERROR!!!");
        break;
    }
    return SUCCESS;
}
...
INT32 nim_s3503_set_fc_search_range(struct nim_device *dev, UINT8 s_case, UINT32 rs)
{
    ...
    break;
    default:
        S3503_PRINTF(NIM_LOG_DBG," CMD for nim_s3503_set_FC_Search_Range ERROR!!!");
        break;
    }
    return SUCCESS;
}
...
INT32 nim_s3503_set_rs_search_range(struct nim_device *dev, UINT8 s_case, UINT32 rs)
{
    ...
    case NIM_OPTR_CHL_CHANGE:
        ...
        break;
    default:
        S3503_PRINTF(NIM_LOG_DBG," CMD for nim_s3503_set_RS_Search_Range ERROR!!!");
        break;
    }
    return SUCCESS;
}
...
INT32 nim_s3503_wideband_scan(struct nim_device *dev, UINT32 start_freq, UINT32 end_freq)
{
	...
	priv = (struct nim_s3501_private *) dev->priv;
    S3503_PRINTF(NIM_LOG_DBG,"Enter nim_s3503_WidebandScan\n");
    ...
    if(SUCCESS == nim_s3503_wideband_scan_open(dev, start_freq, end_freq, step_freq))
    {
        for (fft_freq = start_freq; fft_freq < end_freq; fft_freq += step_freq)
        {
            ...
            if(CAP_NUM_100 == j)
            {
                ...
                S3503_PRINTF(NIM_LOG_DBG,"nim_s3503_WidebandScan works wrong !\n");
                S3503_PRINTF(NIM_LOG_DBG,"ERR_FAILURE, Leave nim_s3503_WidebandScan!\n");
                return ERR_FAILURE;
            }
            nim_s3501_fft_wideband_scan(dev, fft_freq, adc_sample_freq);
        }
        //comm_free(priv->ul_status.adcdata_malloc_addr);
        S3503_PRINTF(NIM_LOG_DBG,"SUCCESS, Leave nim_s3503_WidebandScan!\n");
        return SUCCESS;
    }
    else
    {
        S3503_PRINTF(NIM_LOG_DBG,"ERR_NO_MEM, Leave nim_s3503_WidebandScan!\n");
        return ERR_NO_MEM;
    }
}
...
INT32 nim_s3503_wideband_scan_open(struct nim_device *dev, UINT32 start_freq, UINT32 end_freq, UINT32 step_freq)
{
    ...
    S3503_PRINTF(NIM_LOG_DBG,"Enter nim_s3503_WidebandScan_open\n");
...
INT32 nim_s3503_channel_search(struct nim_device *dev, UINT32 crnum)
{
    ...
    dev_priv = (struct nim_s3501_private *)dev->priv;
    S3503_PRINTF(NIM_LOG_DBG,"ENTER nim_s3503_channel_search: TP is %d\n", crnum);
...
static INT32 nim_s3503_r04_0x18(struct nim_device *dev,NIM_SEARCH_PARAMS *params)
{
    ...
    {
        S3503_PRINTF(NIM_LOG_DBG,"    path2:lock chanel \n");
        S3503_PRINTF(NIM_LOG_DBG,"            Freq is %d\n", (LNB_LOACL_FREQ - tempfreq));
        params->priv->ul_status.m_cur_freq = tempfreq ;
        S3503_PRINTF(NIM_LOG_DBG,"            rs_input is %d\n", rs_input);
        nim_s3503_reg_get_code_rate(dev, &code_rate);
        S3503_PRINTF(NIM_LOG_DBG,"            code_rate is %d\n", code_rate);
        S3503_PRINTF(NIM_LOG_DBG,"            work_mode is %d\n", work_mode);
        nim_s3503_reg_get_map_type(dev, &map_type);
        S3503_PRINTF(NIM_LOG_DBG,"            map_type is %d\n", map_type);
...
static INT32 nim_s3503_init_search(struct nim_device *dev,NIM_SEARCH_PARAMS *params)
{
    ...
    if (params->priv->nim_tuner_command != NULL){
         if(SUCCESS == params->priv->nim_tuner_command(params->priv->tuner_id, NIM_TUNER_M3031_ID, &channel_freq_err))
		     params->low_sym = 0;   	  					  // always use centre frequency    
         else
		     params->low_sym = params->rs < 6500 ? 1 : 0;      /* Symbol rate is less than 10M, low symbol rate */
    }
    else
          params->low_sym = params->rs < 6500 ? 1 : 0;   		/* Symbol rate is less than 10M, low symbol rate */
    ...
    //S3503_PRINTF(NIM_LOG_DBG,"Tuner lock = %d\n",lock);
    comm_delay(500);
    params->lock = 200;
    nim_s3503_interrupt_mask_clean(dev);
    data = 0xff;
    ...
    nim_s3503_cr_adaptive_configure(dev, params->rs);  // Seen add begin, copy from channel change
    nim_s3503_cr_new_modcod_table_init(dev,params->rs);
...
static INT32 nim_s3503_get_lock_status(struct nim_device *dev,NIM_SEARCH_PARAMS *params)
{
    ...
    if (0 == (intindex & 0x10))
    {
        ...
        {
            S3503_PRINTF(NIM_LOG_DBG,"    path1:lock chanel \n");
            tempfreq = params->freq;
            nim_s3503_reg_get_freq(dev, &tempfreq);
            S3503_PRINTF(NIM_LOG_DBG,"            Freq is %d\n", (LNB_LOACL_FREQ - tempfreq));
            params->priv->ul_status.m_cur_freq = tempfreq ;
            ...
            modcod = modcod >> 1;
            S3503_PRINTF(NIM_LOG_DBG,"            Modcod is %x\n", modcod);
            final_est_freq = tempfreq;
            final_est_sym_rate = rs_input;
#ifdef NIM_S3501_ASCAN_TOOLS
            nim_s3501_ascan_add_tp(ASCAN_ADD_REAL_TP, 0x00, tempfreq, rs_input, tempfreq > 1550 ? 1 : 0);
...
static  INT32 nim_s3503_soft_search(struct nim_device *dev, UINT32 *est_rs, UINT32 *est_freq, INT32 delfreq)
{
    ...
    while (1)
    {
        if (params.lock == 0)
        {
            ...
            if (params.lock == 1)
            {
                S3503_PRINTF(NIM_LOG_DBG,"Tuner lock = %d at timeout = %d\n", params.lock, params.timeout);
            }
        }
        params.timeout ++ ;
        ...
        S3503_PRINTF(NIM_LOG_DBG,"        Lock status is 0x%x\n", params.intindex);
...
void nim_s3503_interrupt_clear(struct nim_device *dev)
{
    ...
    //S3503_PRINTF(NIM_LOG_DBG,"    enter nim_s3503_interrupt_clear\n");
}
...
void nim_s3503_set_rs(struct nim_device *dev, UINT32 rs)
{
    ...
    nim_s3503_get_dsp_clk(dev, &sample_rate);
    //S3503_PRINTF(NIM_LOG_DBG,"\t\t dsp clock is %d\n", sample_rate);
    S3503_PRINTF(NIM_LOG_DBG,"\t\t set ts %d with dsp clock %d\n", rs, sample_rate / 1000);
    temp = nim_s3501_multu64div(rs, 184320, sample_rate); // 184320 == 90000 * 2.048
    if(temp > 0x16800)
    {
        S3503_PRINTF(NIM_LOG_DBG,"\t\t NIM_Error: Symbol rate set error %d\n", temp);
        temp = 0x16800;
    }
    //CR3F
