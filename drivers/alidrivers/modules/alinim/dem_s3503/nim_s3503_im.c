                             ...
                             UINT32 rs, UINT32 *bit_rate)
{
    ...
    if (work_mode != M3501_DVBS2_MODE) // DVBS mode
    {
        ...
        *bit_rate = data;
        S3503_PRINTF(NIM_LOG_DBG,"xxx dvbs bit_rate is %d \n", *bit_rate);
        return SUCCESS;
    }
    else    //DVBS2 mode
    {
        ...
        {
            S3503_PRINTF(NIM_LOG_DBG,"Map type error: %02x \n", map_type);
        }
        ...
        S3503_PRINTF(NIM_LOG_DBG,"Code rate is: %02x \n", code_rate);
        S3503_PRINTF(NIM_LOG_DBG,"Map type is: %02x \n", map_type);

        data += 1; // Add 1M
        *bit_rate = data;
        S3503_PRINTF(NIM_LOG_DBG,"xxx dvbs2 bit_rate is %d \n", *bit_rate);
        return SUCCESS;
    }
}
...
INT32 nim_s3503_get_bypass_buffer(struct nim_device *dev)
{
    ...
    S3503_PRINTF(NIM_LOG_DBG,"ADCdata_malloc_addr=0x%08x\n", priv->ul_status.adcdata_malloc_addr);
    ...
    S3503_PRINTF(NIM_LOG_DBG,"ADCdata=0x%08x\n", priv->ul_status.adcdata);
...
INT32 nim_s3503_get_tune_freq(struct nim_device *dev, INT32 *freq)
{
    ...
    *freq += priv->ul_status.m_step_freq;
    ...
    //S3503_PRINTF(NIM_LOG_DBG,"  tune *freq step is %d\n", *freq );
    return SUCCESS;
}
...
INT32 nim_s3503_reg_get_code_rate(struct nim_device *dev, UINT8 *code_rate)
{
    ...
    //  S3503_PRINTF(NIM_LOG_DBG,"Enter Fuction nim_s3503_reg_get_code_rate \n");
...
INT32 nim_s3503_hw_check(struct nim_device *dev)
{
    ...
    S3503_PRINTF(NIM_LOG_DBG,"    Enter fuction nim_s3503_hw_check\n");
...
INT32 nim_s3503_adc2mem_cap(struct nim_device *dev, UINT32 startfreq, UINT32 sym, UINT32 *cap_buffer, UINT32 dram_len)
{
    ...
    if(NULL == dram_base_t)
    {
        S3503_PRINTF(NIM_LOG_DBG,"S3503 adc2mem memory allocation error!\n");
        return ERR_FAILUE;
    }
...
INT32 nim_s3503_adc2mem_start(struct nim_device *dev, UINT32 startfreq, UINT32 sym, UINT32 *cap_buffer, UINT32 dram_len)
{
    ...
    dev_priv = (struct nim_s3501_private *)dev->priv;
	//dram_base_t = (UINT8 *) comm_malloc(dram_len );-->kmalloc in device init
    if(NULL == dram_base_t)
    {
        S3503_PRINTF(NIM_LOG_DBG,"S3503 adc2mem memory allocation error!\n");
        return ERR_FAILUE;
...
void nim_s3503_task(UINT32 param1, UINT32 param2)
{
    ...
    S3503_PRINTF(NIM_LOG_DBG,"            Enter nim_s3503_task:\n");

    while (priv->work_alive)
    {
#ifdef CHANNEL_CHANGE_ASYNC

        flag_ptn = nim_flag_read(&priv->flag_lock, NIM_FLAG_CHN_CHG_START, OSAL_TWF_ANDW | OSAL_TWF_CLR, 0);
        ...
        if((flag_ptn & NIM_FLAG_CHN_CHG_START) && (flag_ptn != OSAL_INVALID_ID))
        {
            ...
            S3503_PRINTF(NIM_LOG_DBG,"\t\t Here is the task for S3503 wait channel lock \n");
        }
#endif
        ...
        if(priv->tsk_status.m_lock_flag == NIM_LOCK_STUS_CLEAR)
        {
            ...
            {
                v_cnt_val = 0;
              //  S3503_PRINTF(NIM_LOG_DBG,"\t\t Here is the task for M3501C plug_in_out \n");
...
INT32 nim_s3503_channel_change(struct nim_device *dev, UINT32 freq, UINT32 sym, UINT8 fec)
{
	...
	priv = (struct nim_s3501_private *) dev->priv;	
    //starttime = 0;
    S3503_PRINTF(NIM_LOG_DBG,"    Enter Fuction nim_s3503_channel_change \n");
    S3503_PRINTF(NIM_LOG_DBG,"    freq is %d\n", freq);
    S3503_PRINTF(NIM_LOG_DBG,"    sym is %d\n", sym);
    S3503_PRINTF(NIM_LOG_DBG,"    fec is %d\n", fec);
    ...
    S3503_PRINTF(NIM_LOG_DBG,"    Leave Fuction nim_s3503_channel_change \n");
...
INT32 nim_s3503_autoscan(struct nim_device *dev, NIM_AUTO_SCAN_T *pstauto_scan)
{
    ...
    S3503_PRINTF(NIM_LOG_DBG,"\t Start autoscan \n");
   ...
   (((params.end_t - params.start_t) + 500) / 1000 - (((params.end_t - params.start_t) + 500) / 1000 / 60) * 60));
    S3503_PRINTF(NIM_LOG_DBG,"\tfinish autoscan\n");
