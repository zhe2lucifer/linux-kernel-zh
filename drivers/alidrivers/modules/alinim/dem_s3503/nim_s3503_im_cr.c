...
* Description: MER = 10*log10(E(I^2+Q^2)/E(DI^2+DQ^2)), unit 0.1dB
*
* Arguments:
...
INT32 nim_s3503_get_mer(struct nim_device *dev, UINT32 *mer)
{
    ...
    { // CR lock
        ...
        else
        {
            ...
            {
            	va_mer = va_mer + err_sum - (va_mer/NIM_MER_STA_LEN);
            }
            mer_tmp = va_mer/NIM_MER_AVG_LEN;
        }
        ...
        mer_tmp = 323 - log10times100_l(mer_tmp); // 323 = 10*log10(1681)*10, align to 0.1dB.
    ...
    {
        S3503_PRINTF(NIM_LOG_DBG, "Can not get MER when demod unlock.\n");      
        *mer = 0;
        va_mer = 0;
        va_mer_window = 0;
        return ERR_FAILUE;
...
INT32 nim_s3503_get_snr(struct nim_device *dev, UINT8 *snr)
{
	...
	S3503_PRINTF(NIM_LOG_DBG,"[%s]line=%d, snr=%d\n", __FUNCTION__, __LINE__, *snr);
...
INT32 nim_s3503_get_ber(struct nim_device *dev, UINT32 *rsubc)
{
    ...
    if (0x00 == (0x80 & data))
    {
        //      S3503_PRINTF(NIM_LOG_DBG, "CR78= %x\n", data);
        //CR76
    ...
    *rsubc = 1010;
    ...
    if (0x00 != (0x20 & data))
    {
        ...
        *rsubc = (UINT32) percount;
        S3503_PRINTF(NIM_LOG_DBG,"current PER is  %d\n", percount);
...
INT32 nim_s3503_get_new_per(struct nim_device *dev, UINT32 *per)
{
    ...
    *per = myper;
    //  S3503_PRINTF(NIM_LOG_DBG,"!!!!!!!! myPER cost %d time, per = %d\n",t_count,myper);
    return SUCCESS;
}

/*****************************************************************************
* static INT32 nim_s3503_get_demod_gain(struct nim_device *dev, UINT8 *agc)
*
...
* Return Value: INT32
*****************************************************************************/
INT32 nim_s3503_get_demod_gain(struct nim_device *dev, UINT8 *agc)
{	
	struct nim_s3501_private *priv = NULL;
	if (dev == NULL)
    {
	...
	S3503_PRINTF(NIM_LOG_DBG,"demod get agc value %d\n",*agc);
	return SUCCESS;
}

/*****************************************************************************
* static INT32 nim_s3503_get_tuner_gain(struct nim_device *dev,UINT8 agc, INT32 *agc_tuner)
*
...
static INT32 nim_s3503_get_tuner_gain(struct nim_device *dev,UINT8 agc, INT32 *agc_tuner)
{
	...
	//S3503_PRINTF(NIM_LOG_DBG, "agc_tmp value %d\n",agc);
	...
	{
		if(SUCCESS == priv->nim_tuner_command(priv->tuner_id, NIM_TUNER_M3031_ID, &channel_freq_err))
		{	
			*agc_tuner = priv->nim_tuner_command(priv->tuner_id, NIM_TUNER_GET_M3031_GAIN, &channel_freq_err);
		}
	}
	
	//S3503_PRINTF(NIM_LOG_DBG, "demod get tuner gain value %d\n",*agc_tuner);
	return SUCCESS;
}

/*****************************************************************************
* static INT32 nim_s3503_get_dBm_level(struct nim_device *dev,UINT8 agc_demod, INT32 agc_tuner,INT8 *level_db)
*
...
* Return Value: INT32
*****************************************************************************/
INT32 nim_s3503_get_dbm_level(struct nim_device *dev,INT32 *level_db)
{
	INT32 channel_freq_err = 0;
	INT32 level_db_tmp ;
	...
	nim_s3503_get_demod_gain(dev,&agc_demod);	
	S3503_PRINTF(NIM_LOG_DBG,"[%s]line=%d, agc_demod = %d\n", __FUNCTION__, __LINE__, agc_demod);
	if (priv->nim_tuner_command != NULL)
	{
		if(SUCCESS == priv->nim_tuner_command(priv->tuner_id, NIM_TUNER_M3031_ID, &channel_freq_err))
		{
			nim_s3503_get_tuner_gain(dev, agc_demod, &agc_tuner);		
			
			if(agc_tuner<9200)
			{//linearity is 300, but with different reference, test under 5M symbol rate		   
				if((agc_tuner>-5600) && (agc_tuner<-1300))
				{
					level_db_tmp = -3-(5505+agc_tuner)/300;
				}	
				else if(agc_tuner>-1200)
				{
					level_db_tmp = -18-(1134+agc_tuner)/300;
				}
				else
				{
					level_db_tmp = -(6894+agc_tuner)/300;
				}
			}
			else
			{//linearity is 170, but with different reference, test under 5M symbol rate			
				...
				{
					level_db_tmp = -59-(agc_tuner-10032)/170;
				}
				else
				{
					level_db_tmp = -55-(agc_tuner-9255)/170;
				}
			}
			
			if(level_db_tmp>-55)
			{
				level_db_tmp = level_db_tmp - 2;
			}
			if(level_db_tmp>-9)
			{
				level_db_tmp = level_db_tmp - 1;
			}
		}
		else
		{
			...
			//S3503_PRINTF(NIM_LOG_DBG,"agc_demod_tmp = %d\n",agc_demod_tmp);
			...
			{
				level_db_tmp = agc_demod_tmp*2 -176 ;
			}
	...
	else
	{
		...
		S3503_PRINTF(NIM_LOG_DBG,"[%s]line=%d, agc_demod_tmp = %d\n", __FUNCTION__, __LINE__, agc_demod_tmp);
	...
	//S3503_PRINTF(NIM_LOG_DBG,"level_db_tmp = %d\n",level_db_tmp);
	
	*level_db = level_db_tmp ;
	S3503_PRINTF(NIM_LOG_DBG,"level_db = %d\n",*level_db);

}

/*****************************************************************************
* static INT32 nim_s3503_get_AGC_filter(struct nim_device *dev, INT8 *agc)
*
...
static INT32 nim_s3503_get_agc_filter(struct nim_device *dev, INT32 *agc)
{
    INT32 s_sum=0;     
...
{	
	...
	nim_s3503_get_dbm_level(dev,&level_db_tmp);
	//S3503_PRINTF(NIM_LOG_DBG,"-----------------agc gain dBm value %d\n",level_db_tmp);
	
	nim_s3503_get_agc_filter(dev,&level_db_tmp);	
	//S3503_PRINTF(NIM_LOG_DBG,"------dBm value after filter %d\n",level_db_tmp);

	*agc= (UINT8)(level_db_tmp+100);
	S3503_PRINTF(NIM_LOG_DBG,"[%s]line=%d--------send out value agc=%d\n", __FUNCTION__, __LINE__, *agc);

	return SUCCESS;
}

/*****************************************************************************
* static INT32 nim_s3503_get_AGC_dBm(struct nim_device *dev, INT8 *agc_dBm)
*
*  This function will access the NIM to determine the AGC feedback value
*  with unit of dBm
* Arguments:
...
*****************************************************************************/
// get signal intensity
INT32 nim_s3503_get_agc_dbm(struct nim_device *dev, INT8 *agc_dbm)
{	
	//INT32 channel_freq_err = 0;
	INT32 level_db_tmp = 0;
	//UINT8 data = 0;
	...
	nim_s3503_get_agc_filter(dev,&level_db_tmp);	
	S3503_PRINTF(NIM_LOG_DBG,"[%s]line=%d------dBm value after filter %d\n",__FUNCTION__, __LINE__, level_db_tmp);
...
static void nim_s3503_print_map_type(UINT8 work_mode,UINT8 map_type)
{
    if(1 == work_mode)
    {
        ...
        case 0 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            map_type is HBCD\n");
            break;
        }
        case 1 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            map_type is BPSK\n");
            break;
        }
        case 2 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            map_type is QPSK\n");
            break;
        }
        case 3 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            map_type is 8PSK\n");
            break;
        }
        case 4 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            map_type is 16APSK\n");
            break;
        }
        case 5 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            map_type is 32APSK\n");
            break;
        }
        default:
            break;
...
static void nim_s3503_print_code_rate(UINT8 work_mode,UINT8 code_rate)
{
    if(0 == work_mode)
    {
        ...
        case 0 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 1/2\n");
            break;
        }
        case 1 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 2/3\n");
            break;
        }
        case 2 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 3/4\n");
            break;
        }
        case 3 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 5/6\n");
            break;
        }
        case 4 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate wrong\n");
            break;
        }
        case 5 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 7/8\n");
            break;
        }
        default:
            break;
    ...
    else if(1 == work_mode)
    {
        ...
        case 0 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 1/4\n");
            break;
        }
        case 1 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 1/3\n");
            break;
        }
        case 2 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 2/5\n");
            break;
        }
        case 3 :
        {
        ...
        case 4 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 3/5\n");
            break;
        }
        case 5 :
        {
        ...
        case 7 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 4/5\n");
            break;
        }
        case 8 :
        {
        ...
        case 9 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 8/9\n");
            break;
        }
        case 10 :
        {
            S3503_PRINTF(NIM_LOG_DBG,"            code_rate is 9/10\n");
            break;
        }
        default:
             break;
...
static void nim_s3503_print_work_mode(UINT8 work_mode)
{
    ...
    case 0 :
    {
        S3503_PRINTF(NIM_LOG_DBG,"            work_mode is DVB-S \n");
        break;
    }
    case 1 :
    {
        S3503_PRINTF(NIM_LOG_DBG,"            work_mode is DVB-S2 \n");
        break;
    }
    case 2 :
    {
        S3503_PRINTF(NIM_LOG_DBG,"            work_mode is DVB-H8PSK \n");
        break;
    }
   default:
       break;
...
static void nim_s3503_print_roll_off(UINT8 roll_off)
{
    ...
    case 0 :
    {
        S3503_PRINTF(NIM_LOG_DBG,"            roll_off is 0.35\n");
        break;
    }
    case 1 :
    {
        S3503_PRINTF(NIM_LOG_DBG,"            roll_off is 0.25\n");
        break;
    }
    case 2 :
    {
        S3503_PRINTF(NIM_LOG_DBG,"            roll_off is 0.20\n");
        break;
    }
    case 3 :
    {
        S3503_PRINTF(NIM_LOG_DBG,"            roll_off is wrong\n");
        break;
    }
   default:
       break;
...
INT32 nim_s3503_waiting_channel_lock(struct nim_device *dev, UINT32 freq, UINT32 sym)
{
    ...
    while (priv->ul_status.s3501_chanscan_stop_flag == 0)
    {
        ...
        if (locktimes < timeout)                     // hardware timeout
        {
            ...
            S3503_PRINTF(NIM_LOG_DBG,"    timeout \n");
        ...
        if ((intdata & 0x3f) == data) // if lock, get and print TP information
        {
            ...
            S3503_PRINTF(NIM_LOG_DBG,"            Freq is %d\n", (LNB_LOACL_FREQ - tempfreq));

            nim_s3503_reg_get_symbol_rate(dev, &rs);
            S3503_PRINTF(NIM_LOG_DBG,"            rs is %d\n", rs);
            ...
            if(1 == work_mode)
            {
                ...
                if(modcod & 0x01)
                {
                   S3503_PRINTF(NIM_LOG_DBG,"            Pilot on \n");
                }
                else
                {
                    S3503_PRINTF(NIM_LOG_DBG,"            Pilot off \n");
                }
                modcod = modcod >> 2;
                S3503_PRINTF(NIM_LOG_DBG,"            Modcod is %d\n", modcod);
            }

            nim_s3503_reg_get_iqswap_flag(dev, &iqswap_flag);
            S3503_PRINTF(NIM_LOG_DBG,"            iqswap_flag is %d\n", iqswap_flag);
            ...
            {
                S3503_PRINTF(NIM_LOG_DBG,"            Demod Error: wrong map_type is %d\n", map_type);
            }
            else
            {
                S3503_PRINTF(NIM_LOG_DBG,"        lock chanel \n");
...
INT32 nim_s3503_debug_intask(struct nim_device *dev)
{
  ...
  //  S3503_PRINTF(NIM_LOG_DBG," Enter function nim_s3503_debug_intask \n");
#ifdef HW_ADPT_CR
...
INT32 nim_s3503_check_ber(struct nim_device *dev, UINT32 *rsubc)
{
    ...
    if(0x3f != (data & 0x3f))
    {
        ...
        va_ber_window = 0;
        *rsubc = 0xffff;
        S3503_PRINTF(NIM_LOG_DBG,"----NIM_WARNING: FEC unlock, can not get BER\n");
        return SUCCESS;
    }
    else
    {
...
INT32 nim_s3503_get_phase_error(struct nim_device *dev, INT32 *phase_error)
{
    ...
    if ((data & 0x04) == 0)
    {
        ...
        *phase_error = rdata;       // phase error is signed!!!
        ...
        S3503_PRINTF(NIM_LOG_DBG,"phase error is %d\n", (*phase_error));
...
INT32 nim_s3503_reg_get_modcod(struct nim_device *dev, UINT8 *modcod)
{
    ...
    //  S3503_PRINTF(NIM_LOG_DBG,"Enter Fuction nim_s3503_reg_get_modcode \n");
