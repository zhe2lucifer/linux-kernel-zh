...
tmErrorCode_t demod_tda10025_read( tmUnitSelect_t tunit, UInt32 addrsize, UInt8* paddr, UInt32 readlen, UInt8* pdata )
{
	...
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,error,back!\n", __FUNCTION__,__LINE__);
...
#if 0
	TDA10025_PRINTF(NIM_LOG_DBG,"[%s] line=%d,index=%d,i2c_type=%d,i2c_addr=0x%x!\n", __FUNCTION__,__LINE__,
		    index,
	...
	if ( errcode != SUCCESS )
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,error,back,i2c_type=%d,i2c_addr=0x%x!\n", __FUNCTION__,__LINE__,
			ali_tda10025_nim_priv[index]->ext_dem_config.i2c_type_id,
			ali_tda10025_nim_priv[index]->ext_dem_config.i2c_base_addr);
		return TM_ERR_READ;
	}
...
tmErrorCode_t demod_tda10025_write( tmUnitSelect_t tunit, UInt32 addrsize, UInt8* paddr, UInt32 writelen, UInt8* pdata )
{
	...
	if ( errcode != SUCCESS )
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,error,back,i2c_type=%d,i2c_add=0x%x!\n", __FUNCTION__,__LINE__,
			                       ali_tda10025_nim_priv[index]->ext_dem_config.i2c_type_id,
			                       ali_tda10025_nim_priv[index]->ext_dem_config.i2c_base_addr);
		return TM_ERR_WRITE;
	}
...
INT32 nim_tda10025_get_snr(struct nim_device *dev, UINT8 *snr )
{
	...
	nim_tda10025_get_snr_db(dev, &snr_db_value);

	//the real formula is (snr_db_value-230)/10*5.3
	*snr = (snr_db_value - 230)*53/100;
	...
	TDA10025_PRINTF(NIM_LOG_DBG, "*snr=%d\n", *snr);
	return SUCCESS;
}

INT32 nim_tda10025_get_snr_db(struct nim_device *dev, UINT16 *snr_db)
{
	tmErrorCode_t		errcode = TM_OK;
	tmFrontEndFracNb32_t quality;
	struct nim_tda10025_private *priv = dev->priv;	

	errcode = tmbslTDA10025_GetChannelEsno(priv->dev_idx, &quality);
	if ( errcode != TM_OK )
	{
		*snr_db = 0;
		return ERR_DEV_ERROR;
	...
	*snr_db = (UINT16)(quality.lInteger);
	
	TDA10025_PRINTF(NIM_LOG_DBG, "*snr_db=%d\n", *snr_db);
...
INT32 nim_tda10025_get_ber(struct nim_device *dev, UINT32 *err_count)
{
	...
	*err_count = stfrac.lInteger/stfrac.uDivider;
	
	return SUCCESS;
}

INT32 nim_tda10025_get_per(struct nim_device *dev, UINT32 *err_count)
{
	tmErrorCode_t		    errcode = TM_OK;
	tmFrontEndFracNb32_t 	stfrac;
	UInt32 			        uncors = 0;
...
INT32 nim_tda10025_get_agc(struct nim_device *dev, UINT8 *agc )
{
	...
	nim_tda10025_get_agc_dbm(dev, &rf_level);
	...
	*agc = 100 + rf_level;
	TDA10025_PRINTF(NIM_LOG_DBG, "*agc=%d\n", *agc);
	
	return SUCCESS;
}

//uint 0.1dbm
INT32 nim_tda10025_get_agc_dbm(struct nim_device *dev, INT16 *agc_dbm)
{
	INT32 ret;
	INT32 rf_level;
    struct nim_tda10025_private *priv = dev->priv;
	...
	{		
		ret = priv->nim_tuner_command(priv->tuner_id, NIM_TUNER_GET_RF_POWER_LEVEL, &rf_level);

		if (SUCCESS == ret)
		{
			TDA10025_PRINTF(NIM_LOG_DBG, "rf_level=%d\n", rf_level);

			rf_level = (rf_level - 4900)/10;

			if (rf_level&0x80000000)//rf_level is minus, so we must keep the sign bit when translate from INT32 to INT8
			{
				*agc_dbm = (INT16)(rf_level | 0x8000);
			}
			else
			{
				*agc_dbm = (INT16)(rf_level);
			}

			TDA10025_PRINTF(NIM_LOG_DBG, "*agc_dbm=%d, *agc_dbm=0x%x\n", *agc_dbm, *agc_dbm);			
		}
		else
		{
			*agc_dbm = 0;
...
INT32 nim_tda10025_channel_change(struct nim_device *dev, NIM_CHANNEL_CHANGE_T *pstChl_Change )
{
	...
	TDA10025_PRINTF(NIM_LOG_DBG,"[%s] line =%d, enter,index=%d!\n ", __FUNCTION__, __LINE__,priv->dev_idx);
	
	uif = priv->tuner_config_ext.w_tuner_if_freq*1000;
	TDA10025_PRINTF(NIM_LOG_DBG,"[%s] line=%d,freq: %d,modulation=%d,sym=%d\n", __FUNCTION__,__LINE__,
	...
	if (priv->nim_tuner_control != NULL )
	{
		...
		{
			TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,nim_tuner_control error,back!\n", __FUNCTION__,__LINE__);
			return ERR_FAILED;
		}
	}
	...
	if(TM_OK != errcode)
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,tmbslTDA10025_SetFECMode error,back!\n", __FUNCTION__,__LINE__);
		return ERR_FAILUE;
	}	
	...
	if(TM_OK != errcode)
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,tmbslTDA10025_SetIF error,back!\n", __FUNCTION__,__LINE__);
		return ERR_FAILUE;
	}	
	...
	if(TM_OK != errcode)
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,tmbslTDA10025_SetSI error,back!\n", __FUNCTION__,__LINE__);
		return ERR_FAILUE;
	}	
	...
	if(TM_OK != errcode)
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,tmbslTDA10025_SetMod error,back!\n", __FUNCTION__,__LINE__);
		return ERR_FAILUE;
	}	
	...
	if(TM_OK != errcode)
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,tmbslTDA10025_SetSR error,back!\n", __FUNCTION__,__LINE__);
		return ERR_FAILUE;
	}	
	...
	if(TM_OK != errcode)
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,tmbslTDA10025_StartLock error,back!\n", __FUNCTION__,__LINE__);
		return ERR_FAILUE;
	}	
	...
	do
	{
       ...
       // TDA10025_PRINTF(NIM_LOG_DBG,"[%s] line=%d,loopcount=%d!\n", __FUNCTION__,__LINE__,loopcount);
   ...
   // MAGIC_CONFIG_20
    ...
    {
    	TDA10025_PRINTF(NIM_LOG_DBG,"[%s] line=%d,success,loopcount=%d!\n", __FUNCTION__,__LINE__,loopcount);
    	return SUCCESS;
    }
    else
    {
    	TDA10025_PRINTF(NIM_LOG_ERR,"[%s] line=%d,fail,loop=%d,lock=%d!\n", __FUNCTION__,__LINE__,loopcount,efrontendlock);
    	return ERR_FAILED;
    }
}
...
INT32 nim_tda10025_enter_standby(struct nim_device *dev )
{
	...
	if ( TM_OK != errcode )
	{
		TDA10025_PRINTF(NIM_LOG_ERR,"Set TDA10025 Standby Failed!\n");
		return ERR_DEV_ERROR;
	}
	if (priv->nim_tuner_close != NULL )
	{
		...
		{
			TDA10025_PRINTF(NIM_LOG_ERR,"[%s %d]ERR_FAILED\n ", __FUNCTION__, __LINE__);
			return ERR_FAILED;
		}
	}
...
INT32 nim_tda10025_dev_init(struct nim_device *dev)
{
    ...
    if(!g_nim_status[priv->dev_idx])
    {
		...
		if(TM_OK != errcode)
		{
			TDA10025_PRINTF(NIM_LOG_ERR,"[%s %d, errcode =%d \n ", __FUNCTION__, __LINE__, errcode);
			return ERR_FAILUE;
		}
		...
		errcode = tmbslTDA10025_SetBERWindow(priv->dev_idx, TDA10025_BERWindow1E6);
		if(TM_OK != errcode)
		{
			TDA10025_PRINTF(NIM_LOG_ERR,"[%s %d], errcode =%d \n ", __FUNCTION__, __LINE__, errcode);
			return ERR_FAILUE;
