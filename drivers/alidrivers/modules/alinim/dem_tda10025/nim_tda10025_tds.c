...
INT32 nim_tda10025_attach(struct QAM_TUNER_CONFIG_API * ptrQAM_Tuner)
{
	...
	if (dev == NULL)
	{
		TDA10025_PRINTF(NIM_LOG_DBG,"Error: Alloc nim device error!\n");
		return ERR_NO_MEM;
	}
	...
	{
		dev_free(dev);
		TDA10025_PRINTF(NIM_LOG_DBG,"Alloc nim device prive memory error!\n");
		return ERR_NO_MEM;
	}
	...
	{
		TDA10025_PRINTF(NIM_LOG_DBG,"Error: Register nim device error!\n");
		comm_free(priv);
		dev_free(dev);
		return ERR_NO_DEV;
	}
...
static INT32 nim_tda10025_open(struct nim_device *dev)
{
	...
	{
		TDA10025_PRINTF(NIM_LOG_DBG,"nim_tda10025_open: Create mutex failed!\n");
	}
...
static INT32 nim_tda10025_close(struct nim_device *dev)
{
	...
	{
		TDA10025_PRINTF(NIM_LOG_DBG,"nim_tda10025_close: Delete mutex failed!\n");
		ret = ERR_FAILUE;
	}
