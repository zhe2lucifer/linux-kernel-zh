...
*****************************************************************************/
#include "tun_m3031.h"
#include "tun_m3031_reg.h"


#if 0
#define NIM_PRINTF   printk
#else
#define NIM_PRINTF(...)
#endif
...
static INT32 remember_rf_signal_level[MAX_TUNER_SUPPORT_NUM] = {0}; //remember rf signal level for detecting signal level large change
static INT32 rf_vrms_signal_thres[3] = {11,16,21}; //the first value must >9 for VRMS=3
...
static int tuner_i2c_write(UINT32 tuner_id, unsigned char reg_start, unsigned char* buff, unsigned char length)
{
	...
	while((rd+15)<length)
	{
		comm_memcpy(&data[1], &buff[rd], 15);
		i2c_result = nim_i2c_write(m3031_ptr->i2c_type_id, m3031_ptr->c_tuner_base_addr, data, 16);
		rd+=15;
		data[0] += 15;
	...
	comm_memcpy(&data[1], &buff[rd], length-rd);
	i2c_result = nim_i2c_write(m3031_ptr->i2c_type_id, m3031_ptr->c_tuner_base_addr, data, length-rd+1);
    if(SUCCESS != i2c_result)
		NIM_PRINTF("[%s]line=%d, nim_i2c_write error i2c_addr=0x%x\n", __FUNCTION__,__LINE__,m3031_ptr->c_tuner_base_addr);
...
static int tuner_i2c_read(UINT32 tuner_id, unsigned char reg_start, unsigned char* buff, unsigned char length)
{
	...
	m3031_ptr = &m3031_dev_id[tuner_id];	
	data[0] = reg_start;
	i2c_result = nim_i2c_write_read(m3031_ptr->i2c_type_id, m3031_ptr->c_tuner_base_addr, data, 1, length-rd);
    if(SUCCESS != i2c_result)
		NIM_PRINTF("[%s]line=%d, nim_i2c_write error i2c_addr=0x%x\n", __FUNCTION__,__LINE__,m3031_ptr->c_tuner_base_addr);

	comm_memcpy(&buff[0], &data[0],length);
...
INT32 fast_settle_rf_agc(UINT32 tuner_id, UINT8 slow_speed) //This is NOT a API function
{
	...
	comm_sleep(FAST_RFAGC_WAIT_MS); //wait for RF AGC to settle at FAST_RFAGC_SPEED
	//msleep(FAST_RFAGC_WAIT_MS);
...
INT32 tun_m3031_init(UINT32* tuner_id, struct QPSK_TUNER_CONFIG_EXT * ptr_tuner_config)
{
	...
	comm_memcpy(m3031_ptr, ptr_tuner_config, sizeof(struct QPSK_TUNER_CONFIG_EXT));

	NIM_PRINTF("[%s]Initial the Tuner!! m3031_ptr->c_tuner_base_addr=0x%x,m3031_ptr->i2c_type_id=%d\n",__FUNCTION__, m3031_ptr->c_tuner_base_addr,m3031_ptr->i2c_type_id);
...
//#ifdef FOR_ES_TEST
	...
	comm_sleep(FAST_RFAGC_WAIT_MS); //Wait for RF AGC and FSMs to settle
	//msleep(FAST_RFAGC_WAIT_MS);
	...
	tun_m3031_LT_gain(*tuner_id, LT_GAIN_DISABLE);//disable Loop Through for non-Loop_Through models
	//tun_m3031_LT_gain(*tuner_id, 3);

    return SUCCESS;
}

/*****************************************************************************
* INT32 nim_m3031_control(UINT32 tuner_id,UINT32 channel_freq, UINT32 bb_sym, INT32 *channel_freq_err)
*
...
//is reverse of other tuners (such as AV2018). The deomdulator's driver must be changed to use the 
...
INT32 tun_m3031_control(UINT32 tuner_id,UINT32 channel_freq, UINT32 bb_sym)
{
	UINT8 pll_p;//PLL_P;
	UINT32 p; 
	UINT32 fvco;//Fvco;
	...
	if(NIM_TUNER_SET_STANDBY_CMD == channel_freq){//the standby command
		result = tun_m3031_standby(tuner_id);
		return result;
	}

	NIM_PRINTF("[%s]line=%d,Enter:channel_freq = %d; bb_sym = %d; channel_freq_err = %d\n",__FUNCTION__,__LINE__,channel_freq,bb_sym,g_channel_freq_err);
  ...
  if(!(remember_auto_search_mode[tuner_id] & auto_search_mode)){ //only skip decide RF VRMS when two continuous auto search mode is called
    ...
    else if (channel_freq < 313)  //312.5 is a float value so change to 313 int value
    {
...
/*****************************************************************************
* INT32 nim_m3031_status(UINT32 tuner_id)
*
...
*  Parameter1: tuner_id		:	which tuner
*
* Return Value: INT32		:	M3031 FSM current status
*****************************************************************************/
#define M3031_STATUS_ERROR -1
...
/*****************************************************************************
* INT32 nim_m3031_gain(UINT32 tuner_id, int AGC_level)
*
...
*****************************************************************************/
INT32 tun_m3031_gain(UINT32 tuner_id, UINT32 agc_level)
{
	INT32 total_gain;
...
/*****************************************************************************
* INT32 nim_m3031_LT_gain(UINT32 tuner_id,  UINT32 LT_gain)
*
...
INT32 tun_m3031_LT_gain(UINT32 tuner_id, INT32 lt_gain){
   //UINT8 rfagc_gain_c_index;  //clean warning
	UINT8 lt_gain_apply;//LT_gain_apply;
	UINT8 byte;
	int result;
	struct QPSK_TUNER_CONFIG_EXT * m3031_ptr;
	//int rf_signal_level;  //clean warning
...
INT32 tun_m3031_command(UINT32 tuner_id, INT32 cmd, INT32 *param)
{
	...
	switch(cmd)
		{
		    case NIM_TUNER_M3031_ID://m3031 tuner

				break; 
		    case NIM_TUNER_SET_M3031_FREQ_ERR://set	m3031b channel_freq_err

				g_channel_freq_err = *param;
				break; 
			case NIM_TUNER_GET_M3031_FREQ_ERR://get m3031b channel_freq_err
              
			    *param = g_channel_freq_err;
				break;

			case NIM_TUNER_GET_M3031_GAIN://get m3031b channel_freq_err
               
				return tun_m3031_gain(tuner_id, *param);		
			case NIM_TUNER_GET_M3031_GAIN_FLAG://get c3031b LNA status 
