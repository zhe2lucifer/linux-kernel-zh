...
INT32 tun_mxl603_init(UINT32 *tuner_id, struct COFDM_TUNER_CONFIG_EXT * ptrtuner_config)
{
    ...
    {
		MXL603_PRINTF("[%s]line=%d,tuner_cnt = %ld,error,back!\n",__FUNCTION__,__LINE__,tuner_cnt);
		return ERR_FAILUE;
    }	
...
INT32 tun_mxl603_init_DVBC(UINT32 *tuner_id, struct QAM_TUNER_CONFIG_EXT * ptrtuner_config)
{
    ...
    MXL603_PRINTF("[%s]line=%d,enter,*tuner_id=%d,reopen=%d!\n",__FUNCTION__,__LINE__,(unsigned int)*tuner_id,
                                                                (unsigned int)ptrtuner_config->c_tuner_reopen);
...
static INT32 tun_mxl603_init_common(UINT32 *tuner_id,UINT8 tunerid_input)   //, ptrtuner_config
{
	...
	if(!tunerid_input)
	{
	    ...
	    MXL603_PRINTF("[%s]line=%d,here,tuner_cnt=%d!\n",__FUNCTION__,__LINE__,(unsigned int)tuner_cnt);
	    tuner_cnt++;
	}
...
{
	//INT32 result = 0;//clean warning unused variable
	MXL_STATUS status = 0; 
	UINT8 devId = 0;
	INT32 rf_info = 0;		
                         ...
                         UINT8 agc_time_const,UINT8 *data,UINT8 _i2c_cmd)	
{
	...
	else if( freq == NIM_TUNER_SET_LOOPTHROUGH_CMD)
	{
		if(0 == bandwidth )
		{
			MXL603_PRINTF("[%s]line=%d,loopthrough disable!!\n",__FUNCTION__,__LINE__);
			MxLWare603_API_CfgTunerLoopThrough(tuner_id, MXL_DISABLE);//MXL_DISABLE,MXL_ENABLE
		}	
		else if(1 == bandwidth)
		{
			MXL603_PRINTF("[%s]line=%d,loopthrough enable!!\n",__FUNCTION__,__LINE__);
			MxLWare603_API_CfgTunerLoopThrough(tuner_id, MXL_ENABLE);//MXL_DISABLE,MXL_ENABLE			
		}
		else
		{
	...
	switch(bandwidth)
	{
	    ...
	    case 8:
               ...
               break;
	    default:
	       MXL603_PRINTF("don't support bandwidth=%d\n",(unsigned int)bandwidth);  
	       break;
	}
...
INT32 tun_mxl603_control_DVBC(UINT32 tuner_id, UINT32 freq, UINT32 bandwidth, UINT8 agc_time_const, UINT8 _i2c_cmd)
{
	MXL603_PRINTF("[%s]line=%d,enter,tuner_id=%ld!\n",__FUNCTION__,__LINE__,tuner_id);
	return tun_mxl603_control(tuner_id, freq, bandwidth,0,0,0);
}

INT32 tun_mxl603_control_DVBC_X(UINT32 tuner_id, UINT32 freq, UINT32 bandwidth)
{
	MXL603_PRINTF("[%s]line=%d,enter,tuner_id=%ld!\n",__FUNCTION__,__LINE__,tuner_id);
	return tun_mxl603_control(tuner_id,freq,bandwidth,0,0,0);
}
...
{	
    ...
    //MXL603_IF_FREQ_E ifoutfreq = MXL603_IF_3_65MHz; //clean warning unused variable
...
INT32 tun_mxl603_release(void)
{
	MXL603_PRINTF("[%s]line=%d\n",__FUNCTION__,__LINE__);
	if(tuner_cnt)
	{
    	tuner_cnt--;
	}
