...
void tve_write_ttx (UINT8 line_addr, UINT8 Addr, UINT8 Data)
{
    	jump_to_func(NULL, ali_rpc_call, line_addr, (LLD_VP_M36F_MODULE<<24)|(3<<16)|FUNC_TVE_WRITE_TTX, NULL);
}
...
RET_CODE vpo_open(struct vpo_device *dev,struct vp_init_info *pInitInfo)
{
...
RET_CODE vpo_win_mode(struct vpo_device *dev, BYTE bWinMode, struct mpsource_call_back *pMPCallBack,struct pipsource_call_back *pPIPCallBack)
{
...
RET_CODE vpo_aspect_mode(struct vpo_device *dev, enum tvmode e_tvaspect, enum display_mode e169DisplayMode)
{
...
RET_CODE vpo_tvsys(struct vpo_device *dev, enum tvsystem e_tvsys)
{
...
RET_CODE vpo_tvsys_ex(struct vpo_device *dev, enum tvsystem e_tvsys, BOOL b_progressive)
{
...
RET_CODE vpo_ioctl(struct vpo_device *dev, UINT32 dwCmd, UINT32 dwParam)
{
	...
	switch(dwCmd)
	{
		...
		case VPO_IO_CGMS_STATUS_GET:	
		case VPO_IO_GET_CGMS_INFO:	
        ...
        case VPO_IO_AFD_CONFIG:
			...
			break;
		case VPO_IO_GET_DAC_INFO: 
			DESC_OUTPUT_STRU_SET_SIZE(common_desc, 0, sizeof(struct vp_dac_state));
			break;
        ...
        case VPO_IO_SET_CUTOFF:
            DESC_STATIC_STRU_SET_SIZE(common_desc, 0, sizeof(struct ali_vpo_cutoff_pars));
            break;
        case VPO_IO_GET_VBI_TTX_ISR:
            DESC_OUTPUT_STRU_SET_SIZE(common_desc, 0, sizeof(struct ali_vbi_ttx_pars));
            break;
        case VPO_IO_WRITE_TTX_PACKET:
            DESC_STATIC_STRU_SET_SIZE(common_desc, 0, sizeof(struct vpo_io_ttx_packet));
            break;
        ...
        case VPO_CLOSE_DEO:
        case VPO_IO_EXCHANGE_VIDEO_LAYER:
...
RET_CODE vpo_set_progres_interl(struct vpo_device *dev, BOOL b_progressive)
{
...
RET_CODE tvenc_set_tvsys(struct tve_device *dev,enum TVSystem e_tvsys)
{
...
RET_CODE tvenc_set_tvsys_ex(struct tve_device *dev,enum TVSystem e_tvsys, BOOL b_progressive)
{
...
RET_CODE tvenc_register_dac(struct tve_device *dev,enum dac_type e_dac_type, struct vp_dac_info *pInfo)
{
...
RET_CODE tvenc_unregister_dac(struct tve_device *dev,enum dac_type e_dac_type)
{
