...
static int g_mem_alloced = 0;
static struct MY_PDA g_my_pda;
	...
	IN	PRTMP_ADAPTER	pAd)
{
    ...
    if(1 == g_mem_alloced)
    {
        printk("don't free mem, reset it \n");
...
//mzhu
    if(1 == g_mem_alloced)
    {
		for(acidx=0; acidx<4; acidx++)
        {
            memcpy(&(pAd->TxContext[acidx]), &(g_my_pda.TxContext[acidx]),sizeof(HT_TX_CONTEXT));
        }
        memcpy(&(pAd->MgmtDescRing), &(g_my_pda.MgmtDescRing), sizeof(RTMP_DMABUF));
        memcpy(&(pAd->NullContext), &(g_my_pda.NullContext), sizeof(TX_CONTEXT));
        memcpy(&(pAd->PsPollContext), &(g_my_pda.PsPollContext), sizeof(TX_CONTEXT));
        for (i = 0; i < (RX_RING_SIZE); i++)
        {
            memcpy(&(pAd->RxContext[i]), &(g_my_pda.RxContext[i]), sizeof(RX_CONTEXT));
        }

        memcpy(&(pAd->FragFrame), &(g_my_pda.FragFrame),  sizeof(FRAGMENT_FRAME));
        return 0;
    }
	...
	do
	{
		...
		for(acidx=0; acidx<4; acidx++)
		{
			...
			Status = RTMPAllocUsbBulkBufStruct(pAd, 
												...
												&pHTTXContext->data_dma,
												"HTTxContext");
            memcpy(&(g_my_pda.TxContext[acidx]), &(pAd->TxContext[acidx]), sizeof(HT_TX_CONTEXT));
        ...
        memcpy(&(g_my_pda.MgmtDescRing), &(pAd->MgmtDescRing), sizeof(RTMP_DMABUF));
        ...
        memcpy(&(g_my_pda.NullContext), &(pAd->NullContext), sizeof(TX_CONTEXT));
		if (Status != NDIS_STATUS_SUCCESS)
			goto err;
        ...
        memcpy(&(g_my_pda.PsPollContext), &(pAd->PsPollContext), sizeof(TX_CONTEXT));
		if (Status != NDIS_STATUS_SUCCESS)
			goto err;
		...
		for (i = 0; i < (RX_RING_SIZE); i++)
		{
			...
			Status = RTMPAllocUsbBulkBufStruct(pAd, 
												...
												&pRxContext->data_dma, 
												"RxContext");
            memcpy(&(g_my_pda.RxContext[i]), &(pAd->RxContext[i]), sizeof(RX_CONTEXT));
			if (Status != NDIS_STATUS_SUCCESS)
				goto err;
		}
        ...
        memcpy(&(g_my_pda.FragFrame), &(pAd->FragFrame), sizeof(FRAGMENT_FRAME));
