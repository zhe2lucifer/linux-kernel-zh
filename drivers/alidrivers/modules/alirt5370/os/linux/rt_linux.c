...
extern unsigned char* g_macaddr;
...
#if 0
    NdisMoveMemory(net_dev->dev_addr, g_macaddr, 6);
 /*   printk("mzhu set mac addr %02x:%02x:%02x:%02x:%02x:%02x\n",
            net_dev->dev_addr[0],net_dev->dev_addr[1],net_dev->dev_addr[2],  net_dev->dev_addr[3],  
...
int rt_eth_mac_addr(struct net_device *dev, void *p)
{
	...
	memcpy(pAd->CurrentAddress, addr->sa_data, ETH_ALEN);
	DBGPRINT(RT_DEBUG_TRACE, ("1659 mac addr=%02x:%02x:%02x:%02x:%02x:%02x!\n", PRINT_MAC(addr->sa_data)));
	pAd->bUserAdminMAC = TRUE;
	return 0;
}
...
#endif
			pNetDevOps->ndo_set_mac_address 	= rt_eth_mac_addr,
...
/*			pNetDev->wireless_handlers = &rt28xx_ap_iw_handler_def; */
    ...
    //add by mzhu
    //NdisMoveMemory(pNetDev->dev_addr, g_macaddr, 6);
