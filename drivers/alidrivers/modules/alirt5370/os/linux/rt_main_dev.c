...
========================================================================
*/
//add by martin.zhu 20110602
#include <linux/time.h>
struct timer_list g_operstate_timer;
static void check_operstate(unsigned long data)
{
	struct net_device * net_dev = (struct net_device *)data;
//	printk("operstate = %d \n", net_dev->operstate);
	del_timer(&g_operstate_timer);
	if(net_dev->operstate == IF_OPER_DOWN)
	{
...
/*	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_CPU_SUSPEND)) */
	...
	if(!Flag)
	{
		if(pm_usage_cnt == 0)
		{
			...
			if(pUsb_Dev->dev.power.runtime_auto == true)
#else
...
/*	RT_CFG80211_CRDA_REG_RULE_APPLY(pAd); */
	...
	init_timer(&g_operstate_timer);
	g_operstate_timer.data = (unsigned long)net_dev;
    g_operstate_timer.function = check_operstate;
	mod_timer(&g_operstate_timer,  jiffies + 50);
