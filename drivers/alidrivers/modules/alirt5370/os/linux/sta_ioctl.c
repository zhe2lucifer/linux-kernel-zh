...
/* because android will set scan and get scan when interface down */
	...
	if (pIoctlScan->BssNr == 0)
	{
		...
		status = 0;
		goto go_out;
	} else { /* patch for search AP */
		for(i=0; i<pIoctlScan->BssNr; i++) {
			if((pIoctlScan->pBssTable[i].WpaIeLen > MAX_CUSTOM_LEN)
			   || (pIoctlScan->pBssTable[i].RsnIeLen > MAX_CUSTOM_LEN)
			   || (pIoctlScan->pBssTable[i].WpsIeLen > MAX_CUSTOM_LEN))
			{
				pIoctlScan->BssNr=0;
				data->length = 0;
				status = 0;
...
/*				HT_CAP_INFO capInfo = pIoctlScan->pBssTable[i].HtCapability.HtCapInfo; */
		...
		if (pIoctlScan->pBssTable[i].WpaIeLen > 0)
        {
			...
			if(pIoctlScan->pBssTable[i].WpaIeLen > MAX_CUSTOM_LEN) {
				printk(KERN_ERR"WARNING WARNING !!!!!!!!! WpaIeLen %d\n",pIoctlScan->pBssTable[i].WpaIeLen);
			}
        ...
        if (pIoctlScan->pBssTable[i].RsnIeLen > 0)
        {
        	memset(&iwe, 0, sizeof(iwe));
			memset(&custom[0], 0, MAX_CUSTOM_LEN);
			if(pIoctlScan->pBssTable[i].RsnIeLen > MAX_CUSTOM_LEN) {
				printk(KERN_ERR"WARNING WARNING !!!!!!!!!RsnIeLen %d\n",pIoctlScan->pBssTable[i].RsnIeLen);
			}
		...
		if (pIoctlScan->pBssTable[i].WpsIeLen > 0)
        {
        	memset(&iwe, 0, sizeof(iwe));
			memset(&custom[0], 0, MAX_CUSTOM_LEN);
			if(pIoctlScan->pBssTable[i].WpsIeLen > MAX_CUSTOM_LEN) {
				printk(KERN_ERR"WARNING WARNING !!!!!!!!!WpsIeLen %d\n",pIoctlScan->pBssTable[i].WpsIeLen);
			}
