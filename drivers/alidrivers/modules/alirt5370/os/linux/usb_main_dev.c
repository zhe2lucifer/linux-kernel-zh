...
extern int MainVirtualIF_close(IN struct net_device *net_dev);
...
{	
	...
	if(g_connected)
	{
		printk("only support one wifi card \n");
		return -1;
	}
	g_connected = 1;
	...
	struct usb_interface *intf)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("mzhu ===> rt2870_resume()\n"));
...
#define ALI_MAC_ADDR            0xA7F90000
...
{
	printk("rtusb init %s --->\n", RTMP_DRV_NAME);
#if 0 /*support C2000's app requirement,keep MAC address in same between wireless and wire, disable by stephen*/	
