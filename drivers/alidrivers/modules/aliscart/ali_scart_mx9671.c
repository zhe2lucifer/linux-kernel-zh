...
static int ali_scart_probe(struct platform_device * pdev)
{
	int result;
	dev_t devno = 0;

	ret = of_get_major_minor(pdev->dev.of_node,&devno, 
			0, 1, ALI_SCART_DEVICE_NAME);
	if (ret  < 0) {
...
static int ali_scart_remove(struct platform_device * pdev)
{
...
static const struct of_device_id ali_scart_of_match[] = {
       { .compatible = "alitech, scart", },
       {},
};

MODULE_DEVICE_TABLE(of, ali_scart_of_match);

static struct platform_driver ali_scart_platform_driver = {
	.probe   = ali_scart_probe, 
	.remove   = ali_scart_remove,
	.driver   = {
			.owner  = THIS_MODULE,
			.name   = "ali_scart",
			.of_match_table = ali_scart_of_match,
	},
};
module_platform_driver(ali_scart_platform_driver);
