...
static int scr_dio_fake_rw_check(struct ca_scr_session *s,
	struct ca_dio_write_read *pdio)
{
	...
	if (pdio->crypt_mode != CA_ENCRYPT &&
		...
		dev_dbg(s->scr->dev, "invalid crypt_mode[%d]\n",
			pdio->crypt_mode);
		return -EINVAL;
	}
...
static int scr_dio_fake_rw(struct file *file,
	struct ca_dio_write_read *pdio)
{
	int ret;
	...
	ret = scr_dio_fake_rw_check(s, pdio);
	if (ret)
		return ret;
	...
	dev_dbg(s->scr->dev, "input[%p],output[%p],len[%d]\n",
	...
	while (wr_pos < pdio->length) {
		...
		while (n < m) {
			...
			dev_dbg(s->scr->dev, "rd_pos:%d,wr_pos:%d, n:%d, m:%d\n",
				rd_pos, wr_pos, n, m);
		}
	}
			 ...
			 unsigned long args)
{
	...
	case CA_DIO_WRITE_READ:
	{
		...
		ret = ali_scr_umemcpy(&dio, (void __user *)args,
