...
static inline int smc_atr_isr1_count_trigger(struct smc_device *dev, void __iomem *p)
{
    ...
    writel(0x9c41,p+0x32);//changge 39999CLK to 40001CLK 
	t_old_tick = jiffies;
	for ( ; ; )
	{		
		...
		{
			tp->isr1_interrupt_status &= (~SMC_ISR1_RST_NATR);
			smc_debug(KERN_ERR "SMC ATR: SMC_ISR1_RST_NATR bit is set!\n");			
			return -ETIMEDOUT;
		}

		if((jiffies - t_old_tick)*1000/HZ > 24)//40000*(1/3600000) s = 40000*(1/3600) ms = 11.1ms, exit loop for exception 
		{
			smc_debug(KERN_ERR "SMC ATR: timeout!\n", __func__);
			return -ETIMEDOUT;
		}
		
		if (0 == smc_misc_card_exist(dev))
		{
			smc_debug(KERN_ERR "SMC ATR: In %s, No card exist!\n", __func__);
			return -EIO;
		}		
	}    
	return 0;
...
static inline int smc_atr_isr0_process(struct smc_device *dev, void __iomem *p)
{
	...
	smc_debug(KERN_INFO "SMC ATR:Enter %s\n", __FUNCTION__);
    t_wait_atr_tmo = (9600 * 2 * readw(p + REG_ETU0)) / (tp->smc_clock / 1000);
	t_wait_atr_time = jiffies;
	i_rx_cnt = smc_read_rx(tp, p); 
    smc_debug(KERN_ERR "SMC ATR:line=%d, i_rx_cnt = %d!\n", __LINE__, i_rx_cnt);
	for ( ; ; )
	{
		if (0 == smc_misc_card_exist(dev))
		{
			smc_debug(KERN_ERR "SMC ATR:line=%d, card not exit!\n", __LINE__);
			return -EIO;
		}		
		...
		{
			smc_debug(KERN_ERR "SMC ATR:line=%d, tp->isr0_interrupt_status = 0x%x!\n", __LINE__, tp->isr0_interrupt_status);
...
static inline int smc_atr_decode(struct smc_device *dev, void __iomem *p)
{
	...
	if (0 == c || c > ATR_MAX_SIZE)
	{
		...
		{
			smc_debug(KERN_ERR "SMC ATR: line=%d, return!\n", __LINE__);
			return -EINVAL;
		}
	}
...
static inline int smc_atr_extract_interface_bytes(struct smartcard_atr *atr, int TDi, unsigned char *const pn, UINT8 *p_atr_pointer)
{
    UINT8 pointer = 1;
...
static inline int smc_atr_init(struct smartcard_atr *atr)
{
	...
	if (atr->atr_size < 2)
	{
		smc_debug(KERN_ERR "SMC ATR: line=%d, SMART_WRONG_ATR\n", __LINE__);
		return SMART_WRONG_ATR;
	}
    ...
    if (SMART_NO_ERROR != (ret = smc_atr_extract_interface_bytes(atr, TDi, &pn, &pointer)))
    {
    	smc_debug(KERN_ERR "SMC ATR: line=%d, smc_atr_extract_interface_bytes error, ret=%d\n", __LINE__, ret);
        return ret;
    }
...
static inline void smc_atr_handle_F(struct smartcard_private *tp)
{
    ...
    if (p_atr_info->ib[0][ATR_INTERFACE_BYTE_TA].present)
	{
		...
		/*The Fi value of T14 is different with the Fi value of T0/T1 for irdeto*/
        if(14 == tp->T)
        	F = atr_f_table_T14[FI];
		else
			F = atr_f_table[FI];
...
static inline void smc_atr_handle_D(struct smartcard_private *tp)
{
    ...
    if (p_atr_info->ib[0][ATR_INTERFACE_BYTE_TA].present)
	{
		...
		/*The Di value of T14 is different with the Di value of T0/T1 for irdeto*/
        if(14 == tp->T)
        	D = atr_d_table_T14[DI];
		else
			D = atr_d_table[DI];
...
static inline void smc_atr_config_T_0(struct smartcard_private *tp)
{
    ...
    if (p_atr_info->ib[1][ATR_INTERFACE_BYTE_TC].present)//TC2
	{
		WI = p_atr_info->ib[1][ATR_INTERFACE_BYTE_TC].value;
	}
	else
		WI = ATR_DEFAULT_WI;
	...
	tp->first_cwt = tp->cwt = (960 * WI * tp->F) / (tp->smc_clock / 1000) + 3;
}
...
static inline void smc_atr_config_T_1(struct smartcard_private *tp)
{
    ...
    for (i = 1 ; i < p_atr_info->pn ; i++) 
	{
    		...
    		ATR_PROTOCOL_TYPE_T1 == (p_atr_info->ib[i][ATR_INTERFACE_BYTE_TD].value & 0x0F)) 
		{
			...
			smc_t1_set_param(T1, tp->error_check_type, 0);//add on 20150818, set the check type base on ATR
    
		}
	}
    
	smc_debug(KERN_INFO "SMC ATR: In %s T1 special -- ifsc = %ld,  CWI = %d,  tp->cwt = %d, BWI = %d, tp->first_cwt = %d\
                  checksum = %d(3: LRC;2: CRC)\n",
				  __FUNCTION__, T1->ifsc, tp->CWI, tp->cwt, tp->BWI, tp->first_cwt, tp->error_check_type);
}
...
static inline int smc_atr_config_T(struct smc_device *dev,struct smartcard_private *tp)
{
	int ret = 0;
	...
	{
		smc_atr_config_T_0(tp); //Irdeto spec not mention WI ,so we use T0 as defulat
...
static inline int smc_atr_config_parameter(struct smc_device *dev)
{
	UINT8 i;
	struct smartcard_private *tp = (struct smartcard_private *)dev->priv;
    struct smartcard_atr *atr = (struct smartcard_atr *)tp->atr;
...
static inline void smc_atr_set_guart_value(struct smc_device *dev, void __iomem *p)
{
	...
	{
		if (1 == tp->T)
            writeb(11, p + REG_GTR0);//for T=1, if N=255, GT=11etu
		else
			writeb(12, p + REG_GTR0);//for other protocol, if N=255, GT=12etu
	}
	else
        writeb(12 + tp->N, p + REG_GTR0);//N=0~254
}
                                   ...
                                   UINT32 bk_first_cwt, UINT32 bk_cwt)
{
    ...
    if ((ret = smc_misc_set_pps(dev, p, pps0 | (tp->T & 0xf), FI, DI, b_need_reset_etu)) < 0)
	{
		//I think below method hava some problem. If pps fail, use the default value, needn't deactive device. 
    ...
    else
    {
		...
		{
			tp->first_cwt = tp->cwt =
				(960*ATR_DEFAULT_WI*ATR_DEFAULT_F)/(tp->smc_clock/1000);
		}		
		smc_debug(KERN_ERR "SMC ATR: In %s PPS OK, b_need_reset_etu = %d, set etu as %ld\n",
			__func__, *b_need_reset_etu, tp->smc_etu);
        return 0;
    }
}
                                 ...
                                 int *b_need_reset_etu, int *p_DI, int *p_FI, int *protocol_num)
{
	...
	if (b_multi_protocol || b_diff_f_d)
	{
		...
		tp->first_cwt = tp->cwt = (960 * ATR_DEFAULT_WI * ATR_DEFAULT_F) / (tp->smc_clock / 1000) + 1;

		if(0x11 == (pps0&0x11))/* T1_PPS for vmx T1 BC card temp */
		{
			FI = 0xa;     
			*p_FI = FI;
			DI = 0x5;//125k                 
			*p_DI = DI;
			tp->smc_etu = 48;
			smc_debug("[ %s %d ], pps0 = 0x%x, atr_fi = 0x%x ,atr_di = 0x%x\n", 
						__FUNCTION__, __LINE__, pps0, FI, DI);
		}
...
{  
    ...
    if (SMART_WRONG_ATR == smc_atr_init(atr))
    {
		...
		smc_debug(KERN_ERR "SMC ATR: In %s Specific mode!\n", __func__);
		return -EIO;
    }
	else
		atr->atr_rlt = SMC_ATR_OK;

	smc_atr_config_parameter(dev);
