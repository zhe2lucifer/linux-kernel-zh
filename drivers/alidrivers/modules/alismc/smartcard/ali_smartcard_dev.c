...
int smc_dev_class_create(struct platform_device * pdev, dev_t *dev_no)
{
    int ret= 0;
    long result;

	ret = of_get_major_minor(pdev->dev.of_node,&ali_smc_class.dev_no, 
			0, ali_smc_class.dev_count, ali_smc_class.class_name);
	if (ret  < 0) {
...
static inline void smc_dev_config_reset_param(struct smartcard_private *tp, void __iomem *p)
{
    if (1 == tp->reseted)
	{
		...
		/*if (1 == tp->warm_reset_enable)
			tp->warm_reset = 1;*/
		tp->warm_reset = tp->warm_reset_enable;
	}
	else
		tp->warm_reset = 0;
...
{    	    
    ...
    SMC_CARD_INSERTED_STATUS(tp, ret, error);
    ...
    {
    	smc_debug(KERN_ERR "SMC DEV: In %s Smart card atr_cfg error.\n", __func__);
        goto error;
    }
    smc_debug("SMC: In %s Device reset time %ld\n", __func__, jiffies - u32_time);
...
ssize_t smc_dev_read(struct smc_device *dev, char *buf, size_t count)
{
	...
	smc_debug(KERN_INFO "SMC DEV: enter %s,line=%d\n", __FUNCTION__, __LINE__);
	...
	u32_smc_rd_tmo = smc_txrx_data_timeout(tp, count);
    if (u32_smc_rd_tmo < 0)
    {
        ret_size = 0;
        goto out;
	...
	smc_debug(KERN_INFO "SMC DEV: line=%d, in %s, u32_smc_rd_tmo=%d, cost:%d, %d s!\n", __LINE__, __FUNCTION__, u32_smc_rd_tmo, jiffies-u32_rd_tick, (jiffies-u32_rd_tick)/HZ);
