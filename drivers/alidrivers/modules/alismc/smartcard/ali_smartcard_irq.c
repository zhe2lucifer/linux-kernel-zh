...
static void smc_irq_card_status_notification(struct work_struct *work)
{
	...
	ali_kumsgq_sendmsg(p_smc_dev->smc_kumsgq,
                            &smc_notification, 
                                          ...
                                          void __iomem *p_io_base)
{
	...
	/*fix issue 74047  tp->smc_rx_head can not change in IRQ function without lock */
    spin_lock(&dev->smc_spinlock);
