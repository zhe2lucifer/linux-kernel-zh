...
{
	#if 0
	ali_gpio_set_value(SMC_CA1_POWER_ENABLE, 0);	
	ali_gpio_set_value(SMC_CA2_POWER_ENABLE, 0);	
	#endif
}
...
{
	#if 0
	ali_gpio_set_value(SMC_CA1_POWER_ENABLE, 1);	
	ali_gpio_set_value(SMC_CA2_POWER_ENABLE, 1);	 
	#endif
}
...
void smc_misc_set_pin(void)
{
    ...
    ali_gpio_set_value(SMC_STRIP_PIN_BIT, 1);
...
void smc_misc_unset_pin(void)
{
    ...
    ali_gpio_set_value(SMC_STRIP_PIN_BIT, 0);
...
/* active */
void smc_misc_dev_active(struct smc_device *dev)
{	
    struct smartcard_private *tp = NULL;     
    void __iomem *p = NULL;    
    int i;		
	...
	{	
		if (tp->internal_ctrl_vpp)
            ...
            ali_gpio_set_value(tp->gpio_vpp_pos, tp->gpio_vpp_pol);
		smc_debug(KERN_INFO"SMC MISC: In %s SMC VPP High! ", __func__);
	}
    udelay(DATV_IO2VCC>>1);
...
{    
	...
	{
		clk = DFT_WORK_CLK;
		smc_debug(KERN_INFO "[%s %d]SMC MISC: clk is 0, use default 3600000! \n", __FUNCTION__, __LINE__);
	}
...
{    
	...
	*/
	...
	writel(9600, p + REG_CBWTR0);	//20160226, set 9600etu for atr wait time
	writeb(0x49, p + REG_RCVPR);		//set value1_r to 4 and glitch_v to 3
	writeb(0x33, p + REG_RXTX_PP);	    //set rxpp to 3 and txpp to 3
}
...
int smc_misc_is_card_insert(struct smc_device *dev)
{
	...
	mdelay(1);//delay 1ms before read REG_ICCSR for smc stable
...
void smc_misc_set_card_flags(struct smc_device *dev, unsigned long flags)
{
    ...
    if (waitqueue_active(&dev->smc_wq_head))//check waitqueue isn't empty
        wake_up(&dev->smc_wq_head);//wake up task in waitqueue
} 
        ...
        (a_pps_buf[1] & 0xf) == (a_pps_echo[1] & 0xf))
	{
		if (PPS0 & 0x10)
		{
			if ((a_pps_buf[1] & 0x10) == (a_pps_echo[1] & 0x10))
			{
				ret = 0;
				*b_need_reset_etu = 1;
				smc_debug(KERN_INFO "SMC MISC: In [ %s %d ], PPS SUCCESS!\n", __func__, __LINE__);
			}
			else
			{
				ret = 0;
				*b_need_reset_etu = 0;
				smc_debug(KERN_INFO "SMC MISC: In [ %s %d ], PPS SUCCESS, USE default Fi, Di!\n", __func__, __LINE__);
			}			
		}
		else
		{
...
void smc_misc_set_etu(struct smc_device *dev, unsigned long etu)
{
	...
	writew(tp->smc_etu, p + REG_ETU0);
	smc_debug(KERN_INFO "SMC MISC: In [ %s %d ], SET etu= %d SUCCESS!\n", __func__, __LINE__,tp->smc_etu);
}
