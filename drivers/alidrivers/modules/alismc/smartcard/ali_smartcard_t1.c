...
{	
	smc_t1_set_defaults(t1);//add on 20150804, If don't set t1->ns=0 after reset card, it will cause T1 transfer error. And it may be adjust new IFSC/IFSD after reset
                            ...
                            UINT8 *rblock, UINT32 rmax, UINT32 *ractual)
{
	...
	if (1 != tp->T)
	{
		smc_error("[%s]line=%d, tp->T!=1\n", __FUNCTION__, __LINE__);
		return -EINVAL;
	}
	
	if ((NULL == sblock) || (NULL == rblock))
	{
		smc_error("[%s]line=%d, (NULL == sblock) || (NULL == rblock)\n", __FUNCTION__, __LINE__);
		return -EINVAL;
	}
	...
	//20150818
	//u32_actual = smc_dev_read(dev, rblock, rmax);//before modify

	if (3!=smc_dev_read(dev, rblock, 3))//read first 3 bytes to get length
	{
		smc_debug(KERN_ERR "SMC T1: In %s Read fetal error!\n", __func__);
		return -EIO;
	}
	u32_actual = smc_dev_read(dev, rblock + 3, rblock[2] + t1->rc_bytes);//read the infomation and epilogure filed
	u32_actual += 3;	
	//20150818
...
#define T1_R_BLOCK_BUILD \
	...
	for ( ; ; )
    {
		...
		case T1_I_BLOCK:
			...
			if (smc_t1_buf_put(&rbuf, rdata + 3, rdata[LEN]) < 0)
			{
				smc_debug("[ %s %d ], buffer overrun by %d bytes\n", __FUNCTION__, __LINE__, 
