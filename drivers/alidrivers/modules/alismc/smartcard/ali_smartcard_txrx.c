                                    ...
                                    int *wait_ret, UINT16 *actsize)
{
    ...
    t_wait_tmo = tp->first_cwt + 5;//add extra time for max character interval    
    ret_wq = wait_event_timeout(dev->smc_wq_head, dev->wq_flags & (SMC_REMOVED | \
                                SMC_RX_FINISHED | SMC_RX_BYTE_RCV), t_wait_tmo);//if the condition is not meet, the process will sleep
...
int smc_txrx_data_timeout(struct smartcard_private *tp, size_t size)
{
	...
	smc_debug(KERN_ERR "SMC TXRX:line=%d, in %s, tp->first_cwt=%d, size=%d!\n", __LINE__, __FUNCTION__, tp->first_cwt, size);
	...
	while (tp->smc_rx_head < ((UINT32)size))
	{
		...
		//if (u32_smc_rd_tmo < (jiffies - u32_tmp_rd_tick))
		if (5+u32_smc_rd_tmo < ((jiffies - u32_tmp_rd_tick)*1000)/HZ)
		{	
			smc_debug(KERN_ERR "SMC TXRX: line=%d, in %s, read timeout!\n", __LINE__, __FUNCTION__);
			return 0;
		}
	}
...
static INT32 smc_txrx_process_bytes(struct smc_device *dev, UINT8 INS, 
                                        INT32 num_to_transfer, UINT8 status[2])
{
	INT32 r = 0;
	UINT8 buff = 0;
                                                           ...
                                                           int num_to_transfer, int b_write)
{
	...
	for ( ; ; )
	{
		...
		{
			ret_size = smc_dev_read(dev, p_iso_transfer->response + u16_length, 
				u16_temp_length);
			if (0 == ret_size)
			{
			...
			if ((u16_temp_length - ret_size) > 0)
			{
				smc_debug("no enough data, reset card, for A.61. u16_temp_length=0x%x, ret_size=0x%x\n",
					u16_temp_length, ret_size);
                return -EIO;
			}
...
#else
	//#65804  fixed iso_transfer_t1 is too late
	if (0 == smc_dev_write(dev, p_iso_transfer->command, p_iso_transfer->num_to_write))
	{
		smc_debug(KERN_ERR "SMC T1: In %s Write error!\n", __func__);	
		return SMART_WRITE_ERROR;
	}

	memset(p_iso_transfer->response, 0, p_iso_transfer->num_to_read);

	if (3!=smc_dev_read(dev, p_iso_transfer->response, 3))//read first 3 bytes to get length
	{
		smc_debug(KERN_ERR "SMC T1: In %s Read fetal error!\n", __func__);
		return -EIO;
	}
	p_iso_transfer->actual_size = smc_dev_read(dev, p_iso_transfer->response + 3, p_iso_transfer->response[2] + t1->rc_bytes);//read the infomation and epilogure filed
	p_iso_transfer->actual_size += 3;	
#endif
