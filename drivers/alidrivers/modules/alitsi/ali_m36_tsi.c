...
struct ali_m36_tsi_dev
{
	...
	/* mutiple process forbid to set different value, this member value only use in ci */
    int spi_0_1_swaped;
...
static int ali_tsi_probe(struct platform_device * pdev)
{
    int                     result;
    ...
    */
	...
	result = of_get_major_minor(pdev->dev.of_node,&tsi->dev_id, 
			0, 1, "ali_m36_tsi_0");
	if (result  < 0) {
...
static int ali_tsi_remove(struct platform_device * pdev)
{
    printk("%s\n", __FUNCTION__);
	return 0;
}

static const struct of_device_id ali_tsi_of_match[] = {
       { .compatible = "alitech, tsi", },
       {},
};

MODULE_DEVICE_TABLE(of, ali_tsi_of_match);

static struct platform_driver ali_tsi_platform_driver = {
	.probe   = ali_tsi_probe, 
	.remove   = ali_tsi_remove,
	.driver   = {
			.owner  = THIS_MODULE,
			.name   = "ali_tsi",
			.of_match_table = ali_tsi_of_match,
	},
};

module_platform_driver(ali_tsi_platform_driver);
MODULE_AUTHOR("ALi Corporation, Inc.");
MODULE_DESCRIPTION("ALI TSI driver");
MODULE_LICENSE("GPL");
