...
#include "../platform/arm_ali/platform_ali.h"
...
#define MAX_FREQ_LIMIT /*Added by Allen 20170328, for Thermal freq limitation */
extern unsigned int max_freq_internal;
	...
	if (0 == current_fps) {
		...
		if (current_gpu_util > over_perform_boundary_value) {
			if (cur_clk_step != gpu_clk->num_of_steps - 1) {
				clock_changed = true;
#ifndef ALI_INTERNAL_GATED_TEST
...
real_setting:
#ifdef MAX_FREQ_LIMIT
MALI_DEBUG_PRINT(3, ("clock_step %d, max_freq_internal %d\n ",clock_step,max_freq_internal));
	if (clock_step > (int)max_freq_internal){
		clock_step = (int)max_freq_internal;
		clock_changed = true;
 	 }
#endif	
#ifndef ALI_INTERNAL_GATED_TEST
...
#else
	if (1) {
		clock_step = (clock_step +1)%7;
		printk("<ALi_DEBUG>clock_step = %d\n", clock_step);
#endif
...
#ifdef MAX_FREQ_LIMIT	
	unsigned int clk_step_apply = 0;
#endif
	cur_clk_step = mali_gpu_get_freq();
#ifdef MAX_FREQ_LIMIT
	if((gpu_clk->num_of_steps-1) > max_freq_internal)	clk_step_apply = max_freq_internal;
	else							clk_step_apply = gpu_clk->num_of_steps-1;

	if (cur_clk_step != clk_step_apply) {
		mali_gpu_set_freq(clk_step_apply);

		_mali_osk_profiling_add_event(MALI_PROFILING_EVENT_TYPE_SINGLE |
					      MALI_PROFILING_EVENT_CHANNEL_GPU |
					      MALI_PROFILING_EVENT_REASON_SINGLE_GPU_FREQ_VOLT_CHANGE, gpu_clk->item[clk_step_apply].clock,
					      gpu_clk->item[clk_step_apply].vol / 1000, 0, 0, 0);
	}
