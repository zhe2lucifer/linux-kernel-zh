...
#ifndef LINUX_buildroot_S3922	//in buildroot S3922, we get interrupt controller to handle this
	printk("Enable SOC GPU interrupt mask !!!\n");    
#ifdef CHIP_C3921
	ALI_HWREG_SET_UINT32(0x003FFFFF, 0x2A8); // force enable GPU interrupt mask    
	ALI_HWREG_SET_UINT32(ALI_HWREG_GET_UINT32(ALI_GPU_CTRL_REG)|0x00200000, ALI_GPU_CTRL_REG); // force set Mali at 297Mhz    
	ALI_HWREG_SET_UINT32(0xC0740800, 0x4E0); // PLL setting at 351Mhz    
#endif

#ifdef CHIP_S3922
	ALI_HWREG_SET_UINT32(0x0000E5C0, 0x40); // force enable GPU interrupt mask    	
#endif

#else	//#ifndef LINUX_buildroot_S3922

	if(ali_sysreg_mapping_init())
	{
		printk("<ALi><Mali>in %s, SOC register mapping done\n", __func__);		
	}
