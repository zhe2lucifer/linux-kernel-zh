...
static struct
{
	UINT32 reg_base;
	int    irq;
} i2c_scb_reg[I2C_SCB_NUM] = {{0x18018200,INT_ALI_SCB1},{0x18018700,INT_ALI_SCB2},{0x18018b00,INT_ALI_SCB3}, {0x18018d00,-1}};

#ifdef CONFIG_ALI_STANDBY_TO_RAM
#define MAX_REG_OFF  0x32
struct ali_i2creg_cfg{
	int nr;
	unsigned char ali_i2c_cfg[MAX_REG_OFF];
};
static struct ali_i2creg_cfg ali_i2creg_cfgs[I2C_SCB_NUM];
#endif
...
{
	int i, j, k, sum,num,timeout;
	unsigned char isr1_status;
	...
	for (j = I2C_SCB_RETRY; j > 0; j--)
	{
		...
		SCB_WRITE8(id, SCB_IER, 0x00);
		SCB_WRITE8(id, SCB_ISR, 0xff);
		SCB_WRITE8(id,SCB_ISR1,0xff);
		SCB_WRITE8(id,SCB_IER1,0x00);
		SCB_WRITE8(id,SCB_FIFO_TRIG_LEVEL,0);
		...
		if(len>16)
		{
			SCB_WRITE8(id,SCB_FIFO_TRIG_LEVEL, 8);
		}
		...
		if(len>16)
		{
			sum =(len & 0x07)?(len/8+1):(len/8);
			for(k=2;k<sum;k++) 
			{
				num =(len-k*8 >8)?(8):(len-k*8);
				for (timeout = 10000; timeout > 0; timeout--)
				{
					isr1_status =SCB_READ8(id, SCB_ISR1);
					if ( isr1_status & 0x01)
					{
						SCB_WRITE8(id,SCB_ISR1,isr1_status);//clear
						break;
					}
					ndelay(100);
				}
				...
				for (i = 0; i < num; i++)
				{
					...
					SCB_WRITE8(id, SCB_FDR, data[i+k*8]);
				}
 ...
 */
static u32 ali_i2c_func(struct i2c_adapter *adapter)
{
...
{
  struct i2c_adapter *adapter = platform_get_drvdata(pdev);
  struct ali_i2creg_cfg  *ali_reg_cfg;
  int i;
  
  if(adapter->nr > I2C_SCB_NUM)
  {
  	dev_err(&pdev->dev, "Adapter %s Num lager than I2C_SCB_NUM\n",
				adapter->name);
	return 0;
  	} 
  ali_reg_cfg = &(ali_i2creg_cfgs[adapter->nr]);
  for(i=0;i<MAX_REG_OFF;i++)
  	ali_reg_cfg->ali_i2c_cfg[i] = SCB_READ8(adapter->nr,i);
...
static int ali_i2c_resume(struct platform_device *pdev)
{
  ...
  ali_reg_cfg = &(ali_i2creg_cfgs[adapter->nr]);
  for(i=0;i<MAX_REG_OFF;i++)
  	SCB_WRITE8(adapter->nr, i, ali_reg_cfg->ali_i2c_cfg[i]);
