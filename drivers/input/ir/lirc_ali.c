...
#else
#include "../../../arch/mips/include/asm/dec/system.h"
#endif
...
#define IRC_EN (1 << 7) 
#define INT_STATE_FIFO (1) 
#define INT_STATE_TIMEOUT (2)
#define INT_STATE_TIMEOUT_AND_FIFO (3)
...
static long ir_ioctl(struct file *filep, unsigned int cmd, unsigned long arg);
#else
...
static int ali_ir_resume(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "Failed to register ali_m36_ir interrupt %ld \n", g_ir_irq_num);
...
static long delta(struct timeval *tv1, struct timeval *tv2)
{
  ...
  *
  */
extern bool repeat_key_flag;
extern unsigned int g_repeat_key_value;
static irqreturn_t ali_m36_ir_interrupt(int irq, void *dev_id)
{
	volatile unsigned char int_status;
	static struct timeval last_tv;
	struct timeval curr_tv;
	unsigned long deltv = 0, last_deltv = 0;
	lirc_t val;
	...
	case INT_STATE_TIMEOUT_AND_FIFO:
		if ((INT_STATE_TIMEOUT == last_int_status) \
			|| (INT_STATE_TIMEOUT_AND_FIFO == last_int_status))/*Repeat key case.*/
		{
			do_gettimeofday(&curr_tv);
			deltv = delta(&last_tv, &curr_tv);

			//printk(KERN_EMERG "\n<Kernel> INT_STATE_TIMEOUT int deltv: %d\n", (unsigned int)deltv);
			if((true == repeat_key_flag) \
				&& ((deltv < 88000) || (deltv > 120000)))/*88ms~120ms is valid repeat gap.*/
			{
				repeat_key_flag = false;
				local_irq_disable();
				mdelay(200);
				local_irq_enable();
				read_fifo();/*Fetch data to IR buffer.*/
				do_gettimeofday(&curr_tv);//chuhua
				//printk(KERN_EMERG "<Kernel> INT_STATE_TIMEOUT Report release event!\n");
				break;
			}
	...
	case INT_STATE_FIFO:
		if (INT_STATE_TIMEOUT == last_int_status)/*New key case.*/
		{
			...
			//printk(KERN_EMERG "\n<Kernel>INT_STATE_FIFO int deltv: %d\n", (unsigned int)deltv);
			if((true == repeat_key_flag) \
				&& ((deltv < 88000) || (deltv > 120000)))/*88ms~120ms is valid repeat gap.*/
			{
				repeat_key_flag = false;
				...
				read_fifo();/*Fetch data to IR buffer.*/
				do_gettimeofday(&curr_tv);//chuhua
				//printk(KERN_EMERG "<Kernel> INT_STATE_FIFO Report release event!\n");
				break;
			}
	...
	//printk(KERN_EMERG "last_int_status: %d, int_status: %d\n", last_int_status, int_status);
	last_int_status = int_status;
	last_deltv = deltv;
	//spin_unlock(&lock);
