...
static struct spi_nand_flash spi_nand_table[] = {
       ...
       SPI_NAND_INFO("GD5F 128MiB 3.3V", 0xC8, 0XB1, 2048, 128, 64, 1024,
                       0),
       SPI_NAND_INFO("GD5F 128MiB 3.3V", 0xC8, 0XD1, 2048,  64, 64, 1024,
                       0),
       SPI_NAND_INFO("GD5F 128MiB 3.3V", 0xC8, 0XF1, 2048,  64, 64, 1024,
                       0),
       {.name = NULL},
};
                               ...
                               u32 colunm, bool raw, int *corrected)
{
	...
	/* support quad access */
	if ((chip->spi->mode & SPI_RX_QUAD) == SPI_RX_QUAD)
	{
		ret = chip->enable_qe(chip);
	       	if (ret < 0) {
	                pr_debug("enable qe failed\n");
	       }   
	}
	...
	if ((chip->spi->mode & SPI_RX_QUAD) == SPI_RX_QUAD)
       	{
		ret = chip->disable_qe(chip);
	       	if (ret < 0) {
	                pr_debug("disable qe failed\n");
	       }   
	}
                               ...
                               u32 column, bool raw)
{
       ...
       /* support quad access */
       if ((chip->spi->mode & SPI_TX_QUAD) == SPI_TX_QUAD)
	{
		ret = chip->enable_qe(chip);
	       	if (ret < 0) {
	                pr_debug("enable qe failed\n");
...
/**
 * spi_nand_enable_qe  enable internal qe
 * @chip: SPI-NAND device structure
 * Description:
 *   There is one bit( bit 0x1 ) to set  the internal QE.
 *   
 */
static int spi_nand_enable_qe(struct spi_nand_chip *chip)
{
...
/**
 * spi_nand_disable_qe  enable internal qe
 * @chip: SPI-NAND device structure
 * Description:
 *   There is 0 bit( bit 0x1 ) to clear the internal QE.
 *   
 */

static int spi_nand_disable_qe(struct spi_nand_chip *chip)
{
       struct spi_device *spi = chip->spi;
       int ret;
       u8 otp = 0;
...
/*
 * spi_nand_program_data_to_cache_x4--to write a page to cache
 * @chip: SPI-NAND device structure
 * @page_addr: page to write
 * @column: the location to write to the cache
 * @len: number of bytes to write
 ...
 *
 * Description:
 *   The write command used here is 0x32--indicating that the cache is
 *   cleared first.
 *   Since it is writing the data to cache, there is no tPROG time.
 *   This is X4 I/O to SI/SO 
 */
static int spi_nand_program_data_to_cache_x4(struct spi_nand_chip *chip,
               u32 page_addr, u32 column, size_t len, u8 *wbuf)
{
       struct spi_nand_cmd cmd = {0};
       struct spi_device *spi = chip->spi;

       cmd.cmd = SPINAND_CMD_PROG_PAGE_CLRCACHE_X4;
       cmd.n_addr = 2;
       cmd.addr[0] = (u8)(column >> 8);
       if (chip->options & SPINAND_NEED_PLANE_SELECT)
               cmd.addr[0] |= (u8)(((page_addr >>
...
static void spi_nand_set_defaults(struct spi_nand_chip *chip)
{
       ...
       if (!chip->store_cache)
       	{
       	       if (spi->mode & SPI_TX_QUAD)
	       		chip->store_cache = spi_nand_program_data_to_cache_x4;
	       else
       ...
       if (!chip->enable_qe)
               chip->enable_qe = spi_nand_enable_qe;
       if (!chip->disable_qe)
               chip->disable_qe = spi_nand_disable_qe;	   
