...
#define SCI_16550UART_RX_BUF_SIZE	2048
...
static struct ktermios pm_termios;
#endif 
...
{	
	 ...
	 * clear it. */
    while (max_count-- > 0) {
	    if (((interrupt_source = (SCI_READ8(port, SCI_16550_UIIR) & 0x0f)) & 1) == 0) {
...
static int ali_uart_startup(struct uart_port *port)
{
	...
	if (retval) {
		printk("ali_uart: ali_uart_startup - Can't get irq\n");
		return retval;
	}
	return retval;
} 
...
static void ali_uart_set_termios(struct uart_port *port, struct ktermios *termios, struct ktermios *old)
{
    ...
    #ifdef CONFIG_ALI_STANDBY_TO_RAM
    memcpy(&pm_termios, termios, sizeof(struct ktermios));
    #endif
	/* Word Length Select */
	...
	SCI_WRITE8(port, SCI_16550_UFCR, 0x07);		/* 0x00: no FIFO, 0x47:Reset FIFO */
...
static void __devinit ali_init_port(void)
{
	...
	for (i = 0; i < ALI_UART_NR; i++) {
		...
		port->membase	= (unsigned char __iomem *)sci_16550_reg[i].reg_base;		/* Memory mapped */ 		
...
#ifdef CONFIG_ALI_STANDBY_TO_RAM
static int ali_uart_suspend(struct device *dev)
{

	return 0;
}

static int ali_uart_resume(struct device *dev)
{
	struct ali_uart_port *port = dev_get_drvdata(dev);
	struct uart_port *u = &port->uart;
	unsigned long flags;
   
	spin_lock_irqsave(&u->lock, flags);		
    /* Disable all interrupt */
   	SCI_WRITE8(u, SCI_16550_UIER, 0x00);
    /* Enable setup baud rate */
	SCI_WRITE8(u, SCI_16550_ULCR, 0x9b);
    /* set divsor latch */
    SCI_WRITE8(u, SCI_16550_UDLL, 0x01);
    SCI_WRITE8(u, SCI_16550_UDLM, 0x00);
    SCI_WRITE8(u, SCI_16550_ULCR, 0x1b);
    
    SCI_WRITE8(u, SCI_16550_UFCR, 0x07); /* 0x00: no FIFO, 0x47: Reset FIFO */
    SCI_WRITE8(u, SCI_16550_ULSR, 0x00);   /* Reset Line Status */
    SCI_WRITE8(u, SCI_16550_UMCR, 0x03);   /* Set modem control */
    /* Enable receiver interrupt */
    SCI_WRITE8(u, SCI_16550_UIER, 0x05);   /* Enable RX & timeout interrupt */
	/* signon message or measure TIMEOUT */	
	spin_unlock_irqrestore(&u->lock, flags);
    ali_uart_set_termios(u,&pm_termios,&pm_termios);
	return 0;
}


static const struct dev_pm_ops ali_uart_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(ali_uart_suspend, ali_uart_resume)
};
