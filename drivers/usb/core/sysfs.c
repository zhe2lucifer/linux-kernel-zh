...
static ssize_t ali_usb_if_test_store(struct device *dev,
                                   struct device_attribute *attr,
                                   const char *buf, size_t count)
{
    struct usb_device	*udev = to_usb_device(dev);
    struct usb_hcd *hcd = bus_to_hcd(udev->bus);
    int ret = 0, testmode = 0, port = 0, restore = 0;
    char b[50] = {0};

    sscanf(buf, "%s %d %d", b, &port, &restore);

    pr_debug("[%s] test port:%d, restore:%d\n", __func__, port, restore);
    ...
    else if (!strncmp(buf, "test_port_reset", 15))
        testmode = TEST_PORT_RESET;
    else if (!strncmp(buf, "test_suspend_resume", 19))
        testmode = TEST_SUSPEND_RESUME;
    else if (!strncmp(buf, "test_get_descriptor", 19)) {
        if(hcd->driver->hub_control)
            ret = hcd->driver->hub_control(hcd, SetPortFeature, USB_PORT_FEAT_TEST,
                                           (0x06<<8)|(port+2), NULL, 0);
        else
            pr_debug("hub_control is null.\n");
        return count;
    } else
        return count;
    ...
    if(hcd->driver->usb_if_test)
        ret = hcd->driver->usb_if_test(hcd, port, testmode, restore);
    else
        printk("test_packet is null\n");

    usb_unlock_device(udev);
    return count;
}
static DEVICE_ATTR_WO(ali_usb_if_test);
#endif
#ifdef	CONFIG_PM
