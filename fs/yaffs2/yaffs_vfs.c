...
#define YPROC_uid(p) (YCRED(p)->fsuid.val)
#define YPROC_gid(p) (YCRED(p)->fsgid.val)
#else
...
#ifdef CONFIG_UIDGID_STRICT_TYPE_CHECKS
	gid_t gid =
	    (dir->i_mode & S_ISGID) ? EXTRACT_gid(dir->i_gid.val) : YPROC_gid(current);
#else
	gid_t gid =
...
#ifdef CONFIG_UIDGID_STRICT_TYPE_CHECKS
		inode->i_uid.val = MAKE_uid(obj->yst_uid);
		inode->i_gid.val = MAKE_gid(obj->yst_gid);
#else
